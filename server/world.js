...
function createWorld(store) {
   ...
   */
  /**
   * 지난 시즌에 이 사람이 어디까지 갔나 — 세이브를 지우기 **전에** 한 줄로 남긴다.
   *
   * 0.70.15 — 시즌이 끝나면 세이브를 통째로 비우므로, 그 뒤에는 아무것도 물어볼
   * 수 없다. 그래서 지우는 그 자리에서 요약을 떠 둔다. 다음 접속 때 직업을
   * 고르는 창이 이것을 보여 준다 — "지난 시즌엔 여기까지 갔습니다" 한 줄이 없으면
   * 처음부터 다시 시작하는 자리가 그냥 허전하다.
   *
   * 무겁게 남기지 않는다. 숫자 몇 개와 이름 몇 개뿐이다.
   */
  function summarizeSave(save, season) {
    if (!save || typeof save !== 'object') return null;
    const reached = (save.quests && save.quests.reached) || {};
    let stage = 0;
    let floor = 0;
    for (const id of Object.keys(reached)) {
      if (!reached[id]) continue;
      const f = /^field_(\d+)$/.exec(id);
      if (f) stage = Math.max(stage, Number(f[1]));
      const d = /^dungeon_(\d+)$/.exec(id);
      if (d) floor = Math.max(floor, Number(d[1]));
    }
    const kills = (save.quests && save.quests.kills) || {};
    const downed = Object.keys(save.bossFirstKill || {});
    return {
      season,
      name: save.name || '',
      ...
      stage,           // 들판을 몇 단계까지 밟았나
      floor,           // 지하감옥 몇 층까지 내려갔나
      bosses: downed,  // 처음으로 눕힌 보스들
      dragons: (Number(kills.great_dragon) || 0) + (Number(kills.elder_dragon) || 0),
      endedAt: Date.now(),
    };
  }
  ...
  async function resetAllSaves() {
    ...
    for (const id of ids) {
      ...
      await store.set(id, {
        ...
        // 지난 시즌 요약. 직업을 고르고 나면 지운다(takeSeasonNotice 참고).
        lastSeason: summarizeSave(acct.save, meta.season - 1 > 0 ? meta.season - 1 : meta.season),
      });
      touched++;
    }
  ...
  async function takeSeasonNotice(id) {
    ...
    const last = acct.lastSeason || null;
    const next = { ...acct };
    delete next.seasonNotice;
    delete next.lastSeason;
    await store.set(id, next);
    // 예전 모양(숫자 하나)을 쓰던 자리가 있어서, 번호는 그대로 두고 요약만 얹는다.
    return last ? { season: n, last } : n;
  }
