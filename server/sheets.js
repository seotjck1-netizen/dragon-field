...
const ITEM_HEAD = [
  ...
  // 0.70.13 — '관통' 칸이 crit 뒤에 들어왔다(활의 기본 옵션).
  //   ⚠ 칸이 늘어났으므로 그 뒤 칸의 자리가 모두 하나씩 밀렸다.
  //     아래 rowsToItems 의 번호를 함께 보라.
  '가격', 'atk', 'def', 'hp', 'spd', 'crit', '관통', '치명피해', '회복HP', '마법', '설명', '기타',
];
...
function rowsToItems(rows, original = {}) {
  ...
  body.forEach((r, i) => {
    ...
    put(stats, 'pierce', readNum(r[14]));
    put(stats, 'critDmg', readNum(r[15]));
    ...
    const heal = readNum(r[16]);
    if (heal != null) def.use = { hp: heal };
    put(def, 'magic', readText(r[17]));
    put(def, 'desc', unescNl(r[18]));
    Object.assign(def, unpackExtra(r[19], at));
...
const MON_HEAD = [
  'id', '이름', '레벨', 'hp', 'atk', '실제 값', 'def', 'spd', 'crit',
  // 0.70.14 — '마법저항' 칸이 '분노' 뒤에 들어왔다. 그 뒤 칸의 자리가 하나씩 밀렸다.
  'exp', '골드', '이동', '속성', '보스', '정예', '전투크기', '분노', '마법저항',
  '필드그림', '전투그림', '등장대사', '쓰러질때', '기타',
];
// '실제 값' (0.70.6) — **읽기 전용.** 게임은 이 칸을 읽지 않는다.
//
// 왜 넣었나: 시트의 hp·atk·def 는 **맨몸 값**이다. 실제로 만나는 숫자는
// 그 몬스터가 사는 땅의 보정(maps.json 의 power)을 곱한 것이다.
// 지하감옥 5층은 ×18.6 이라 시트에 atk 500 을 적으면 싸움에서는 9,305 가 된다.
// 이 차이를 모르면 "고룡 공격력을 1000 으로 올렸는데 5층 주인(500)보다 약하다"
// 같은 일이 생긴다 — 실제로 0.70.5 에 그 질문을 받았다. 숫자를 고치는 자리
// **바로 옆**에 실제 값을 띄워 두면 그 착각이 아예 생기지 않는다.
//
// 여기에 무엇을 적든 다음번 `npm run sheets` 에 다시 계산되어 덮인다.
...
// ── 제목 줄 밑에 붙는 '칸 설명' 한 줄 (0.70.4) ────────────────
//
// 왜 시트 안에 두는가: 설명이 README 에만 있으면, 시트를 여는 사람은
// 그 파일을 열어 볼 생각을 못 한다. 특히 '전투크기' 와 '분노' 는 이름만
// 보고는 무엇에 걸리는 값인지 알 수가 없다 — 하나는 **그림 크기**일 뿐이고
// 하나는 **판이 갈수록 세지는 정도**다. 둘을 바꿔 짐작하면 난이도를 엉뚱한
// 칸으로 만지게 된다(0.70.3 에 분노가 안 걸리고 있었던 것도 그래서 오래 갔다).
//
// ⚠ **첫 칸(id)은 반드시 비운다.** 읽는 쪽(rowsToMonsters)은 id 가 빈 줄을
//   그냥 지나친다 — 그 성질에 기대어 "게임이 안 읽는 줄"을 만든다.
//   첫 칸에 무언가를 적으면 그 순간 이 줄이 **몬스터 한 마리가 된다.**
const MON_NOTE = [
  '', // ← id. 비어 있어야 게임이 이 줄을 건너뛴다. 절대 채우지 말 것.
  '↓ 칸 설명 (이 줄은 게임이 읽지 않습니다. 지워도 되고, 그대로 둬도 됩니다)',
  '화면에 뜨는 숫자 = 이 레벨 + 땅 보정',
  '체력', '공격력',
  '[자동] 왼쪽 공격력에 그 땅의 보정을 곱한 **싸움에서 만나는 값**. 적어도 소용없습니다',
  '방어력', '속도(선공 순서)', '치명타 확률 0~1',
  '잡으면 주는 경험치', '잡으면 주는 골드',
  'chase=쫓아옴 / wander=어슬렁 / still=가만히',
  'magic 이면 공격이 통째로 마법 — 상대의 마법 저항(지능)이 깎는다. 비우면 물리',
  '예/아니오', '예/아니오',
  // ⚠ 이 둘이 이 줄의 이유다.
  '[보기용] 전투 화면에서 그림을 몇 배로 그릴지. 1=보통, 2.1=두 배 조금 넘게. 싸움 계산에는 안 걸린다',
  '[싸움] 제 차례에 한 대 칠 때마다 공격력이 이만큼씩 곱으로 붙는다. 0.1=1.1배씩(10대째 2.36배). 비우면 0',
  '[싸움] 받는 마법 피해 감소 0~1. 0.3=30% 덜 아프다. 전투에서는 80%에서 멎는다. 비우면 0',
  '들판 그림 이름', '전투 그림 이름',
  '보스가 나타날 때 한 줄', '보스가 쓰러질 때 한 줄',
  '표로 못 펴는 값(JSON). 건드리지 마세요',
];
const MON_KNOWN = ['name', 'level', 'stats', 'exp', 'gold', 'moveStyle', 'school', 'boss', 'elite', 'battleScale', 'rage', 'magicResist', 'sprite', 'battleSprite', 'intro', 'defeat'];

/**
 * 몬스터가 **어느 땅에 나오는지**와 그 땅의 보정을 모은다.
 *
 * 한 마리가 여러 땅에 나올 수 있다(슬라임은 1단계에도, 3단계에도 산다).
 * 그럴 때는 가장 약한 곳과 가장 센 곳을 함께 보여 준다 — 하나만 보여 주면
 * "내 시트 값이 왜 저 숫자가 되지" 가 또 생긴다.
 *
 * @param {object} maps  maps.json
 * @returns {Map<string, {min:number, max:number, where:string, count:number}>}
 */
function monsterPowers(maps) {
  const out = new Map();
  const add = (id, name, power) => {
    if (!id) return;
    const p = Number(power) || 1;
    const was = out.get(id);
    if (!was) { out.set(id, { min: p, max: p, where: name, count: 1 }); return; }
    was.count += 1;
    if (p < was.min) was.min = p;
    if (p > was.max) { was.max = p; was.where = name; }
  };
  for (const def of Object.values((maps && maps.maps) || {})) {
    const name = def.name || '';
    for (const id of def.monsters || []) add(id, name, def.power);
    if (def.boss) add(def.boss, name, def.power);
    if (def.timedBoss && def.timedBoss.monster) add(def.timedBoss.monster, name, def.power);
  }
  return out;
}

const comma = (n) => Math.round(n).toLocaleString('en-US');
const times = (p) => (Math.round(p * 10) / 10).toLocaleString('en-US');

/** '실제 값' 칸 한 줄. 계산만 한다 — 게임은 이 글을 읽지 않는다. */
function realAtkCell(atk, at) {
  if (atk == null || atk === '' || !Number.isFinite(Number(atk))) return '';
  if (!at) return '(어느 땅에도 안 나옵니다)';
  const a = Number(atk);
  if (at.min === at.max) {
    return at.max === 1
      ? `→ 실전 ${comma(a)} (보정 없음 · ${at.where})`
      : `→ 실전 ${comma(a * at.max)} (×${times(at.max)} · ${at.where})`;
  }
  return `→ 실전 ${comma(a * at.min)}~${comma(a * at.max)} (×${times(at.min)}~${times(at.max)} · ${at.count}곳, 가장 센 곳은 ${at.where})`;
}

function monstersToRows(monsters, maps) {
  const powers = monsterPowers(maps);
  // 제목 줄 바로 밑에 '칸 설명' 한 줄. 첫 칸이 비어 있어 게임은 지나친다.
  const rows = [MON_HEAD, MON_NOTE];
  for (const [id, d] of Object.entries(monsters)) {
    if (id.startsWith('_')) continue;
    const st = d.stats || {};
    rows.push([
      id, d.name || '', d.level ?? '', st.hp ?? '', st.atk ?? '',
      realAtkCell(st.atk, powers.get(id)),
      st.def ?? '', st.spd ?? '', st.crit ?? '',
...
function rowsToMonsters(rows, original = {}) {
  ...
  body.forEach((r, i) => {
    const id = readText(r[0]);
    // 첫 칸이 빈 줄은 지나친다 — '칸 설명' 줄(MON_NOTE)과 사람이 남긴 빈 줄이 여기 걸린다.
    if (!id) return;
    const at = `monsters 시트 ${i + 2}번째 줄`;
    if (out[id]) throw new Error(`${at}: 아이디 '${id}' 가 두 번 나옵니다.`);

    // ⚠ r[5] 는 '실제 값' — **읽지 않는다.** 계산해서 보여 주는 칸이다.
    const stats = {};
    put(stats, 'hp', readNum(r[3]));
    put(stats, 'atk', readNum(r[4]));
    put(stats, 'def', readNum(r[6]));
    put(stats, 'spd', readNum(r[7]));
    put(stats, 'crit', readNum(r[8]));

    const def = { name: readText(r[1]) };
    if (!def.name) throw new Error(`${at}: 이름이 비어 있습니다.`);
    put(def, 'level', readNum(r[2]));
    put(def, 'sprite', readText(r[18]));
    put(def, 'battleSprite', readText(r[19]));
    const was = original[id];
    if (Object.keys(stats).length) def.stats = orderLike(stats, was && was.stats);
    put(def, 'exp', readNum(r[9]));
    put(def, 'gold', readNum(r[10]));
    put(def, 'moveStyle', readText(r[11]));
    put(def, 'school', readText(r[12]));
    if (readBool(r[13])) def.boss = true;
    if (readBool(r[14])) def.elite = true;
    put(def, 'battleScale', readNum(r[15]));
    put(def, 'rage', readNum(r[16]));
    put(def, 'magicResist', readNum(r[17]));
    put(def, 'intro', readText(r[20]));
    put(def, 'defeat', readText(r[21]));
    Object.assign(def, unpackExtra(r[22], at));
    out[id] = orderLike(def, was);
  });
  return out;
}
...
const QUEST_HEAD = [
  ...
  '열림조건',
  // 0.63 — 'O' 면 **재료를 안 가져간다.** 비어 있으면 여느 때처럼 가져간다.
  //
  // 왜 필요한가: '쓸모를 잃은 무기'(dragon_scrap)는 10만 골드짜리 검을 내는 의뢰인데,
  // 그 검은 보석 둘을 박으면 용기사 무기로 **변신한다**(affixes.json 의 변신).
  // 의뢰가 그걸 가져가 버리면 다시 10만을 주고 사야 했다. 값은 검이 아니라
  // 변신에 치르는 것이라, 의뢰에 한 번 보여 주고 돌려받는 것이 맞다.
  '재료유지',
];
...
function questsToRows(quests) {
  ...
  for (const row of (quests && quests['표']) || []) {
    const [id, title, type, target, count, exp, gold, rItem, rCount, desc,
      needLv, choices, unlock, keep] = row;
    rows.push([
...
function rowsToQuests(rows, original = {}) {
  const out = [];
  const seen = new Set();
  const body = dropHeader(rows, QUEST_HEAD, 'quests');
  // 0.64 — 시트에 '재료유지' 칸이 아직 없을 수 있다(그 칸은 0.63 에 생겼다).
  // 없으면 사람이 비운 것이 아니므로 **지금 쓰고 있는 값을 그대로 둔다.**
  // 비워 버리면 옛 시트 하나가 '쓸모를 잃은 무기' 를 다시 가져가게 만든다.
  const hasKeep = hasCol(rows, QUEST_HEAD.indexOf('재료유지'));
  const wasKeep = new Map(
    ((original && original['표']) || []).map((r) => [r[0], !!r[13]])
  );
  body.forEach((r, i) => {
    const id = readText(r[0]);
    if (!id) return;
    ...
    // 'O' · 'o' · '예' 를 모두 참으로 본다 — 사람이 시트에 적는 글자다.
    const keep = hasKeep
      ? /^(o|O|예|Y|y|true)$/.test(readText(r[13]).trim())
      : !!wasKeep.get(id);
    // 뒤 네 칸은 없어도 되는 칸이다. 필요한 만큼만 붙인다.
    // (앞 칸을 건너뛸 수 없으므로, 뒤 칸이 있으면 앞 칸도 기본값으로 채워 넣는다)
    if (needLv != null || choices.length || unlock || keep) row.push(needLv ?? 0);
    if (choices.length || unlock || keep) row.push(choices);
    if (unlock || keep) row.push(unlock);
    if (keep) row.push(1);
    out.push(row);
  });
  return out;
...
const MOD_KEYS = [
  ...
  // evadeRevenge — 0.59 '바람의 걸음'. 회피에 성공한 **다음 한 대**의 피해 배율.
  'evadeBonus', 'evadeRevenge', 'openerBonus', 'openerPowerBonus', 'cleaveBonus', 'chargeBonus',
  // 특성 여섯 갈래가 쓰는 '곱연산' 배율과, 직업 패시브가 여는 확률
  'atkMult', 'defMult', 'potionMult', 'goldFind', 'materialDouble', 'engraveBonus',
  'absorbChance',
  // 0.63 — 매직 투구. 쓰러져도 골드를 안 잃는다.
  'keepGoldOnDeath',
];
...
function rowsToSkills(rows, originals) {
  ...
  body.forEach((r, i) => {
    ...
    // 0.64 — 걸음이 고르지 않은 스킬은 '기타' 칸의 steps 로 온다(보호막 연장).
    // 그것도 엄연한 효과이므로 '아무 일도 안 한다' 로 보면 안 된다.
    const hasEffect = def.effect && Object.keys(def.effect).length;
    const hasSteps = def.steps && Object.keys(def.steps).length;
    if (!hasEffect && !hasSteps) {
...
const CLASS_FIELDS = [
  ...
  ['combat.lastStand', '치명상 버티기', 'num', '쓰러질 피해를 HP 1 로 버틸 확률. 첫 번째는 확정, 그 뒤부터 이 확률'],
  ['combat.lastStandPerDef', '버티기/방어력200', 'num',
   '방어력 200 마다 위 확률에 더해지는 몫. 0.01 = +1%p (방어력 3,800 이면 +19%p)'],
  ['combat.potionPower', '물약 배율', 'num', '물약 회복량 배율. 2 = 두 배'],
  ['combat.opener', '선제 공격 횟수', 'num', '전투 시작하자마자 공짜로 때리는 횟수'],
  ['combat.openerWeapon', '선제 공격 무기', 'text',
   '이 무기 갈래를 들었을 때만 선제 공격이 나간다. bow=활 sword=검 greatsword=대검 staff=지팡이 club=몽둥이. 비우면 아무 무기나'],
...
function dropHeader(rows, head, name) {
  ...
  //
  // ⚠ **뒤에 칸이 모자란 것은 막지 않는다** (0.64).
  //
  //   판을 올리면서 표 맨 뒤에 칸을 하나 더하는 일이 있다(0.63 의 '재료유지').
  //   그러면 아직 안 갈아 끼운 시트는 그 칸이 없는데, 예전에는 여기서 멈춰
  //   **표 전체가 반영을 멈췄다.** 아이템 값 하나 고치려다 시트가 통째로 막힌다.
  //   칸 하나가 없다고 나머지 아홉 탭까지 멎을 이유는 없다.
  //
  //   대신 없는 칸은 **지금 쓰고 있는 값을 그대로 둔다** — 비워 버리면
  //   옛 시트 하나가 새 기능을 조용히 꺼 버린다(그 검을 다시 가져가게 된다).
  //   그 되살리기는 읽는 쪽(rowsToQuests 의 keep)이 한다.
  const cols = first.length;
  for (let i = 0; i < Math.min(head.length, cols); i++) {
...
/**
 * 탭 이름 → 우리가 아는 제목 줄 (0.64).
 *
 * "시트가 우리보다 뒤처졌다" 를 **알려 주기 위해서만** 쓴다. 읽는 일 자체는
 * 못 하게 막지 않는다(위 dropHeader 참고) — 다만 조용히 넘어가면 사람은
 * 자기 시트가 한 판 뒤처진 줄 모르고, 새 줄이 왜 안 나오는지도 모른다.
 */
const HEADS = {
  items: () => ITEM_HEAD,
  monsters: () => MON_HEAD,
  ...
  traits: () => TRAIT_HEAD,
  stats: () => STAT_HEAD,
  engrave: () => ENGRAVE_HEAD,
  config: () => CONFIG_HEAD,
};

/**
 * 그 탭에 **모자란 뒤쪽 칸들**의 이름. 없으면 빈 배열.
 * @returns {string[]}
 */
function missingTailColumns(tab, rows) {
  const make = HEADS[tab];
  if (!make || !rows || !rows.length) return [];
  const head = make();
  const cols = rows[0].length;
  return cols < head.length ? head.slice(cols) : [];
}

/**
 * 시트에 그 자리의 칸이 **실제로 있는가** (0.64).
 *
 * 뒤에 새로 생긴 칸을 읽을 때 쓴다. 없으면 "사람이 비웠다" 가 아니라
 * "아직 그 칸이 없는 시트다" 이므로, 읽는 쪽이 지금 값을 그대로 두어야 한다.
 */
function hasCol(rows, index) {
  return !!(rows && rows.length) && rows[0].length > index
    && readText(rows[0][index]) !== '';
}
...
const SHEETS = {
  ...
  monsters: {
    files: ['monsters.json'],
    label: '몬스터',
    toRows: (all) => monstersToRows(all['monsters.json'], all['maps.json']),
  ...
  quests: {
    ...
    apply: (rows, all) => ({ 'quests.json': { ...all['quests.json'], ['표']: rowsToQuests(rows, all['quests.json']) } }),
