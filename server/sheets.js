...
function rowsToMonsters(rows, original = {}) {
  ...
  body.forEach((r, i) => {
    ...
    put(def, 'sprite', readText(r[17]));
    put(def, 'battleSprite', readText(r[18]));
    const was = original[id];
    if (Object.keys(stats).length) def.stats = orderLike(stats, was && was.stats);
    ...
    put(def, 'intro', readText(r[19]));
    put(def, 'defeat', readText(r[20]));
    Object.assign(def, unpackExtra(r[21], at));
    out[id] = orderLike(def, was);
  });
  return out;
}
