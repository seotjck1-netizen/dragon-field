...
/**
 * 건너뛴 전투만큼 시계를 **앞으로 민다** (0.70.17).
 *
 * ── 왜 이것만은 게임이 보내는 숫자를 받는가 ──────────────────
 * 이 파일의 규칙은 "시간은 서버 시계로만 잰다" 이고, 그 규칙은 그대로다.
 * 그런데 **건너뛴 전투**는 서버 시계로 잴 수가 없다. 전투는 게임 안에서 벌어지고,
 * 서버는 그것이 있었는지조차 모른다. 스킵을 누른 사람은 20초짜리 싸움을 0초에
 * 끝내므로, 타임어택에서 **가만히 보는 사람보다 빨라진다.** 연출을 건너뛴 것이
 * 실력이 되어 버린다.
 *
 * 그래서 게임이 "이만큼 건너뛰었습니다" 를 보내고, 서버가 그만큼 **더한다.**
 *
 * ⚠ 이 값은 속여도 이득이 없다:
 *   · 적게 보내면 → 지금(0.70.16)과 똑같다. 새로 열리는 구멍이 아니다.
 *   · 많이 보내면 → 제 기록이 나빠진다. 스스로를 해치는 거짓말이다.
 *   그래서 받아도 안전하다. 그래도 한 번에 받을 수 있는 몫은 막아 둔다 —
 *   실수로 큰 수가 오면 시계가 몇 년씩 튀어 화면이 이상해지기 때문이다.
 *
 * @param {number} ms 게임이 보낸 건너뛴 시간
 * @returns {object|null} 계정에 덧쓸 조각. 받을 값이 아니면 null.
 */
const OWED_CAP_MS = 10 * 60 * 1000; // 한 번에 10분까지

function addSkipped(acct, ms, now = Date.now()) {
  const add = Math.min(OWED_CAP_MS, Math.max(0, Math.floor(num(ms))));
  if (!add) return null;
  const t = tick(acct, now); // 먼저 이번 토막을 쌓고
  return { ...t, playMs: t.playMs + add }; // 그 위에 건너뛴 몫을 얹는다
}
...
module.exports = { GAP_CAP_MS, OWED_CAP_MS, playedMs, tick, addSkipped, startSession, resetClock, seedMs };
