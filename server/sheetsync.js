 ...
 *
 * 왜 들고 있나: 문서 아이디가 판마다 바뀌는 통에 **서버가 옛 문서를 읽는 일**이
 * 열 번째다. 그 사실은 지금까지 "시트 지금 읽기" 를 눌러야만 보였다 — 누를 이유가
 * 없는 사람은 영영 모른다. 자동 확인이 알아낸 것을 여기 담아 두고,
 * 운영자 창 표 단이 열릴 때 그대로 보여 준다.
 */
let lastLook = { at: 0, behind: [], errors: [] };
function noteLook(next) {
  lastLook = { at: Date.now(), behind: [], errors: [], ...next };
}
function getLastLook() {
  return lastLook;
}
...
async function pullOnce(env = process.env, note = '', onBeforeWrite = null) {
  ...
  try {
    ...
    noteLook({ errors: [err.message] });
    return { ok: false, errors: [err.message] };
  }

  const next = { ...current };
  const texts = {};
  // 0.64 — 시트가 우리보다 뒤처진 탭들. 막지는 않지만 반드시 알린다.
  const behind = [];
  for (const tab of TABS) {
    ...
    const lack = sheets.missingTailColumns(tab, rows);
    if (lack.length) behind.push(`'${tab}' 탭에 ${lack.join(' · ')} 칸이 없습니다`);
    try {
  ...
  // 뒤처진 탭 이야기를 경고에 얹는다 — 사람이 표 단에서 곧바로 본다.
  const warnings = behind.length
    ? [
      ...behind,
      '이 시트는 지금 판보다 뒤처져 있습니다. 없는 칸은 지금 값을 그대로 두었지만,'
      + ' **새로 생긴 줄은 시트에 없으므로 사라집니다.**'
      + ' 통합문서를 새로 받아 올리면(운영자 창 → 계정 의 복사 단추와는 다른 파일입니다)'
      + ' 이 알림이 사라집니다.',
      ...(v.warnings || []),
    ]
    : v.warnings;
  ...
  if (!changed.length) return { ok: true, skipped: true, changed: [], warnings };

  const pub = content.publish(note || '구글 시트에서 반영');
  if (!pub.ok) return { ok: false, errors: pub.errors };
  return { ok: true, version: pub.version, changed, warnings };
}
...
module.exports = {
  ...
  setSavedId, getSavedId, idFromAny, getLastLook };
