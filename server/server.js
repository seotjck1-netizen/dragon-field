...
const accountsync = require('./accountsync.js');
const { createStore } = require('./store.js');
const { createWorld } = require('./world.js');
const playclock = require('./playclock.js');
...
/**
 * 배포된 표에서 카르나크의 체력을 읽는다 (0.70.14).
 *
 * ⚠ 여태 이 값이 **표와 따로 놀았다.** 서버는 700,000 을 들고 있었고 표는
 *   560,000 이었다(0.70.13 에 바꿨다). 그러면 여럿이 함께 때리는 그 고룡만
 *   혼자 다른 몸이 된다 — 화면에는 56만이라 적히는데 서버는 70만을 깎는다.
 *   표를 고치면 서버도 따라오게 한다. 못 읽으면 예전 값으로 돌아간다.
 */
function dragonMaxHpFromTables() {
  try {
    const { files } = content.readAll();
    const hp = ((files['monsters.json'] || {}).great_dragon || {}).stats;
    if (hp && Number(hp.hp) > 0) return Math.round(Number(hp.hp));
  } catch { /* 표를 못 읽으면 아래 기본값 */ }
  return 0;
}
...
const DRAGON = {
  ...
  // 환경변수가 있으면 그것이 먼저다(시험 서버가 값을 낮춰 쓴다).
  // 없으면 **표에 적힌 값**을 쓰고, 표도 못 읽으면 예전 기본값으로 간다.
  //
  // ⚠ 읽는 때가 중요하다. 이 파일이 뜨는 순간에는 server/content 가 아직
  //   비어 있을 수 있다(표를 까는 initFromSource 가 그 뒤에 돈다). 그때 한 번
  //   읽어 두면 영원히 옛 기본값으로 굳는다 — 그래서 **물어볼 때마다** 읽는다.
  get maxHp() {
    return Number(process.env.DRAGON_HP || dragonMaxHpFromTables() || 700000);
  },
...
/**
 * 아이디로 쓸 수 없는 말인가 (0.69).
 *
 * 잣대는 `src/systems/ReservedId.js` **하나**다. 손님(브라우저)과 서버가 같은
 * 함수를 쓴다 — 표를 두 군데 적으면 반드시 갈라진다(0.68 의 타일 글자가 그랬다).
 * 그 파일은 ES 모듈이라 시작할 때 한 번 불러와 둔다.
 *
 * ⚠ **막는 것은 계정 만들기뿐이다.** 접속까지 막으면 진짜 운영자(`admin`)가
 *   제 계정으로 못 들어온다.
 *
 * ⚠ 불러오지 못했으면 **계정 만들기를 막는다**(열어 두지 않는다).
 *   못 거르는 채로 받아 주면 그 사이에 만들어진 `admin1` 은 되돌릴 수 없다.
 *   게임의 나머지는 그대로 돌아간다 — 멎는 것은 가입뿐이다.
 */
let reservedIdReason = null;
let reservedWhy = '아직 불러오지 않았습니다.';

async function loadReserved() {
  const file = path.join(__dirname, '..', 'src', 'systems', 'ReservedId.js');
  const mod = await import(pathToFileURL(file).href);
  if (typeof mod.reservedIdReason !== 'function') {
    throw new Error('reservedIdReason 을 못 찾았습니다.');
  }
  reservedIdReason = mod.reservedIdReason;
  reservedWhy = '';
}
...
function checkNewId(id) {
  checkId(id);
  if (!reservedIdReason) {
    throw new Error(`서버가 아이디 규칙을 못 읽었습니다(${reservedWhy}). 관리자에게 알려 주세요.`);
  }
  const taken = reservedIdReason(id);
...
const api = {
  ...
  '/api/register': async (body, req) => {
    ...
    // 계정을 **만들 때만** 예약어를 본다 (0.69). 접속(login)은 checkId 그대로다.
    ...
    const record = {
      ...
      // 타임어택 시계도 여기서 0 초로 건다 (0.70.2).
      // 만들자마자 그대로 들어가므로(login 을 안 거친다) 여기가 그 자리다.
      ...playclock.resetClock(),
    };
  ...
  '/api/login': async (body, req) => {
    ...
    const upgraded = check.upgrade
      ...
      : acct;
    // 타임어택 시계의 **새 토막이 여기서 시작한다** (0.70.2).
    //
    // 끊겨 있던 동안은 세지 않는다 — 그게 이 시계의 전부다(playclock.js 참고).
    // 옛 계정이면 여태 쓰던 벽시계 값을 물려받아 시계가 뒤로 가지 않게 한다.
    await store.set(id, {
      ...upgraded,
      ...playclock.startSession(upgraded),
      lastLoginAt: new Date().toISOString(),
    });
    ...
    return {
      ...
      // ⚠ **캐릭터 이름을 함께 준다** (0.68).
      //
      //   여태 안 줘도 티가 안 났다. 이름은 세이브 안에도 들어 있어서,
      //   접속하면 applySave 가 덮어써 주었기 때문이다.
      //   그런데 세이브가 **없는** 사람이 있다 — '다시 도전' 으로 방금 되돌린
      //   사람, 그리고 계정만 만들고 한 번도 저장 못 한 사람.
      //   그때는 이름이 아이디로 내려앉았다. 0.66 부터 화면에 나오는 것이
      //   캐릭터 이름이므로, 다시 도전하면 남들에게 아이디로 보였다.
      name: acct.name || id,
      save: acct.save || null,
  ...
  '/api/save': async (body, req) => {
    ...
    // 저장 요청은 곧 "저 아직 여기 있습니다" 다 (0.70.2).
    //
    // 게임은 20초마다 저장한다. 그러니 이 자리에서 시계를 한 칸씩 밀어 두면,
    // 탭을 닫는 순간(인사도 없이 사라진다) 시계도 거기서 멈춘다.
    // ⚠ 시간을 **게임이 보내는 숫자로 세지 않는다** — 서버 시계로만 잰다.
    // 0.70.17 — 건너뛴 전투만큼 시계를 앞으로 민다.
    //
    // 전투는 게임 안에서 벌어지므로 서버 시계로는 잴 수 없다. 스킵을 누르면
    // 20초짜리 싸움이 0초에 끝나고, 타임어택에서 **연출을 건너뛴 것이 실력이 된다.**
    // 그래서 게임이 건너뛴 몫을 보내고 여기서 더한다.
    // 속일 수 있지만 속여서 얻을 것이 없다 — 자세한 까닭은 playclock.addSkipped 참고.
    const owed = playclock.addSkipped(acct, body.skippedMs);

    await store.set(id, {
      ...acct,
      ...born,
      ...(owed || playclock.tick(acct)),
  ...
  '/api/rank': async (body, req) => {
    ...
    // 지금 도전에 쓴 시간을 함께 준다 (0.70.2).
    //
    // 시계가 **접속해 있는 동안만** 흐르게 바뀌었는데, 그 값이 어디에도 안 보이면
    // 사람은 여전히 벽시계로 짐작한다. 랭킹 창에 한 줄로 보여 준다.
    // ⚠ 여기서 시계를 밀지는 않는다 — 표를 보는 것은 "아직 있다" 는 소식이 아니다
    //   (로그인 안 한 손님도 부를 수 있는 문이다).
    let played = null;
    if (me) {
      const acct = await store.get(me);
      if (acct) played = playclock.playedMs(acct);
    }
    return {
  ...
  '/api/rank/submit': async (body, req) => {
    ...
    // ── 운영자는 표에 오르지 않는다 (0.70.18) ────────────────
    //
    // 운영자는 보러 온 사람이다. 리젠 단추로 보스를 몇 번이고 불러낼 수 있고
    // 세 직업 스킬을 다 쓰며 투명하게 다닌다 — 그 몸으로 세운 기록은
    // 다른 사람이 견줄 수 있는 값이 아니다. 한 줄 오르는 순간 표 전체가 뜻을 잃는다.
    //
    // ⚠ **서버에서 막는다.** 게임 쪽에서도 안 보내지만(main.js 의 submitTimeAttack),
    //   그건 예의지 자물쇠가 아니다. 보내는 쪽은 누구나 고칠 수 있다.
    // ⚠ 오류로 던지지 않는다 — 보스를 눕힌 것은 잘한 일인데 붉은 글씨가 뜨면
    //   무엇이 잘못된 줄 안다. 조용히 안 올리고, 왜 안 올렸는지만 적어 돌려준다.
    if (ADMIN_KEY && id === ADMIN_ID) {
      return { ok: false, reason: '운영자는 랭킹에 오르지 않습니다.', admin: true };
    }

    // 걸린 시간은 **접속해 있던 동안만** 센다 (0.70.2).
    //
    //   그 전에는 `지금 − bornAt` 이었다. 그건 벽시계라서 자는 동안에도 흘렀다 —
    //   저녁에 시작해 다음 날 이어서 하면 실제로 두 시간을 놀고도 기록은 14시간이
    //   됐다. 하루에 몰아서 할 수 있는 사람만 표에 오를 수 있다는 뜻이다.
    //   이제는 서버가 접속해 있는 동안만 쌓아 둔다(playclock.js).
    //
    // ⚠ 여기서 **한 번 더 밀어 준다.** 마지막 저장(20초 주기) 이후로 흐른 몇 초가
    //   기록에 안 들어가면, 눕히기 직전 20초는 공짜가 된다.
    const ticked = { ...acct, ...playclock.tick(acct) };
    const played = playclock.playedMs(acct);
    if (played > 0) await store.set(id, ticked);
    const firstKill = acct.firstKill && typeof acct.firstKill === 'object' ? acct.firstKill : {};
    const res = await world.submitRank(bossId, {
      id,
      // 0.66 — 랭킹에 남는 이름은 **캐릭터 이름**이다.
      //
      //   0.61 에는 아이디로 했다. 그때는 계정과 아이디가 따로였고 계정이 화면에
      //   나오는 것이 문제라 아이디로 통일했다. 이제는 캐릭터 이름이 있으니
      //   그쪽이 맞다 — 아이디는 계정을 여는 열쇠일 뿐 남에게 보일 이유가 없다.
      //
      //   ⚠ **줄에 남는 것은 그때의 이름**이다. 나중에 이름을 바꿔도 이미 오른
      //   기록의 이름은 안 따라간다. 순위표는 '그때 그 사람'의 기록이므로 그게 맞다.
      //   (누구의 기록인지는 옆에 함께 남는 id 로 언제나 가려낼 수 있다)
      name: acct.name || id,
      // 직업과 레벨은 **서버가 가진 세이브**에서 읽는다 (0.69).
      //
      // 손님이 보내면 그 값은 곧 누구나 고칠 수 있는 값이 된다. 레벨은
      // 순위를 가르는 데 쓰이므로(같은 초면 낮은 쪽이 위), 손님이 "레벨 1" 이라고
      // 보내면 그만인 값이면 그 규칙 자체가 뜻을 잃는다.
      // ⚠ 세이브는 **납작하다** — `save.classId` · `save.level` 이다.
      //   `save.player.level` 이 아니다(AccountSystem.serializeSave 참고).
      //   0.69 에 이걸 틀리게 적었다가 표에 레벨이 전부 1 로 들어갔다.
      //   운영자 창의 계정 목록도 **0.47 부터 줄곧** 같은 자리를 틀리게 읽어
      //   레벨이 늘 비어 있었다(아무도 눈치채지 못했다).
      cls: (acct.save && acct.save.classId) || '',
      level: (acct.save && acct.save.level) || 1,
      playedMs: played,
    ...
    if (res.first) {
      // 적어 두는 것은 **표에 남은 값**(가장 빠른 기록)이다 (0.68).
      // 이번 도전이 더 느렸다면 그 느린 값을 적으면 안 된다 — 다음에 접속했을 때
      // 화면의 '내 기록'이 표와 어긋난다.
      // ⚠ 밀어 둔 시계(ticked)를 바탕에 깐다. `acct` 로 덮으면 방금 민 시계가 되감긴다.
      await store.set(id, { ...ticked, firstKill: { ...firstKill, [bossId]: res.best || res.ms } });
    }

    // 1~3위에 오르면 모두에게 알린다 (0.61).
    //
    // ⚠ 알릴지 말지는 **서버가 정한다.** 글도 서버가 짓는다.
    //   손님이 보내는 것은 '어디였나' 뿐이고, 그마저 길이를 자르고 줄바꿈을 지운다 —
    //   아니면 아무나 아무 글이나 모두의 화면에 띄울 수 있게 된다.
    // ⚠ `beat` 를 함께 본다 (0.68). '다시 도전' 이 생긴 뒤로는 기록을 못 갈아 치운
    //   도전도 여기까지 온다. 그때 알리면 아무 일도 안 일어났는데 모두의 화면에
    //   "1위에 랭킹등록!" 이 뜬다.
    if (res.first && res.beat !== false && res.rank && res.rank <= 3) {
      const where = String(body.where || '').replace(/[\r\n]+/g, ' ').trim().slice(0, 24);
      broadcast({
        t: 'notice',
        kind: 'rank',
        // '○○님이' 로 부른다 (0.70.2). 공지는 남의 이름을 부르는 자리라
        // 높여 부르는 것이 맞고, 받침 있는 이름에서 '가/이' 를 가릴 일도 없어진다.
        text: `${acct.name || id}님이 ${where || '어딘가'}에서 ${res.rank}위에 랭킹등록!`,
      });
    }
    return res;
  },
  ...
  // 랭킹이 타임어택이라, 한 번 느리게 끝내면 그 계정으로는 더 나은 기록을 낼
  // 방법이 없었다. 계정을 새로 파면 아이디가 늘어나고 지운 자리는 못 되돌린다.
  //
  // ⚠ 되돌릴 수 없는 일이다. 그래서 **손님 말만 믿지 않는다** —
  //   토큰으로 본인임을 확인하고, 한 시간에 여섯 번으로 묶는다.
  //   (실수로 두 번 누르는 것과, 남의 열쇠를 주워 장난치는 것 둘 다 막는다)
  '/api/account/restart': async (body, req) => {
    limit(req, 'restart', 6, 3600000);
    const { id } = await authed(body);
    const res = await world.restartAccount(id);
    if (!res.ok) throw new Error(res.reason || '다시 시작하지 못했습니다.');
    console.log(`✓ 다시 도전 — ${id} (${res.restarts}번째)`);
    return res;
  },
  ...
  '/api/dragon/hit': async (body, req) => {
    ...
    const { id, acct } = await authed(body);
    // 고룡 기여도 표와 우편에 적히는 이름도 **캐릭터 이름**으로 (0.66).
    //   기여도는 id 로 세고(그게 열쇠다), 보여 줄 때만 이름을 쓴다.
    const who = acct.name || id;
    const res = await world.hitDragon(DRAGON, id, who, body.damage);
    // 눕은 그 순간 모두에게 알린다. 마지막 한 대를 넣은 사람의 **캐릭터 이름**으로.
    if (res && res.downed) {
      broadcast({ t: 'notice', kind: 'dragon', text: `${who}님이 고룡을 쓰러뜨렸다` });
    }
    return res;
  },
  ...
  '/api/admin/accounts': async (body, req) => {
    ...
    for (const id of ids.slice(0, 500)) {
      ...
      out.push({
        ...
        level: (acct.save && acct.save.level) || null,
  ...
  /**
   * 시즌 일정 — **끝나는 시각**과 **다음 시즌까지 며칠 쉬나** (0.70.5).
   *
   * 끝을 정해 두면 서버가 그 시각에 스스로 표를 얼리고, 쉬는 날수가 지나면
   * 스스로 다음 시즌을 연다. 운영자가 그 시각에 깨어 있을 필요가 없다.
   * 끝을 0 으로 주면 예전처럼 "끝을 안 정한" 상태가 된다.
   */
  '/api/admin/season-schedule': async (body, req) => {
    limit(req, 'adminsched', 30, 3600000);
    await adminOnly(body);
    const res = await world.setSeasonSchedule(body.endsAt, body.gapDays);
    console.log(`✓ 운영자 시즌 일정 — ${res.endsAt ? new Date(res.endsAt).toISOString() : '끝 없음'}`
      + ` · 다음 시즌 ${res.gapDays ? `${res.gapDays}일 뒤` : '즉시'}`);
    // 접속 중인 사람의 랭킹 창이 곧바로 새 남은 시간을 보게 한다.
    broadcast({ t: 'season-schedule', season: res });
    return res;
  },
  ...
  '/api/admin/sheet-info': async (body, req) => {
    ...
    await adminOnly(body);
    const cfg = sheetsync.config();
    const look = sheetsync.getLastLook();
    return {
      ok: true,
      id: cfg.id || '',
      from: cfg.from,          // 'saved' | 'env' | 'file' | null
      pollMin: cfg.pollMs ? cfg.pollMs / 60000 : 0,
      // 마지막으로 시트를 봤을 때 무엇이 어긋나 있었나 (0.64.3).
      // 눌러야만 보이던 것을 창을 여는 순간 보이게 한다.
      lookedAt: look.at || 0,
      behind: look.behind || [],
      lookErrors: look.errors || [],
      gameVersion: content.gameVersion ? content.gameVersion() : '',
    };
  },
  ...
  /**
   * 시트의 '계정' 탭을 지금 본다 (0.64) — 예약을 걸거나 풀거나 실행한다.
   * 자동 확인(시트 폴링)과 같은 일을 사람이 눌러서 한 번 더 하는 것뿐이다.
   */
  '/api/admin/account-sync': async (body, req) => {
    limit(req, 'adminacctsync', 20, 60000);
    await adminOnly(body);
    const r = await runAccountSync('운영자 창');
    if (!r.ok && r.why) throw new Error(r.why);
    return r;
  },

  /** 지금 걸려 있는 삭제 예약과, 그걸 푸는 길 (0.64). */
  '/api/admin/account-pending': async (body, req) => {
    limit(req, 'adminacctpend', 60, 60000);
    await adminOnly(body);
    const cancelIds = Array.isArray(body.cancel) ? body.cancel : [];
    const cancelled = cancelIds.length ? await accountsync.cancel(store, cancelIds) : [];
    if (cancelled.length) console.log(`✓ 계정 삭제 예약 취소 — ${cancelled.join(', ')}`);
    const restoreIds = Array.isArray(body.restore) ? body.restore : [];
    const restored = restoreIds.length ? await world.restoreAccount(restoreIds) : null;
    if (restored && restored.restored.length) {
      console.log(`✓ 계정 되살리기 — ${restored.restored.join(', ')} (랭킹 기록은 안 돌아옵니다)`);
    }
    const cfg = accountsync.config();
    return {
      ok: true,
      cancelled,
      restored,
      on: accountsync.enabled(),
      graceMs: cfg.graceMs,
      graceClamped: cfg.graceClamped,
      graceRaw: cfg.graceRaw,
      trashDays: world.TRASH_DAYS,
      pending: await accountsync.pendingList(store),
      trash: await world.trashList(),
    };
  },
...
/**
 * 시트의 '계정' 탭을 한 번 본다 (0.64).
 *
 * 표(items/monsters/…)를 읽는 것과 **같은 통합문서**를 쓰지만, 하는 일이 전혀 다르므로
 * 길을 따로 둔다 — 표 쪽이 검사에 걸려 멈춰도 계정 쪽은 돌아야 하고, 그 반대도 같다.
 *
 * 무슨 일이 있었는지는 반드시 로그에 남긴다. 계정이 사라지는 일에
 * "언제 왜 사라졌나" 를 나중에 물을 수 없으면 안 된다.
 */
let acctSyncBusy = false;
async function runAccountSync(why = '') {
  if (acctSyncBusy) return { ok: false, skipped: 'busy', why: '이미 보고 있습니다.' };
  acctSyncBusy = true;
  try {
    const r = await accountsync.runOnce({
      store,
      world,
      adminId: ADMIN_ID,
      fetchTab: async () => {
        const cfg = sheetsync.config();
        if (!cfg.enabled) throw new Error(sheetsync.NO_SHEET_MSG);
        const book = await sheetsync.fetchWorkbook(cfg.id);
        return book[accountsync.TAB];
      },
    });
    const tag = `계정 탭${why ? ` (${why})` : ''}`;
    if (!r.ok) {
      // 'off' 는 안 켠 서버의 정상 상태다 — 그것까지 적으면 로그가 그 줄로만 찬다.
      if (r.skipped !== 'off' && r.skipped !== 'no-key') console.log(`(${tag}: ${r.why})`);
      return r;
    }
    if (r.scheduled.length) {
      console.log(`✓ ${tag} — 삭제 예약 ${r.scheduled.length}개: ${r.scheduled.map((x) => x.id).join(', ')}`);
      console.log(`  ${new Date(r.scheduled[0].dueAt).toISOString()} 이후에 지웁니다. 그 전에 되돌릴 수 있습니다.`);
    }
    if (r.cancelled.length) console.log(`✓ ${tag} — 예약 취소 ${r.cancelled.length}개: ${r.cancelled.join(', ')}`);
    if (r.deleted.length) {
      console.log(`✓ ${tag} — 삭제 완료 ${r.deleted.length}개: ${r.deleted.map((x) => x.id).join(', ')}`);
      console.log(`  휴지통에 ${world.TRASH_DAYS}일간 남아 있습니다 — 운영자 창 → 계정 에서 되살릴 수 있습니다.`);
    }
    if (r.failed.length) console.log(`⚠ ${tag} — 실패 ${r.failed.length}개: ${r.failed.map((x) => x.id).join(', ')}`);
    // 기한을 못 읽어 새로 잡아 준 것 — 조용히 넘기면 안 된다.
    if ((r.repaired || []).length) {
      console.log(`⚠ ${tag} — 기한을 못 읽어 다시 잡았습니다(안 지웠습니다): ${r.repaired.join(', ')}`);
    }
    return r;
  } finally {
    acctSyncBusy = false;
...
/**
 * 시즌을 넘긴다 — 표를 지난 시즌으로 옮기고 모두를 갓 만든 상태로 되돌린다.
 *
 * 운영자 단추(`/api/admin/season-reset`)와 **시즌 시계**(아래)가 같은 것을 부른다.
 * 두 곳에 같은 절차를 따로 적어 두면 한쪽만 고쳐지는 날이 반드시 온다.
 */
async function rolloverSeason(auto = false) {
  ...
  const text = res.locked
    ...
    : '다음 시즌이 시작되었습니다';
  console.log(`✓ ${auto ? '시즌 시계' : '운영자'} 시즌 초기화 — 계정 ${res.accounts}개`
    + ` · 랭킹 표 ${ranksRes.cleared.length}개`
    + (res.locked ? ` (시즌 고정 — ${res.season} 시즌 그대로)` : ` → ${res.season} 시즌`));
...
// ── 시즌 시계 (0.70.5) ───────────────────────────────────────
//
// 정해 둔 시각에 스스로 시즌을 닫고, 쉬는 날수가 지나면 스스로 연다.
//
// ⚠ **손님 요청에 기대면 안 된다.** 아무도 접속해 있지 않은 새벽에 마감이
//   지나갈 수 있고, 그러면 다음 사람이 들어올 때까지 시즌이 안 끝난다.
//   그 사이에 들어온 기록은 마감 뒤의 기록이다.
// ⚠ 자주 볼 이유는 없다. 분 단위로만 맞으면 되므로 30초에 한 번 본다.
const SEASON_TICK_MS = 30000;
const seasonTimer = setInterval(async () => {
  try {
    const res = await world.tickSeason();
    if (res.action === 'end') {
      const m = res.season;
      const text = m.nextStartsAt > Date.now()
        ? `${m.season} 시즌이 끝났습니다 — 다음 시즌은 ${
          Math.max(1, Math.round((m.nextStartsAt - m.endedAt) / 86400000))}일 뒤에 열립니다`
        : `${m.season} 시즌이 끝났습니다`;
      console.log(`✓ 시즌 시계 — ${m.season} 시즌 마감`
        + (m.nextStartsAt ? ` · 다음 시즌 ${new Date(m.nextStartsAt).toISOString()}` : ''));
      broadcast({ t: 'notice', kind: 'season', text });
      broadcast({ t: 'season-schedule', season: m });
    } else if (res.action === 'start') {
      await rolloverSeason(true);
    }
  } catch (err) {
    console.error('[시즌 시계]', err.message);
  }
}, SEASON_TICK_MS);
if (seasonTimer.unref) seasonTimer.unref();
...
function startupWarnings() {
  ...
  // 0.64 — 시트에서 지운 줄이 진짜 계정을 지우는 스위치. 켠 줄 모르고 두면 안 된다.
  if (accountsync.enabled()) {
    out.push([
      'SHEET_ACCOUNT_DELETE 가 켜져 있습니다 — 구글 시트 계정 탭에서 줄을 지우면',
      `그 계정이 ${accountsync.config().graceMs / 3600000}시간 뒤에 지워집니다(세이브·우편·랭킹까지).`,
      `지우기 전에 당사자에게 우편이 가고, 지운 뒤 ${world.TRASH_DAYS}일간은 되살릴 수 있습니다.`,
      '시트를 고칠 수 있는 사람이 곧 계정을 지울 수 있는 사람입니다 — 편집 권한을 확인하세요.',
    ]);
  }
  return out;
}
...
async function start() {
  ...
  // 아이디 예약어 — 없으면 **계정 만들기를 막는다**(위 checkNewId 참고).
  try {
    await loadReserved();
  } catch (err) {
    reservedWhy = err.message;
    console.error('\n✗ 아이디 규칙(src/systems/ReservedId.js)을 못 읽었습니다:', err.message);
    console.error('  이 상태에서는 **새 계정 만들기가 막힙니다.** 접속과 게임은 그대로 됩니다.\n');
  }
  ...
  server.listen(PORT, () => {
    ...
    if (sheetCfg.enabled) {
      ...
      // 표를 볼 때 '계정' 탭도 같이 본다 (0.64). 켜 두지 않았으면 곧바로 돌아온다.
      if (accountsync.enabled()) {
        const everyMs = Math.max(60000, sheetCfg.pollMs || 300000);
        const acfg = accountsync.config();
        console.log(`  계정 탭     : 시트에서 지운 줄을 ${acfg.graceMs / 3600000}시간 뒤에 삭제합니다`);
        console.log('              (그 전에 줄을 되돌리거나 운영자 창에서 취소할 수 있습니다)');
        console.log(`              지우기 전에 당사자에게 우편으로 알리고, 지운 뒤 ${world.TRASH_DAYS}일간 되살릴 수 있습니다.`);
        if (acfg.graceClamped) {
          console.log(`  ⚠ ${acfg.graceRaw} 는 너무 짧아 ${accountsync.MIN_GRACE_MS / 3600000}시간으로 올렸습니다.`);
          console.log('     시간 단위로 적으려면 SHEET_ACCOUNT_GRACE_HOURS 를 쓰세요 (예: 24).');
        }
        const t = setInterval(() => { runAccountSync('자동').catch(() => {}); }, everyMs);
        if (t.unref) t.unref();
        setTimeout(() => { runAccountSync('켜자마자').catch(() => {}); }, 20000);
      } else if (accountsync.config().on) {
        console.log('  계정 탭     : SHEET_ACCOUNT_DELETE 는 켜졌지만 ADMIN_KEY 가 없어 돌지 않습니다');
      }
    } else {
