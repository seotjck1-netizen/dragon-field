 ...
 */
/**
 * 설명에 적힌 퍼센트가 실제 값과 맞는가 (0.59).
 *
 * 왜 필요한가: 값은 **시트에서 사람이** 고치고 설명은 그대로 두는 일이 반복됐다.
 * 0.59 에 들어올 때 '처형자' 가 그랬다 — 치명타 피해를 0.5 에서 0.3 으로 내렸는데
 * 화면에는 여전히 "+50%" 라고 적혀 있었다. 값이 틀린 것보다 **설명이 거짓말하는 쪽**이
 * 나쁘다. 값을 보고 스킬을 고르는 것이 아니라 설명을 보고 고르기 때문이다.
 *
 * 잣대는 느슨하게 잡는다 — 막는 것이 아니라 알리는 것이 목적이다.
 *   · 설명에 있는 "+NN%" 를 전부 모은다.
 *   · 효과 값 × 100 중 하나와도 안 맞는 수가 있으면 알린다.
 *   · 효과에 없는 수(기본값 언급 등)는 흔하므로 **오류가 아니라 경고**다.
 */
function descMismatch(at, def) {
  const out = [];
  const text = String(def.desc || '');
  if (!text) return out;
  const nums = [...text.matchAll(/\+\s*(\d+(?:\.\d+)?)\s*%/g)].map((m) => Number(m[1]));
  if (!nums.length) return out;
  // 스킬은 effect, 특성은 mods 에 값이 있다. 둘 다 본다.
  //
  // ⚠ 0.70.6 까지 effect 만 봐서 **특성은 검사 밖에 있었다.** 시트에서
  //   '공격력 강화' 설명을 +15% 로 고치고 값은 0.1(=10%) 로 둔 것이 그대로
  //   들어왔다 — 화면에는 15%, 싸움에는 10%. 사람이 눈으로 찾을 수 없는 어긋남이다.
  const vals = Object.values({ ...(def.effect || {}), ...(def.mods || {}), ...(def.per || {}) })
    .filter((v) => typeof v === 'number')
    .map((v) => Math.round(v * 1000) / 10);
  if (!vals.length) return out;
  const orphan = nums.filter((n) => !vals.some((v) => Math.abs(v - n) < 0.05));
  // 값과 짝이 안 맞는 수가 **하나라도** 있으면 알린다.
  //
  // 처음에는 "하나도 안 맞을 때만" 으로 느슨하게 뒀는데, 그러면 정작 잡으려던
  // '처형자'(설명 +50%/+20%, 값 +30%/+20%)가 안 걸렸다 — 관통 20% 가 맞아서다.
  // 지금 표 전체(스킬 24 · 특성 6)에 헛경고는 없다.
  if (orphan.length) {
    out.push(`${at} 의 설명에 적힌 +${orphan.join('%, +')}% 가 실제 값(+${vals.join('%, +')}%)에 없습니다.`);
  }
  return out;
}

/**
 * per(스탯 1점당 오르는 값)와 설명에 적힌 숫자가 어긋나는가.
 *
 * descMismatch 는 '+N%' 만 본다. 스탯 표의 '체력 +5' 처럼 **%가 없는 숫자**는
 * 그 그물에 안 걸려서, 시트에서 값을 4 로 내리고 설명은 +5 로 둔 것이
 * 그대로 들어왔다(0.70.6). 화면에는 +5, 실제로는 +4 다.
 */
const PER_LABEL = { hp: '체력', atk: '공격력', def: '방어력', spd: '속도' };
function perMismatch(at, def) {
  const out = [];
  const text = String(def.desc || '');
  if (!text) return out;
  for (const [key, label] of Object.entries(PER_LABEL)) {
    const v = Number((def.per || {})[key]);
    const m = text.match(new RegExp(`${label}\\s*\\+\\s*(\\d+(?:\\.\\d+)?)(?!\\s*%)`));
    if (!m || !Number.isFinite(v)) continue;
    if (Math.abs(Number(m[1]) - v) > 0.001) {
      out.push(`${at} 의 설명은 ${label} +${m[1]} 인데 실제 값은 +${v} 입니다.`);
    }
  }
  return out;
}
...
function validate(files) {
  ...
  if (quests && Array.isArray(quests['표'])) {
    quests['표'].forEach((row, i) => {
      ...
      // 사냥 의뢰의 대상은 `a+b` 로 여럿일 수 있다(0.64) — 하나씩 다 있어야 한다.
      if (type === 'hunt') {
        const list = String(target || '').split('+').map((s) => s.trim()).filter(Boolean);
        if (!list.length) errors.push(`${at}: 잡을 몬스터가 비어 있습니다.`);
        for (const t of list) if (!monsters[t]) errors.push(`${at}: 없는 몬스터 '${t}'`);
      }
  ...
  if (classes && classes.list) {
    ...
    for (const [cid, def] of Object.entries(classes.list)) {
      ...
      // 글자로 적는 combat 값들. 나머지는 전부 숫자여야 한다.
      //   school       — 'magic' 이면 마법 공격
      //   openerWeapon — 'bow' 처럼 무기 갈래. 그 무기를 들어야 선제 공격이 나간다
      const TEXT_COMBAT = new Set(['school', 'openerWeapon']);
      const WEAPON_KINDS = new Set(['club', 'sword', 'greatsword', 'bow', 'staff']);
      for (const [k, v] of Object.entries(def.combat || {})) {
        if (k === 'openerWeapon' && v && !WEAPON_KINDS.has(String(v))) {
          errors.push(
            `${at} 의 combat.openerWeapon '${v}' 을 게임이 모릅니다.`
            + ` (${[...WEAPON_KINDS].join(' · ')} 중 하나)`
          );
          continue;
        }
        if (TEXT_COMBAT.has(k)) continue;
        if (typeof v !== 'number') {
  ...
  if (skills && skills.tree) {
    ...
    for (const [sid, def] of Object.entries(skills.tree)) {
      ...
      // 0.64 — steps 로 자라는 스킬(걸음이 고르지 않은 것)도 엄연한 효과다.
      const stepKeys = Object.keys(def.steps || {});
      if (!keys.length && !stepKeys.length) {
        errors.push(`${at} 에 효과가 없습니다(찍어도 아무 일이 없습니다).`);
      }
      for (const k of keys) {
      ...
      for (const k of stepKeys) {
        const arr = def.steps[k];
        if (!MOD_KEYS.includes(k)) { errors.push(`${at} 의 steps '${k}' 을 게임이 모릅니다.`); continue; }
        if (!Array.isArray(arr) || !arr.length) {
          errors.push(`${at} 의 steps '${k}' 이 값 배열이 아닙니다.`);
          continue;
        }
        // 칸 수는 max 와 같아야 한다. 모자라면 마지막 칸에서 멈춰 조용히 안 오른다 —
        // "3점을 찍었는데 2점과 같다" 가 되고, 그건 표만 봐서는 안 보인다.
        if (arr.length !== Number(def.max)) {
          errors.push(`${at} 의 steps '${k}' 은 ${def.max}칸이어야 합니다(지금 ${arr.length}칸).`);
        }
        for (const v of arr) {
          if (typeof v !== 'number') errors.push(`${at} 의 steps '${k}' 에 숫자가 아닌 값이 있습니다.`);
        }
      }
      if (learnable.size && !learnable.has(sid)) {
        warnings.push(`skills.json: '${sid}' 을 배울 수 있는 직업이 없습니다.`);
      }
      for (const w of descMismatch(at, def)) warnings.push(w);
    }
  }
  ...
  if (traits && traits.nodes) {
    for (const [tid, def] of Object.entries(traits.nodes)) {
      ...
      // 설명에 적힌 %와 실제 값이 어긋나면 알린다(스킬과 같은 검사).
      for (const w of descMismatch(at, def)) warnings.push(w);
    }
  ...
  // 스탯(힘·민첩·지능) — 포인트가 없으므로 max 는 없다.
  {
    ...
    if (st && st.nodes) {
      for (const [sid, def] of Object.entries(st.nodes)) {
        ...
        for (const w of descMismatch(at, def)) warnings.push(w);
        for (const w of perMismatch(at, def)) warnings.push(w);
      }
