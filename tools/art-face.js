...
/**
 * 눈썹 한 쌍.
 * @param {number} span 길이 배율. 1 = 기본, 0.8 = 20% 짧게.
 *   ⚠ 눈을 줄이면 눈썹도 같이 줄여야 한다 — 눈만 줄이면 눈썹이 얼굴을
 *     가로지르는 굵은 막대가 되어 화난 얼굴이 된다(0.70.8 에 그렇게 나왔다).
 */
const brow = (c = '#3a2010', w = 2.7, y = BROW_Y, drop = 1.6, span = 1) => {
  const mid = 39.2;           // 왼쪽 눈썹의 가운데
  const half = 5.4 * span;    // 기본 길이 10.8 의 절반
  const L1 = mid - half, L2 = mid + half;
  const R1 = 96 - L1, R2 = 96 - L2;
  return `
  <path d="M${L1.toFixed(2)} ${y} L${L2.toFixed(2)} ${y + drop}" stroke="${c}" stroke-width="${w}" fill="none" stroke-linecap="round"/>
  <path d="M${R1.toFixed(2)} ${y} L${R2.toFixed(2)} ${y + drop}" stroke="${c}" stroke-width="${w}" fill="none" stroke-linecap="round"/>`;
};
