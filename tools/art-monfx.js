...
// ── 걷는 두 장 (0.70.17) ─────────────────────────────────────
//
// 몬스터 그림은 사람 그림과 달리 **부품으로 나뉘어 있지 않다** — 슬라임에게는
// 팔다리가 아예 없고, 늑대는 통짜 path 하나다. 그래서 팔다리를 따로 못 돌린다.
//
// 대신 **몸이 어떻게 움직이는가**로 걸음을 만든다. 걸을 때 몸은 두 가지를 한다:
//   ① 디딜 때 눌린다(무게가 실린다)   ② 넘어갈 때 펴지며 좌우로 기운다
// 이 둘을 엇갈린 두 장으로 구우면, 서 있는 그림과 번갈아 나올 때 **걷는 리듬**이 된다.
//
// ⚠ 앞뒤(dx)로는 옮기지 않는다. 옮기면 칸 사이를 미끄러지는 동안 그림까지 따로
//   움직여서 두 겹으로 흔들린다. 걸음의 앞뒤 이동은 이미 칸 보간이 맡고 있다.
//
// ⚠ 발밑(py = V*0.97)이 축이다 — 그래야 땅에 붙은 채로 흔들린다.
//   attackPose 와 같은 축을 쓴다.
function walkPose(svg, { lean = 3, sx = 1.03, sy = 0.97 } = {}) {
  return attackPose(svg, { dx: 0, lean, sx, sy, mark: '' });
}

/**
 * 몬스터마다 걸음이 얼마나 크나. **몸이 클수록 적게 움직인다** — MON_ATTACK 과 같은 잣대다.
 * 두 장은 이 값의 부호를 뒤집어 만든다(왼쪽으로 기울었다, 오른쪽으로 기울었다).
 */
const MON_WALK = {
  // 물컹한 것은 기울지 않는다. 통통 튄다.
  slime:         { lean: 0, sx: 1.08, sy: 0.93 },
  // 나는 것은 눌리지 않는다. 날갯짓만큼 좌우로 기운다.
  bat:           { lean: 7, sx: 1, sy: 1 },
  mushroom:      { lean: 2, sx: 1.04, sy: 0.97 },
  wolf:          { lean: 4, sx: 1.05, sy: 0.97 },
  imp:           { lean: 5, sx: 1.03, sy: 0.97 },
  skeleton:      { lean: 4, sx: 1.02, sy: 0.98 },
  demon_soldier: { lean: 3.5, sx: 1.03, sy: 0.97 },
  imp_captain:   { lean: 3, sx: 1.02, sy: 0.98 },
  demon_general: { lean: 2.5, sx: 1.02, sy: 0.98 },
  // 고룡은 걸어도 거의 흔들리지 않는다. 큰 것이 촐랑대면 무게가 사라진다.
  great_dragon:  { lean: 1.2, sx: 1.015, sy: 0.99 },
  elder_dragon:  { lean: 1.2, sx: 1.015, sy: 0.99 },
};

module.exports = { attackPose, walkPose, MON_ATTACK, MON_WALK };
