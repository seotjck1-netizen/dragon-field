...
function heroSvg({
  ...
  // ── 걷는 자세 (0.70.17) ────────────────────────────────────
  //
  // 걷는 동안 흔들기만 했다(위아래 2픽셀 사인파). 그림 한 장을 통째로 들었다
  // 놓는 것이라, 걸을수록 **미끄러지는 것처럼** 보였다 — 발이 땅을 짚지 않는다.
  //
  // 그래서 **팔과 다리를 실제로 엇갈린 두 장**을 굽는다. 위에서 비스듬히 보는
  // 화면이라 앞뒤가 아니라 **좌우로 벌어졌다 모이는 것**이 걸음으로 읽힌다.
  // 한쪽 다리가 앞으로 나가면 같은 쪽 팔은 뒤로 간다(사람이 실제로 그렇게 걷는다).
  //
  // ⚠ 발끝은 두 장 모두 **같은 높이**에 둔다. 그림은 발밑을 기준으로 세우므로
  //   (anchor: 'bottom') 한 장만 발이 짧으면 걸을 때마다 몸이 껑충거린다.
  // ── 뒷모습 (0.70.18) ───────────────────────────────────────
  //
  // 위로 걸어가는데 얼굴이 이쪽을 보고 있으면, 걷는 쪽과 보는 쪽이 어긋나
  // **게걸음**처럼 보인다. 그래서 위로 갈 때 쓸 뒷모습을 따로 굽는다.
  //
  // 뒤에서 본다는 것은 세 가지다:
  //   ① 얼굴이 없다 — 눈·코·입 대신 **뒤통수**가 있다(머리색으로 찬 머리 덩이)
  //   ② 앞머리(hairShape)와 목걸이(front)가 안 보인다
  //   ③ 등에 멘 것(behind — 땋은 머리·화살통)이 **앞으로 나온다.** 등을 보고 있으니까.
  //
  // ⚠ 몸통·팔·다리는 **그대로 둔다.** 옷 색을 갑옷 색으로 갈아 끼우는 규칙
  //   (core/Appearance.js 의 regions.tunic)이 그 자리를 보고 있어서, 몸을 다시 그리면
  //   뒷모습만 갑옷 색이 안 먹는다. 사람은 어차피 뒤통수로 앞뒤를 안다.
  const back = pose.startsWith('back');
  const walk = /walk1$/.test(pose) ? 1 : /walk2$/.test(pose) ? -1 : 0;
  // ⚠ 흔드는 몫은 **감싸는 <g> 하나로만** 준다. 팔 path 와 손 circle 양쪽에 따로
  //   더하면 손이 팔에서 떨어져 나간다(두 번 옮겨지기 때문이다).
  const WALK = {
    swing: 10,    // 어깨를 축으로 팔이 도는 각
    lift: 3,      // 그리고 앞뒤로(화면에서는 위아래로) 흔들리는 몫
    legRot: 13,   // 엉덩이를 축으로 다리가 도는 각. 다리는 짧아서 각이 커야 보인다
    legDrop: 2,   // 앞으로 나간 다리가 화면에서 내려오는 몫
  };
  const hdx = (k) => (ready ? READY.handDX[k] : 0);
  const hdy = ready ? READY.handDY : 0;

  /** 팔 하나가 도는 방향. k=0(몸 왼쪽) 과 k=1 이 서로 반대다. */
  const armSign = (k) => walk * (k === 0 ? 1 : -1);

  /**
   * 걷는 동안 팔 하나를 흔든다. 어깨(52)를 축으로 돌리고 손 높이를 함께 옮긴다.
   * 걷지 않으면 **아무것도 감싸지 않는다** — 서 있는 그림이 옛것과 한 픽셀도 달라지지 않게.
   */
  const swingArm = (inner, cx, k) => (walk
    ? `<g transform="rotate(${WALK.swing * armSign(k)} ${cx} 52) `
      + `translate(0 ${WALK.lift * armSign(k)})">${inner}</g>`
    : inner);

  /** 다리 하나가 나가는 방향. 같은 쪽 팔과 **반대**다(사람이 그렇게 걷는다). */
  const legSign = (side) => -armSign(side);

  /**
   * 걷는 동안 다리 하나를 내딛는다. 엉덩이(95)를 축으로 돌리고, 나간 다리는 **조금 내린다.**
   *
   * ⚠ 내리는 몫(WALK.legDrop)이 없으면 두 다리가 벌어졌다 모이기만 해서 **뒤뚱거림**이 된다.
   *   위에서 비스듬히 보는 화면에서 '앞으로 나간 발' 은 화면에서 **아래**에 있다.
   *   그래서 벌리는 것에 내리는 것을 더해야 비로소 걸음으로 읽힌다(처음에 빼먹어서 실제로 뒤뚱거렸다).
   */
  const stepLeg = (inner, hipX, side) => (walk
    ? `<g transform="rotate(${WALK.legRot * legSign(side)} ${hipX} 95) `
      + `translate(0 ${WALK.legDrop * legSign(side)})">${inner}</g>`
    : inner);
  ...
  <!-- 몸 뒤 소품. 뒷모습에서는 등이 이쪽이므로 **맨 앞으로** 옮긴다(아래 참고). -->
  ${back ? '' : behind}

  <!-- 맨다리 + 샌들. 걸을 때는 다리와 그 발을 **한 덩이로** 돌린다 —
       따로 돌리면 발이 정강이에서 떨어져 나간다. -->
  ${ready
  ...
  <ellipse cx="58" cy="112.6" rx="7.6" ry="3.6" fill="#7a5230" transform="rotate(8 58 112.6)"/>`
    : `${stepLeg(`<rect x="37" y="95" width="9" height="18" rx="4.5" fill="url(#arm_${id})"/>
  <path d="M39 99 l0 9" stroke="${skinDark}" stroke-width="1" opacity="0.4"/>
  <ellipse cx="41.5" cy="114" rx="7.6" ry="4.2" fill="#5a3a1e"/>
  <ellipse cx="41.5" cy="112.6" rx="7.2" ry="3.6" fill="#7a5230"/>
  <path d="M36 112 l11 -3" stroke="#9c6b3d" stroke-width="2.2" stroke-linecap="round"/>`, 41.5, 0)}
  ${stepLeg(`<rect x="50" y="95" width="9" height="18" rx="4.5" fill="url(#arm_${id})"/>
  <path d="M52 99 l0 9" stroke="${skinDark}" stroke-width="1" opacity="0.4"/>
  <ellipse cx="54.5" cy="114" rx="7.6" ry="4.2" fill="#5a3a1e"/>
  <ellipse cx="54.5" cy="112.6" rx="7.2" ry="3.6" fill="#7a5230"/>
  <path d="M49 112 l11 -3" stroke="#9c6b3d" stroke-width="2.2" stroke-linecap="round"/>`, 54.5, 1)}`}
  ...
  ${swingArm(arm(ARM_CX[0], -1, 0), ARM_CX[0], 0)}
  ${swingArm(arm(ARM_CX[1], 1, 1), ARM_CX[1], 1)}
  ...
  <!-- 손 (팔과 **같은 감싸개**로 흔든다 — 따로 두면 손만 제자리에 남는다) -->
  ${swingArm(hand(ARM_CX[0], 0), ARM_CX[0], 0)}
  ${swingArm(hand(ARM_CX[1], 1), ARM_CX[1], 1)}

  ${back
    ? `<!-- 뒤통수 — 얼굴 자리를 머리색으로 채우고, 귀만 옆에 남긴다.
            머리 결은 아래로 흐르게 그어 "뒤에서 본 머리" 로 읽히게 한다. -->
  <path d="${F.jawSoft}" fill="url(#hair_${id})"/>
  ${F.ears(skinMid, skinDark)}
  <path d="${F.jawSoft}" fill="url(#hair_${id})" opacity="0.85"/>
  <g stroke="${hair[1]}" stroke-width="1.2" fill="none" opacity="0.45" stroke-linecap="round">
    <path d="M38 20 Q35 34 36 50"/>
    <path d="M44 18 Q42 34 43 53"/>
    <path d="M52 18 Q54 34 53 53"/>
    <path d="M58 20 Q61 34 60 50"/>
  </g>
  <path d="M40 52 Q48 58 56 52" stroke="${hair[1]}" stroke-width="1.6" fill="none" opacity="0.5"/>
  ${/* 목덜미 그늘 — 머리와 등 사이가 한 덩이로 붙어 보이지 않게 */ ''}
  <path d="M42 55 q6 4 12 0" stroke="${skinDark}" stroke-width="1.4" fill="none" opacity="0.5"/>

  <!-- 머리 장식 (띠·두건은 뒤에서도 머리를 두른다) -->
  ${headgear}
  <!-- 등에 멘 것이 이제 이쪽이다 -->
  ${behind}`
    : `<!-- 얼굴 -->
...
const WARRIOR_OPTS = ({
  ...
  ${F.cheekShade()}
  ${/* 0.70.8 — 눈과 눈썹을 **20% 줄였다.** 마법사 얼굴이 기준이고 이 둘이
       그보다 컸다. 눈만 줄이면 눈썹이 얼굴을 가로지르는 막대가 되므로
       굵기·길이를 함께 0.8 배로 한다(art-face.js 의 brow span). */''}
  ${F.brow('#5b3518', 1.92, 28.8, 1.1, 0.8)}
  ${F.eye(F.EYE_L, F.EYE_CY, 4.56, 5.28, '#3f7fd0', -1, 0)}
  ${F.eye(F.EYE_R, F.EYE_CY, 4.56, 5.28, '#3f7fd0', 1, 0)}
  ${F.noseSmall}
  ${F.mouthSmile}
...
const RANGER_OPTS = ({
  ...
  ${F.rangerCheek}
  ${F.warPaintFull}
  ${/* 0.70.8 — 용사와 같은 비율로 20% 줄였다. */''}
  ${F.brow('#171009', 1.84, 28.6, 0.9, 0.8)}
  ${F.eye(39.2, 39.8, 4.8, 5.44, '#7ab04a', -1, 0.4)}
  ${F.eye(56.8, 39.8, 4.8, 5.44, '#7ab04a', 1, 0.4)}
  ${F.rangerNose}
  ${F.rangerMouth}`,
...
/**
 * 걷는 두 장 — 팔과 다리가 실제로 엇갈린다 (0.70.17).
 *
 * 서 있는 그림까지 합쳐 **네 박자**로 돈다: 서기 → 1 → 서기 → 2.
 * 두 장만으로 왔다 갔다 하면 발이 땅을 두 번 짚는 사이가 비어 덜컥거린다.
 * 가운데에 서 있는 그림을 끼우면 그 사이가 메워진다(FieldScene 이 그렇게 고른다).
 */
const CLASS_WALK = {
  hero: [heroSvg({ ...WARRIOR_OPTS, pose: 'walk1' }), heroSvg({ ...WARRIOR_OPTS, pose: 'walk2' })],
  ranger: [heroSvg({ ...RANGER_OPTS, pose: 'walk1' }), heroSvg({ ...RANGER_OPTS, pose: 'walk2' })],
  mage: [heroSvg({ ...MAGE_OPTS, pose: 'walk1' }), heroSvg({ ...MAGE_OPTS, pose: 'walk2' })],
};

/**
 * 뒷모습 세 장 — 서기 · 걷기1 · 걷기2.
 *
 * ⚠ **지금은 굽지 않는다(0.70.19 에 보류).** 그릴 줄은 맞는데, 이것만으로는 모자란다.
 *
 *   0.70.18 에 실제로 넣어 봤더니 머리만 뒤를 보고 **몸에 걸친 것은 그대로 앞**이었다:
 *   손에 든 검이 몸 앞에 떠 있고, 허리띠 버클이 등에 붙어 있고, 투구는 앞면이
 *   뒤통수에 붙었다. 사람이 뒤를 보는 것이 아니라 얼굴만 지워진 꼴이었다.
 *
 *   까닭은 **얹는 구조**다. 장비는 core/Appearance.js 가 이 밑그림 위에 파츠를 얹어
 *   만드는데, 그 파츠는 앞에서 본 모양 한 벌뿐이다. 밑그림만 뒤로 바꿔도 위에
 *   얹히는 것은 앞을 본다.
 *
 *   다시 켜려면 **파츠의 뒷면 한 벌**이 먼저다 —
 *     · 투구  앞면(얼굴 가리개) → 뒤통수
 *     · 허리띠 버클을 뺀 뒤쪽
 *     · 무기  손 앞 → 손 뒤(몸에 가려 일부만 보인다)
 *     · 망토  뒤에서는 통째로 앞으로 나온다
 *   그리고 appearance.json 의 parts 에 그 한 벌을 얹을 자리(`by.back`)를 더해야 한다.
 *   밑그림 세 장으로 끝나는 일이 아니라서 미뤄 두었다.
 */
const OPTS = { hero: WARRIOR_OPTS, ranger: RANGER_OPTS, mage: MAGE_OPTS };
const CLASS_BACK = {};
for (const [name, o] of Object.entries(OPTS)) {
  CLASS_BACK[name] = {
    idle: heroSvg({ ...o, pose: 'back' }),
    walk1: heroSvg({ ...o, pose: 'back_walk1' }),
    walk2: heroSvg({ ...o, pose: 'back_walk2' }),
  };
}
...
module.exports = {
  CLASS_STANCE, CLASS_WALK, CLASS_BACK, CLASS_SPRITES, CLASS_ITEMS, CLASS_MARKS,
