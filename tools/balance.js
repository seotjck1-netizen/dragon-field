...
/**
 * 특성을 **공격 쪽으로 몰았을 때**와 **방어 쪽으로 몰았을 때** (0.70.13).
 *
 * 여태 특성 순서는 직업마다 하나뿐이었다. 그런데 사람은 같은 직업으로도
 * 전혀 다른 몸을 만든다 — "공격에 다 부은 사람" 과 "버티기에 다 부은 사람" 은
 * 고룡 앞에서 완전히 다른 답을 받는다. 한 순서로만 재면 그 절반을 못 본다.
 *   index 0 = 공격형 · index 1 = 방어형
 */
const TRAIT_PLANS = {
  warrior: [
    ['might', 'execution', 'arcana', 'apothecary', 'bulwark', 'fortune'],
    ['bulwark', 'apothecary', 'might', 'execution', 'arcana', 'fortune'],
  ],
  ranger: [
    ['might', 'execution', 'arcana', 'apothecary', 'bulwark', 'fortune'],
    ['bulwark', 'apothecary', 'might', 'execution', 'arcana', 'fortune'],
  ...
  mage: [
    ['arcana', 'might', 'execution', 'apothecary', 'bulwark', 'fortune'],
    ['bulwark', 'apothecary', 'arcana', 'might', 'execution', 'fortune'],
  ],
};
 ...
 */
function buildTraitRanks(G, cls, level, traitIdx = null) {
  ...
  const out = {};
  const order = traitIdx == null
    ? (TRAIT_PLAN[cls] || [])
    : ((TRAIT_PLANS[cls] || [])[traitIdx] || TRAIT_PLAN[cls] || []);
...
/**
 * 홈에 **가장 좋은 보석**을 박았을 때의 값. (0.70.10 — 천장 몸 전용)
 *
 * avgGemMods 는 통 전체의 평균을 쓴다 — "아무거나 박은 사람" 이다.
 * 끝까지 키운 사람은 아무거나 안 박는다. 제 직업에 값진 것 하나로 통일한다
 * (실제로 t00 은 열세 칸 중 아홉 칸을 루비로 채워 두었다).
 * 천장을 재려면 **최선의 선택**을 재야 한다.
 *
 * @param {string} want 무엇을 올리고 싶은가 — 'mods.atkPct' 처럼 보석표의 키
 */
function bestGemMods(G, itemDef, enh, want) {
  const out = {};
  ...
  // ⚠ 송곳(악마의 파편)으로 뚫는 **두 번째 홈**까지 센다.
  //   천장 몸은 "끝까지 모은 사람" 이고, t00 은 실제로 무기·악세서리에 두 칸을
  //   다 뚫어 두었다. 여기서 한 칸만 세면 천장이 실제보다 낮게 나온다.
  const DRILLABLE = ['weapon', 'ring', 'necklace', 'belt'];
  if (sockets && DRILLABLE.includes(itemDef.slot)) sockets = 2;
  if (!sockets || !itemDef.slot) return out;
  const all = (G['affixes.json'] || {})['보석'] || [];
  // want 는 **보석 아이디**('gem_ruby')로도, 보석표의 키('mods.atkPct')로도 줄 수 있다.
  //
  // ⚠ 0.70.13 까지 아이디로 주면 첫 줄(find 의 fallback)이 잡혀서, '전부 루비'와
  //   '전부 다이아' 가 **완전히 같은 몸**으로 측정됐다. 표는 두 줄이 나왔지만
  //   실제로는 한 몸을 두 번 잰 것이었다.
  const pick = all.find(([id, , key]) => id === want || key === want)
    || all.find(([, , key]) => {
      const [kind, prop] = String(key).split('.');
      return kind !== 'mods' || allowedOnSlot(prop, itemDef.slot);
    });
  if (!pick) return out;
  // 그 부위에 못 박는 보석이면(흡혈 오닉스를 투구에) 아무것도 주지 않는다.
  {
    const [kind, prop] = String(pick[2]).split('.');
    if (kind === 'mods' && !allowedOnSlot(prop, itemDef.slot)) return out;
  }
  const [, , key, value] = pick;
  out[key] = Number(value) * sockets;
  return out;
}
...
/** formulas.js 의 BALANCE 숫자 하나를 그대로 읽는다(두 곳에 적지 않기 위해). */
function NUM(key, fallback) {
  const m = new RegExp(`${key}:\\s*([0-9.]+)`).exec(FORMULAS_SRC);
...
// 0.70.13 — 관통도 여기로 온다. 활의 기본 옵션이 치명타에서 관통으로 바뀌었고,
//   관통은 합산 스탯이 아니라 보정이다(StatBlock 이 하는 일과 같다).
const STAT_TO_MOD = { critDmg: 'critMult', pierce: 'pierce' };
// 강화해도 소수점을 유지하는 스탯(formulas.RATIO_STATS 와 같다).
const RATIO_STATS = new Set(['crit', 'critDmg', 'pierce']);
// 강화로 **오르지 않는** 스탯 (formulas.FIXED_STATS 와 같아야 한다).
 ...
 *   주면 '상대' 가 붙은 세트 줄(용린 4세트의 고룡전 한정 효과)이 **켜진다**.
 *   평소 사냥을 재는 자리에서는 null 로 두어야 한다 — 있지도 않은 힘으로 재게 된다.
 */
/**
 * @param {number|{skill:number,trait:number|null}} planIdx 스킬 순서 번호.
 *   객체로 주면 특성 순서(TRAIT_PLANS)도 함께 고를 수 있다 — 공격형 0 · 방어형 1.
 * @param {string|null} gemPick 홈에 박을 보석. 'none' 이면 **홈을 비운 몸**으로 잰다.
 */
function statsOf(G, cls, level, enh, tier, gearOverride = null, planIdx = 0, gemPick = null, foes = null) {
  const skillIdx = typeof planIdx === 'object' && planIdx ? (planIdx.skill || 0) : planIdx;
  const traitIdx = typeof planIdx === 'object' && planIdx ? (planIdx.trait ?? null) : null;
  ...
  for (const id of [set[cls], ...set.wear]) {
    ...
    for (const [k, v] of Object.entries(d.stats || {})) {
      // 치명타 확률은 강화로 오르지 않는다(formulas.FIXED_STATS).
      const m = FIXED_STATS.has(k) ? 1 : mult;
      // 치명타 피해처럼 배율에 해당하는 것은 스탯 칸이 없다 — mods 로 옮긴다.
      // (이걸 빠뜨리면 지팡이가 아무 특성도 없는 막대기로 측정된다)
      const to = STAT_TO_MOD[k];
      if (to) mods[to] = (mods[to] || 0) + v * m;
      else s[k] = (s[k] || 0) + v * m;
    }
    // 전설의 스킬 효과 배가 — 강화 배율을 태우지 않고, **겹치지도 않는다.**
    // (AffixSystem 의 GEAR_MAX_MODS 와 같은 규칙 — 가장 큰 한 장만 센다)
    if (d.skillPower) skillPower = Math.max(skillPower, 1 + d.skillPower);
    if (!d.enhanceable) continue;
    const extra = { ...avgAffixMods(G, d, enh), ...{} };
    // gemPick 이 있으면 **그것만 박은 사람**(천장 몸), 없으면 통 평균(보통 사람).
    // 'none' 이면 홈을 비운 몸이다 — 보석을 아예 세지 않는다.
    const gem = gemPick === 'none' ? {}
      : (gemPick ? bestGemMods(G, d, enh, gemPick) : avgGemMods(G, d, enh));
    for (const [key, v] of Object.entries(gem)) {
  ...
  const traitMult = {};
  {
    ...
    for (const [id, def] of Object.entries(table)) {
      ...
      //   0.70.13 까지 이 자를 그냥 worn.includes(x) 로 세고 있었다 — 배열은
      //   절대 같지 않으므로 용린은 영원히 3/4 였고, 4세트(고룡전 한정) 효과가
      //   측정에서 통째로 빠져 있었다. AffixSystem.setDefs 와 규칙을 맞춘다.
      const have = def['부위'].filter((x) => (Array.isArray(x)
        ? x.some((y) => worn.includes(y))
        : worn.includes(x))).length;
      for (const row of def['효과'] || []) {
        ...
        // ⚠ **'상대' 가 붙은 줄은 그 상대와 붙을 때만 켜진다**(용린 4세트 — 고룡전 한정).
        //   평소 사냥의 승률을 재는 자리(foes = null)에서 켜 두면 있지도 않은 힘으로
        //   재게 되어 맵 배율이 통째로 부풀려진다. 그래서 기본값은 꺼 둔다.
        //   고룡전을 재는 tools/dragon-gate.js 만 foes 를 준다.
        const only = Array.isArray(row['상대']) ? row['상대'] : [];
        if (only.length && !(foes || []).some((f) => only.includes(f))) continue;
        ...
        // 특성배율은 **다 더한 뒤에** 건다 (SkillSystem.effectiveTraits 와 같은 차례).
        // 여기서 바로 곱하면 뒤에 더해지는 몫에는 안 걸려 게임과 값이 달라진다.
        for (const [prop, value] of Object.entries(row['특성배율'] || {})) {
  ...
  const power = 1 + Math.min(NUM('SKILL_POWER_CAP', 1), skillPower - 1);
  for (const [k, v] of Object.entries(buildMods(G, cls, level, skillIdx).mods)) {
    if (k === 'lowHpThreshold') mods[k] = Math.max(mods[k] || 0, v);
    else mods[k] = (mods[k] || 0) + v * power;
  }

  // ── ②-2 세트의 특성배율 (2 = +200% → 세 배). 다 더한 뒤에 한 번만 건다. ──
  for (const [prop, m] of Object.entries(traitMult)) {
    if (m) statRanks[prop] = Math.round((statRanks[prop] || 0) * (1 + m));
  }
  ...
  // ── ④ 찍어 둔 특성 여섯 갈래 ──
  for (const [name, rank] of Object.entries(buildTraitRanks(G, cls, level, traitIdx))) {
  ...
  // ⚠ 0.70.8 — **방어에서 온 몫은 공격 배율을 타지 않는다.**
  //   차례가 StatBlock 과 어긋나면 이 도구만 딴 사람을 재게 된다.
  s.def = Math.round(s.def * (1 + (mods.defPct || 0)) * (1 + (mods.defMult || 0)));
  const toAtk = Math.min(NUM('DEF_TO_ATK_CAP', 0.4), Math.max(0, mods.defToAtk || 0));
  const fromDef = toAtk > 0 ? Math.round(s.def * toAtk) : 0;
  s.atk = Math.round(s.atk * (1 + (mods.atkPct || 0)) * (1 + (mods.atkMult || 0))) + fromDef;
  s.hp = Math.round(s.hp * (1 + (mods.hpPct || 0)) * (1 + (mods.hpMult || 0)));

  // 치명타 확률의 천장과 넘친 몫의 치명타 피해 전환(StatBlock 과 같은 규칙)
  const CAP = NUM('CRIT_CAP', 0.85);
  const OVER = NUM('CRIT_OVERFLOW_TO_DMG', 10);
  const OVER_CAP = NUM('CRIT_OVERFLOW_CAP', 1);
  const raw = +(s.crit + (mods.crit || 0)).toFixed(4);
  if (raw > CAP) mods.critMult = (mods.critMult || 0) + Math.min(OVER_CAP, (raw - CAP) * OVER);
  s.crit = Math.min(CAP, raw);

  // 들고 있는 무기의 갈래 — 무기에 걸린 패시브(사냥꾼 선제 공격)가 본다.
  // SkillSystem.weaponShape 와 같은 자리(appearance.json)에서 읽는다.
  const weaponShape = ((G['appearance.json'] || {}).weapon || {})[set[cls]];
  return { stats: s, mods, weaponShape: (weaponShape && weaponShape.shape) || null };
}
...
async function winRate(sim, G, cls, level, enh, tier, monId, samples, where = 1, planIdx = 0) {
  ...
  const { stats, mods, weaponShape } = statsOf(G, cls, level, enh, tier, null, planIdx);
  ...
  for (let seed = 1; seed <= samples; seed++) {
    const r = sim({
      player: { name: 'p', level, ...stats, maxHp: stats.hp, weaponShape },
      // ⚠ **스탯만 넘기면 안 된다** (0.70.3에 고침).
      //   rage(분노)와 school(속성)은 stats 바깥에 있어서 여태 빠져 있었다.
      //   그래서 이 자는 게임과 다른 상대를 재고 있었다 —
      //   분노가 없는 보스는 오래 끌수록 약해 보이고, school 이 빠진 고룡은
      //   마법 저항이 아예 안 걸린 것으로 잡힌다(magicShare 참고).
      monster: {
        name: m.name, level: m.level, ...m.stats, maxHp: m.stats.hp, magicPart,
        rage: Number(m.rage) > 0 ? Number(m.rage) : 0,
        // 0.70.14 — 표의 '마법저항' 칸. rage·school 과 같은 자리에 있다.
        magicResist: Number(m.magicResist) > 0 ? Number(m.magicResist) : 0,
...
/**
 * 이 몸이 **의뢰 보상을 뺀 바닥값**인가 (0.64.3).
 *
 *   play10·play15·play20 — 사람이 실제로 그 문 앞에 섰을 때의 몸.
 *     의뢰로 받는 직업 무기 · 룬 건틀릿 · 룬 목걸이 · 용의 반지가 빠져 있다.
 *   5 — 5단계 보스는 field_5 에 서 있는데, **거기까지 걸어가는 것 자체가 의뢰**다
 *     (reach5 → 용린 어깨갑옷). 그러니 그 보스를 만나는 사람은 이미 그걸 차고 있다.
 *     GEAR[5] 에는 없다.
 *
 * 지하감옥 주인(20)은 다르다 — 그 앞에 더 받을 의뢰 보상이 없고,
 * 룬 한 벌을 다 갖춘 몸으로 재므로 목표와 곧바로 견줄 수 있다.
 */
function isFloorBody(tier) {
  return (typeof tier === 'string' && tier.startsWith('play')) || tier === 5;
}
 ...
 */
function verdict(rates, target, floor = false) {
  ...
  if (target != null) {
    ...
    // 0.64.3 — **퀘스트 보상을 안 입힌 줄**은 '너무 어려움' 으로 말하지 않는다.
    //
    //   아래 play* 몸에는 의뢰로 받는 장비가 빠져 있다(GEAR 설명 참고).
    //   그런데 10·15·20단계 보스는 **그 의뢰를 끝낸 사람**이 만나는 상대다 —
    //   용린 어깨갑옷 · 직업 무기 · 룬 건틀릿 · 룬 목걸이 · 용의 반지가 다 빠진
    //   몸으로 재고 있으니 낮게 나오는 것이 당연하다.
    //   그걸 판마다 '너무 어려움 -40%p' 로 찍으면, 볼 때마다 고칠 것이 있는 줄 알고
    //   멀쩡한 표를 건드리게 된다. 실제로 그럴 뻔했다.
    //   숫자는 그대로 두고 **무엇을 재고 있는지**를 적는다.
    if (floor) notes.push(`${avg}% (퀘스트 보상 없는 몸 — 바닥값, 목표 ${target}% 는 참고)`);
    else if (Math.abs(off) <= 10) notes.push(`목표 ${target}% ✓`);
...
if (require.main === module) (async () => {
  ...
  for (const [monId, lv, enh, tier, label, target, where = 1] of list) {
    ...
    console.log('  ' + pad(label, 34) + pad(`Lv.${lv} +${enh}`, 12)
      ...
      + '   ' + verdict(now, target, isFloorBody(tier)));
  ...
  console.log('');
  console.log('  ⚠ 이 도구는 **의뢰 보상 장비를 안 입힙니다.** (0.64.3)');
  console.log('     10·15·20단계 보스는 그 앞의 의뢰를 끝낸 사람이 만나는 상대인데,');
  console.log('     용린 어깨갑옷 · 직업 무기 · 룬 건틀릿 · 룬 목걸이 · 용의 반지가');
  console.log('     다 빠진 몸으로 재고 있습니다. 그 줄의 숫자는 **바닥값**입니다 —');
  console.log('     실제로 그 문 앞에 서는 사람은 이보다 한참 셉니다.');
  console.log('     5단계도 같은 이유로 낮게 나옵니다(용린 어깨갑옷·직업 무기가 빠져 있습니다).');
  console.log('  직업 격차가 30%p 를 넘으면 한 직업이 정답이 되어 버립니다.');
  console.log('');
})();

// 다른 도구가 이 계산을 그대로 쓰도록 열어 둔다(측정 방법이 갈라지지 않게).
module.exports = { loadFrom, statsOf, potionsOf, winRate, bestWinRate, scaleMon, MATCHES, TRASH, GEAR, PLAN, PLANS, TRAIT_PLAN, TRAIT_PLANS };
