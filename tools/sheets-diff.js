...
// 계산해서 보여 주기만 하는 칸 (게임은 읽지 않는다 — server/sheets.js 참고).
const COMPUTED = { monsters: ['실제 값'] };
...
// accounts.csv 는 게임 표가 아니다 — 서버에서 떠 온 **계정 목록 사진**이다(0.59).
// 견줄 대상이 아니므로 건너뛴다. 안 그러면 판마다 "시트에 이 탭이 없습니다" 가 뜬다.
const NOT_A_TABLE = new Set(['accounts']);

for (const f of fs.readdirSync(SHEET_DIR).filter((n) => n.endsWith('.csv'))) {
  const tab = f.replace(/\.csv$/, '');
  if (NOT_A_TABLE.has(tab)) continue;
  ...
  // 계산해서 보여 주기만 하는 칸은 견주지 않는다 (0.70.6).
  //
  //   monsters 의 '실제 값' 은 시트 안에서 다시 계산되지 않는다 — 사람이 공격력을
  //   고쳐도 그 칸은 옛 숫자 그대로다. 그걸 "다른 곳" 으로 세면 몬스터 스물넷이
  //   전부 빨갛게 떠서 **진짜 고친 한 줄이 묻힌다.**
  const skipCols = new Set();
  (COMPUTED[tab] || []).forEach((h) => {
    const i = lHead.indexOf(h);
    if (i >= 0) skipCols.add(i);
  });
  ...
  if (wantTake(tab) && headSame) {
    ...
    for (const r of localRows.slice(1)) {
      if (!r[0]) continue;
      // 양쪽에 있으면 시트 값이 이긴다. 다만 계산 칸만은 **여기 값**을 남긴다 —
      // 시트 쪽은 사람이 숫자를 고친 뒤의 옛 계산 결과라 가져오면 거짓말이 된다.
      const sRow = S.get(r[0]);
      if (sRow) for (const i of skipCols) sRow[i] = r[i] || '';
      out.push(sRow || r);
      seen.add(r[0]);
    }
