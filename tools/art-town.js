...
// 마을 타일이 깔고 앉는 풀 (0.67).
//
// ⚠ **그라데이션을 넣으면 안 된다.** 예전에는 위→아래로 밝기가 흘렀는데,
//   같은 장을 격자로 이어 붙이면 칸마다 밝기가 끊겨 **가로줄무늬**가 된다
//   (0.65 에 들판에서 겪고 걷어냈다. 마을 쪽은 그때 놓쳤다).
//   밑색은 들판 풀(gen-assets 의 GRASS_BG)과 **같은 값**이어야 한다 —
//   다르면 마을 울타리 칸만 색이 뜬다.
const grassBase = `
  <rect width="32" height="32" fill="#49a34a"/>
  <g stroke-linecap="round" fill="none">
    <path d="M4 26q2-5 4 0M22 27q2-5 4 0M8 12q2-5 4 0" stroke="#5fbc5c" stroke-width="1.3" opacity="0.8"/>
    <path d="M12 21q2-5 4 0M26 14q2-5 4 0M17 8q2-5 4 0" stroke="#3f9642" stroke-width="1.2" opacity="0.75"/>
  </g>
  <ellipse cx="7" cy="19" rx="6" ry="4" fill="#4fa94f" opacity="0.45"/>
  <ellipse cx="24" cy="6" rx="7" ry="4.5" fill="#449c46" opacity="0.4"/>`;
...
// ── 성 정면 (0.68) ───────────────────────────────────────────
//
// 마을 북쪽에서 보이는 성이다. 0.67 까지는 **한 장짜리 벽 그림을 격자로 도배**한
// 것이 전부라, 성이 아니라 회색 담장으로 보였다. 성문도 32×32 한 칸짜리 아치라
// 바로 아래 민가의 나무문과 크기가 같았다 — 문이 같으면 집도 같아 보인다.
//
// 그래서 **여러 칸에 걸친 그림 한 장**을 그려 놓고 한 칸씩 오려 쓴다.
// 성문은 2칸×2칸(64×64), 망루는 2칸 폭이다.
// 칸마다 따로 그리면 이어 붙였을 때 선이 어긋난다 — 실제로 어긋나 있었다.
//
// quad(그림, 몇째 칸 가로, 몇째 칸 세로) → 그 칸만 보이는 32×32 타일.
const quad = (art, cx, cy) =>
  `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">
     <g transform="translate(${-cx * 32} ${-cy * 32})">${art}</g>
   </svg>`;

/** 성벽 돌 — 큰 그림 어디에 깔아도 이어지는 밑바탕. */
const CASTLE_STONE = `
  <rect x="-8" y="-8" width="96" height="160" fill="#7d8296"/>
  <g fill="#888da2">
    <rect x="-8" y="0" width="26" height="10" rx="1.6"/><rect x="20" y="0" width="30" height="10" rx="1.6"/>
    <rect x="52" y="0" width="30" height="10" rx="1.6"/>
    <rect x="-2" y="12" width="30" height="10" rx="1.6"/><rect x="30" y="12" width="26" height="10" rx="1.6"/>
    <rect x="58" y="12" width="24" height="10" rx="1.6"/>
    <rect x="-8" y="24" width="26" height="10" rx="1.6"/><rect x="20" y="24" width="30" height="10" rx="1.6"/>
    <rect x="52" y="24" width="30" height="10" rx="1.6"/>
    <rect x="-2" y="36" width="30" height="10" rx="1.6"/><rect x="30" y="36" width="26" height="10" rx="1.6"/>
    <rect x="58" y="36" width="24" height="10" rx="1.6"/>
    <rect x="-8" y="48" width="26" height="10" rx="1.6"/><rect x="20" y="48" width="30" height="10" rx="1.6"/>
    <rect x="52" y="48" width="30" height="10" rx="1.6"/>
  </g>
  <g fill="#969bb0" opacity="0.6">
    <rect x="2" y="2" width="10" height="3" rx="1.5"/>
    <rect x="34" y="14" width="10" height="3" rx="1.5"/>
    <rect x="6" y="26" width="10" height="3" rx="1.5"/>
    <rect x="38" y="38" width="10" height="3" rx="1.5"/>
    <rect x="2" y="50" width="10" height="3" rx="1.5"/>
  </g>`;

/**
 * 성문 — 2칸 × 2칸 (64 × 64).
 *
 * 민가 문(32×32 한 칸)보다 **네 배** 크다. 그래야 같은 마을 안에서
 * "저건 집이고 저건 성이다" 가 크기만으로 읽힌다.
 * 아치 · 쇠창살 · 좌우 횃불 · 안쪽의 어둠까지 한 장에 그린다.
 */
const CASTLE_GATE_ART = `
  ${CASTLE_STONE}
  <!-- 아치를 두른 큰 돌 -->
  <path d="M4 64 L4 30 A28 28 0 0 1 60 30 L60 64 Z" fill="#9aa0b6"/>
  <path d="M7 64 L7 31 A25 25 0 0 1 57 31 L57 64 Z" fill="#6f748a"/>
  <!-- 문 안쪽 어둠 -->
  <path d="M11 64 L11 32 A21 21 0 0 1 53 32 L53 64 Z" fill="#14131c"/>
  <path d="M11 64 L11 32 A21 21 0 0 1 53 32 L53 64 Z" fill="#2b2838" opacity="0.55"/>
  <!-- 쇠창살(올라가 있다 — 지나갈 수 있는 문이라는 뜻) -->
  <g stroke="#4a4a58" stroke-width="2.2" opacity="0.9">
    <path d="M16 32 L16 46M22 30 L22 46M28 29 L28 46M34 29 L34 46M40 30 L40 46M46 32 L46 46"/>
    <path d="M12 36 L52 36M12 42 L52 42"/>
  </g>
  <g stroke="#6a6a7c" stroke-width="1" opacity="0.7">
    <path d="M15 32 L15 46M27 29 L27 46M39 30 L39 46"/>
  </g>
  <!-- 아치 돌 이음매 -->
  <g stroke="#585d70" stroke-width="1.3" fill="none" opacity="0.85">
    <path d="M9 44 L4 44M9 36 L4 36M11 28 L5 26M17 20 L14 15M26 15 L25 9M38 15 L39 9M47 20 L50 15M53 28 L59 26M55 36 L60 36M55 44 L60 44"/>
  </g>
  <!-- 문 위 방패 문장 -->
  <path d="M32 4 L40 7 L40 14 Q40 19 32 22 Q24 19 24 14 L24 7 Z" fill="#c0392b"/>
  <path d="M32 6.5 L37.6 8.6 L37.6 14 Q37.6 17.6 32 19.6 Q26.4 17.6 26.4 14 L26.4 8.6 Z" fill="#e05a4a"/>
  <path d="M32 9 L34 13 L32 17 L30 13 Z" fill="#ffd166"/>
  <!-- 좌우 횃불 -->
  <g>
    <rect x="1.5" y="34" width="3" height="9" rx="1.4" fill="#5a3a1e"/>
    <ellipse cx="3" cy="32" rx="3.2" ry="4.4" fill="#ff9d3a" opacity="0.92"/>
    <ellipse cx="3" cy="32.8" rx="1.6" ry="2.6" fill="#ffe089"/>
    <ellipse cx="3" cy="33" rx="7" ry="8" fill="#ffb457" opacity="0.16"/>
    <rect x="59.5" y="34" width="3" height="9" rx="1.4" fill="#5a3a1e"/>
    <ellipse cx="61" cy="32" rx="3.2" ry="4.4" fill="#ff9d3a" opacity="0.92"/>
    <ellipse cx="61" cy="32.8" rx="1.6" ry="2.6" fill="#ffe089"/>
    <ellipse cx="61" cy="33" rx="7" ry="8" fill="#ffb457" opacity="0.16"/>
  </g>`;

/**
 * 망루 — 2칸 폭, 위 두 칸(꼭대기 + 몸통).
 * 성벽보다 한 칸 높이 솟아 있어야 담장이 아니라 성으로 보인다.
 */
const CASTLE_TOWER_ART = `
  <!-- 몸통(아래 칸) -->
  <rect x="4" y="30" width="56" height="40" fill="#7d8296"/>
  <path d="M4 30 L14 30 L14 70 L4 70 Z" fill="#8f94a8"/>
  <path d="M50 30 L60 30 L60 70 L50 70 Z" fill="#6c7185"/>
  <g fill="#888da2">
    <rect x="16" y="32" width="14" height="9" rx="1.6"/><rect x="32" y="32" width="16" height="9" rx="1.6"/>
    <rect x="16" y="43" width="18" height="9" rx="1.6"/><rect x="36" y="43" width="12" height="9" rx="1.6"/>
    <rect x="16" y="54" width="13" height="9" rx="1.6"/><rect x="31" y="54" width="17" height="9" rx="1.6"/>
  </g>
  <!-- 화살 구멍 -->
  <path d="M30 40 L34 40 L34 54 L32 57 L30 54 Z" fill="#1b1a24"/>
  <path d="M31 41.5 L33 41.5 L33 52 L32 54 L31 52 Z" fill="#3b3a48"/>
  <!-- 꼭대기 총안(위 칸) -->
  <rect x="2" y="18" width="60" height="13" fill="#8f94a8"/>
  <g fill="#a2a7bb">
    <rect x="2" y="10" width="10" height="9"/><rect x="17" y="10" width="10" height="9"/>
    <rect x="37" y="10" width="10" height="9"/><rect x="52" y="10" width="10" height="9"/>
  </g>
  <rect x="2" y="17" width="60" height="2.4" fill="#5f6479"/>
  <!-- 깃발 -->
  <rect x="31" y="0" width="2" height="12" fill="#4a4a58"/>
  <path d="M33 1 L47 4 L33 7 Z" fill="#c0392b"/>
  <path d="M33 1 L40 2.5 L33 4 Z" fill="#e05a4a"/>`;

/** 성벽 총안 — 성문 좌우 담장 꼭대기. */
const CASTLE_BATT_ART = `
  ${CASTLE_STONE}
  <rect x="-8" y="16" width="96" height="4" fill="#5f6479"/>
  <g fill="#a2a7bb">
    <rect x="-8" y="0" width="12" height="16"/><rect x="10" y="0" width="12" height="16"/>
    <rect x="28" y="0" width="12" height="16"/><rect x="46" y="0" width="12" height="16"/>
    <rect x="64" y="0" width="12" height="16"/>
  </g>
  <g fill="#8f94a8">
    <rect x="4" y="0" width="6" height="14"/><rect x="22" y="0" width="6" height="14"/>
    <rect x="40" y="0" width="6" height="14"/><rect x="58" y="0" width="6" height="14"/>
  </g>`;

/** 성벽 창 — 밋밋한 담장에 눈이 걸리는 자리를 만든다. */
const CASTLE_WINDOW_ART = `
  ${CASTLE_STONE}
  <path d="M10 30 L10 14 A6 6 0 0 1 22 14 L22 30 Z" fill="#4c5165"/>
  <path d="M12 28 L12 15 A4 4 0 0 1 20 15 L20 28 Z" fill="#1d1c28"/>
  <path d="M13 26 L13 16 A3 3 0 0 1 19 16 L19 22 Z" fill="#3a4a6b" opacity="0.7"/>
  <path d="M9 30 L23 30 L23 32 L9 32 Z" fill="#9aa0b6"/>`;

// ---------------- 마을/성 타일 ----------------
const TOWN_TILES = {
  // 광장 바닥 — 네모 반듯한 벽돌이 아니라 **깐 돌**이다 (0.67).
  //
  // 예전에는 줄눈이 곧은 격자라, 광장을 넓게 깔면 모눈종이처럼 보였다.
  // 돌마다 크기와 색을 조금씩 달리하고 모서리를 둥글렸다.
  brick: t(`
    <rect width="32" height="32" fill="#8e8375"/>
    <g stroke="none">
      <rect x="0.6" y="0.6" width="13.6" height="9.4" rx="2.2" fill="#c3b39d"/>
      <rect x="15.4" y="0.6" width="16" height="9.4" rx="2.2" fill="#b8a892"/>
      <rect x="0.6" y="11.2" width="9.6" height="9.4" rx="2.2" fill="#bcac96"/>
      <rect x="11.4" y="11.2" width="12.4" height="9.4" rx="2.2" fill="#c7b7a1"/>
      <rect x="25" y="11.2" width="6.4" height="9.4" rx="2.2" fill="#b3a38d"/>
      <rect x="0.6" y="21.8" width="15.2" height="9.6" rx="2.2" fill="#bdad97"/>
      <rect x="17" y="21.8" width="14.4" height="9.6" rx="2.2" fill="#c5b59f"/>
    </g>
    <g fill="#d3c5b1" opacity="0.5">
      <rect x="2" y="2" width="6" height="2.6" rx="1.3"/>
      <rect x="17" y="12.6" width="6" height="2.4" rx="1.2"/>
      <rect x="19" y="23.2" width="7" height="2.4" rx="1.2"/>
    </g>
    <circle cx="24" cy="6" r="1" fill="#7d7365" opacity="0.5"/>
    <circle cx="6" cy="27" r="0.9" fill="#7d7365" opacity="0.45"/>`),

  // 두 번째 장 — 광장은 넓다. 한 장만 깔면 같은 무늬가 줄 서는 게 보인다.
  brick2: t(`
    <rect width="32" height="32" fill="#8e8375"/>
    <g stroke="none">
      <rect x="0.6" y="0.6" width="9.4" height="9.4" rx="2.2" fill="#bcac96"/>
      <rect x="11.2" y="0.6" width="12.6" height="9.4" rx="2.2" fill="#c6b6a0"/>
      <rect x="25" y="0.6" width="6.4" height="9.4" rx="2.2" fill="#b4a48e"/>
      <rect x="0.6" y="11.2" width="15" height="9.4" rx="2.2" fill="#c1b19b"/>
      <rect x="16.8" y="11.2" width="14.6" height="9.4" rx="2.2" fill="#b7a791"/>
      <rect x="0.6" y="21.8" width="11" height="9.6" rx="2.2" fill="#c4b49e"/>
      <rect x="12.8" y="21.8" width="18.6" height="9.6" rx="2.2" fill="#baaa94"/>
    </g>
    <g fill="#d3c5b1" opacity="0.45">
      <rect x="13" y="2" width="6" height="2.4" rx="1.2"/>
      <rect x="2" y="12.8" width="7" height="2.4" rx="1.2"/>
      <rect x="15" y="23.2" width="8" height="2.4" rx="1.2"/>
    </g>
    <circle cx="9" cy="16" r="1" fill="#7d7365" opacity="0.45"/>
    <circle cx="26" cy="27" r="0.9" fill="#7d7365" opacity="0.4"/>`),

  // 벽 — 회칠에 나무 골조를 드러낸 집 (0.67).
  // 처마 밑 그늘을 한 줄 넣으면 지붕이 벽 위에 '얹혀' 보인다.
  house_wall: t(`
    <rect width="32" height="32" fill="#e3d3b4"/>
    <rect y="0" width="32" height="4" fill="#a98f68" opacity="0.55"/>
    <rect y="4" width="32" height="1.6" fill="#c9b795" opacity="0.7"/>
    <g stroke="#8a6a45" stroke-width="2.4" stroke-linecap="square">
      <path d="M3.5 5v27M28.5 5v27"/><path d="M3.5 19h25"/>
    </g>
    <path d="M3.5 19 L16 5.5 L28.5 19" stroke="#8a6a45" stroke-width="1.6" fill="none" opacity="0.5"/>
    <rect x="9" y="7" width="14" height="9.6" rx="1.5" fill="#5f96c2" stroke="#8a6a45" stroke-width="2"/>
    <rect x="9.8" y="7.8" width="5.8" height="3.6" fill="#8fc4e6" opacity="0.65"/>
    <path d="M16 7v9.6M9 11.8h14" stroke="#8a6a45" stroke-width="1.6"/>
    <rect x="0" y="29.5" width="32" height="2.5" fill="#b09873" opacity="0.5"/>`),

  // 지붕 — 기와를 한 장씩 겹쳐 얹는다 (0.67).
  //
  // 예전에는 가로 띠 세 줄이라 지붕이 아니라 **차양**처럼 보였다.
  // 기와를 낱장으로 깔고 줄마다 반 장씩 어긋내면(엇갈려 쌓기) 결이 생긴다.
  house_roof: t(`
    <rect width="32" height="32" fill="#8c322c"/>
    <g fill="#bb5147">
      <path d="M-4 0h8v9h-8z" /><path d="M6 0h8v9H6z"/><path d="M16 0h8v9h-8z"/><path d="M26 0h8v9h-8z"/>
      <path d="M1 11h8v9H1z"/><path d="M11 11h8v9h-8z"/><path d="M21 11h8v9h-8z"/><path d="M-9 11h8v9h-8z"/>
      <path d="M-4 22h8v9h-8z"/><path d="M6 22h8v9H6z"/><path d="M16 22h8v9h-8z"/><path d="M26 22h8v9h-8z"/>
    </g>
    <g fill="#cf6055" opacity="0.9">
      <path d="M-4 0h8v3h-8z"/><path d="M6 0h8v3H6z"/><path d="M16 0h8v3h-8z"/><path d="M26 0h8v3h-8z"/>
      <path d="M1 11h8v3H1z"/><path d="M11 11h8v3h-8z"/><path d="M21 11h8v3h-8z"/>
      <path d="M-4 22h8v3h-8z"/><path d="M6 22h8v3H6z"/><path d="M16 22h8v3h-8z"/><path d="M26 22h8v3h-8z"/>
    </g>
    <g fill="#6f251f" opacity="0.55">
      <rect x="0" y="9" width="32" height="2.2"/><rect x="0" y="20" width="32" height="2.2"/>
      <rect x="0" y="30.8" width="32" height="1.2"/>
    </g>`),
  ...
  // 성 바닥 — 커다란 판석 (0.67).
  //
  // 예전에는 32칸을 4등분한 체크무늬라, 홀을 넓게 깔면 바둑판이 됐다.
  // 판석을 크게 잡고 줄눈만 남긴다. 닳은 자국은 얼룩으로.
  castle_floor: t(`
    <rect width="32" height="32" fill="#4e5265"/>
    <g stroke="none">
      <rect x="0.7" y="0.7" width="19.4" height="14.4" rx="1.6" fill="#666b83"/>
      <rect x="21.1" y="0.7" width="10.2" height="14.4" rx="1.6" fill="#5f6479"/>
      <rect x="0.7" y="16.3" width="11.2" height="15" rx="1.6" fill="#61667d"/>
      <rect x="12.9" y="16.3" width="18.4" height="15" rx="1.6" fill="#6a6f88"/>
    </g>
    <ellipse cx="9" cy="7" rx="7" ry="4" fill="#757a94" opacity="0.35"/>
    <ellipse cx="22" cy="24" rx="7" ry="4" fill="#757a94" opacity="0.3"/>
    <path d="M4 11 q4 1.5 8 0" stroke="#454959" stroke-width="1" fill="none" opacity="0.6"/>
    <path d="M18 27 q4 1.2 8 0" stroke="#454959" stroke-width="0.9" fill="none" opacity="0.5"/>`),

  castle_floor2: t(`
    <rect width="32" height="32" fill="#4e5265"/>
    <g stroke="none">
      <rect x="0.7" y="0.7" width="10.6" height="14.4" rx="1.6" fill="#63687f"/>
      <rect x="12.3" y="0.7" width="19" height="14.4" rx="1.6" fill="#6a6f88"/>
      <rect x="0.7" y="16.3" width="18.8" height="15" rx="1.6" fill="#666b83"/>
      <rect x="20.5" y="16.3" width="10.8" height="15" rx="1.6" fill="#5d6277"/>
    </g>
    <ellipse cx="23" cy="7" rx="7" ry="4" fill="#757a94" opacity="0.32"/>
    <ellipse cx="9" cy="24" rx="7" ry="4" fill="#757a94" opacity="0.28"/>
    <path d="M16 11 q4 1.5 8 0" stroke="#454959" stroke-width="1" fill="none" opacity="0.55"/>
    <path d="M4 27 q4 1.2 8 0" stroke="#454959" stroke-width="0.9" fill="none" opacity="0.45"/>`),

  // 성 정면 조각들 — 위 큰 그림에서 한 칸씩 오려 낸다.
  castle_gate_ul: quad(CASTLE_GATE_ART, 0, 0),
  castle_gate_ur: quad(CASTLE_GATE_ART, 1, 0),
  castle_gate_dl: quad(CASTLE_GATE_ART, 0, 1),
  castle_gate_dr: quad(CASTLE_GATE_ART, 1, 1),
  castle_tower_tl: quad(CASTLE_TOWER_ART, 0, 0),
  castle_tower_tr: quad(CASTLE_TOWER_ART, 1, 0),
  castle_tower_bl: quad(CASTLE_TOWER_ART, 0, 1),
  castle_tower_br: quad(CASTLE_TOWER_ART, 1, 1),
  castle_batt: quad(CASTLE_BATT_ART, 0, 0),
  castle_window: quad(CASTLE_WINDOW_ART, 0, 0),

  // ── 성 안 (0.68) ───────────────────────────────────────────
  //
  // 0.67 까지 성 안은 **판석만 깔린 넓은 빈 방**이었다. 왕이 있고 대장간이 있고
  // 상점이 있는데, 그 셋 사이에 아무것도 없어서 "큰 방"이지 "왕이 있는 곳"이
  // 아니었다. 융단·기둥·창문·횃불 넷이면 방이 홀이 된다.
  castle_rug: t(`
    <rect width="32" height="32" fill="#8e2f2a"/>
    <rect x="0" y="0" width="32" height="32" fill="#a3392f"/>
    <rect x="3" y="0" width="26" height="32" fill="#b5443a"/>
    <g stroke="#e0b25a" stroke-width="1.6" fill="none" opacity="0.9">
      <path d="M3.4 0 L3.4 32M28.6 0 L28.6 32"/>
    </g>
    <g stroke="#8e2f2a" stroke-width="1" fill="none" opacity="0.6">
      <path d="M6 0 L6 32M26 0 L26 32"/>
    </g>
    <g fill="#e0b25a" opacity="0.55">
      <path d="M16 6 l3 4 -3 4 -3 -4 z"/>
      <path d="M16 22 l3 4 -3 4 -3 -4 z"/>
    </g>`),

  // 융단 끝단 — 위·아래 끝에 술이 달린다.
  castle_rug_end: t(`
    <rect x="0" y="0" width="32" height="24" fill="#a3392f"/>
    <rect x="3" y="0" width="26" height="24" fill="#b5443a"/>
    <g stroke="#e0b25a" stroke-width="1.6" fill="none" opacity="0.9">
      <path d="M3.4 0 L3.4 24M28.6 0 L28.6 24M0 22.6 L32 22.6"/>
    </g>
    <path d="M16 6 l3 4 -3 4 -3 -4 z" fill="#e0b25a" opacity="0.55"/>
    <g stroke="#e0b25a" stroke-width="1.4" opacity="0.85" stroke-linecap="round">
      <path d="M3 24 L3 28M7 24 L7 28.5M11 24 L11 28M15 24 L15 28.5M19 24 L19 28M23 24 L23 28.5M27 24 L27 28"/>
    </g>`),

  // 기둥 — 홀의 뼈대. 지나갈 수 없다.
  castle_pillar: t(`
    <ellipse cx="16" cy="29" rx="11" ry="3.2" fill="#000" opacity="0.3"/>
    <rect x="5" y="25" width="22" height="5" rx="1.6" fill="#9aa0b6"/>
    <rect x="8" y="4" width="16" height="21" fill="#8f94a8"/>
    <rect x="8" y="4" width="5" height="21" fill="#a6abbf"/>
    <rect x="20" y="4" width="4" height="21" fill="#767b90"/>
    <g stroke="#6f748a" stroke-width="1" opacity="0.6">
      <path d="M14 4 L14 25M18 4 L18 25"/>
    </g>
    <rect x="5" y="0" width="22" height="5" rx="1.6" fill="#a6abbf"/>
    <rect x="7" y="4.6" width="18" height="2" rx="1" fill="#6f748a" opacity="0.7"/>`),

  // 창 — 색유리. 빛이 바닥으로 흘러내린다.
  castle_glass: t(`
    <path d="M7 30 L7 12 A9 9 0 0 1 25 12 L25 30 Z" fill="#2b2f42"/>
    <path d="M9 28 L9 13 A7 7 0 0 1 23 13 L23 28 Z" fill="#3f6ea8"/>
    <path d="M16 6.5 A7 7 0 0 1 23 13 L23 28 L16 28 Z" fill="#4f83c0" opacity="0.85"/>
    <path d="M12 16 h8 v6 h-8 z" fill="#c0503f" opacity="0.85"/>
    <path d="M13.4 9 h5.2 v4.4 h-5.2 z" fill="#e0b25a" opacity="0.8"/>
    <g stroke="#2b2f42" stroke-width="1.3" fill="none">
      <path d="M16 6.5 L16 28M9 16 L23 16M9 22 L23 22"/>
    </g>
    <path d="M7 29 L25 29 L25 32 L7 32 Z" fill="#9aa0b6"/>`),

  // 벽 횃불 — 성 안과 지하감옥이 같이 쓴다.
  wall_torch: t(`
    <rect width="32" height="32" fill="#4e5265"/>
    <ellipse cx="16" cy="13" rx="12" ry="13" fill="#ffb457" opacity="0.16"/>
    <ellipse cx="16" cy="13" rx="7" ry="8" fill="#ffb457" opacity="0.18"/>
    <rect x="13" y="16" width="6" height="12" rx="2" fill="#4a4a58"/>
    <rect x="14" y="17" width="1.6" height="10" fill="#6a6a7c"/>
    <path d="M11 18 q5 4 10 0" stroke="#4a4a58" stroke-width="2.4" fill="none"/>
    <ellipse cx="16" cy="12" rx="4.4" ry="6.4" fill="#ff7a2a" opacity="0.95"/>
    <ellipse cx="16" cy="12.8" rx="2.8" ry="4.4" fill="#ff9d3a"/>
    <ellipse cx="16" cy="13.4" rx="1.4" ry="2.6" fill="#ffe089"/>`),
...
const NPCS = {
  ...
  // 보석 도박 노인 — 대장간 오른쪽 좌판에 앉은 할아버지 (0.70.14).
  //
  // 촌장과 헷갈리면 안 되므로 **다른 사람으로 읽히게** 그린다:
  //   촌장은 흰 수염에 회색 옷·지팡이. 이쪽은 **자줏빛 옷에 챙 모자**,
  //   수염은 짧고, 손에는 지팡이 대신 **보석 세 알이 놓인 좌판**이 있다.
  // 자줏빛은 마을에서 이 사람만 입는다 — 멀리서도 누구인지 바로 안다.
  npc_gambler: person({
    id: 'gmb', skin: '#e6c49e', skinDark: '#b58f68',
    top: '#6b3f86', top2: '#42264f', bottom: '#332433',
    hair: '#b9b2a6', hair2: '#e2ddd2', age: 'old', eyes: 'narrow',
    beard: 'short', beardColor: '#dcd6c8', brow: '#cfc8ba', blush: 0.22,
    // 옆머리만 남은 정수리 — 모자 밑으로 삐져나온다
    hairShape: 'M26 35 Q26 24 32 19 Q30 27 31 35 Z M70 35 Q70 24 64 19 Q66 27 65 35 Z',
    hat: `<!-- 챙 모자 -->
          <path d="M22 26 Q48 18 74 26 Q48 31 22 26 Z" fill="#33203d"/>
          <path d="M31 25 Q48 9 65 25 Q48 21 31 25 Z" fill="#4a2c59"/>
          <path d="M31 24 Q48 11 65 24" stroke="#6b3f86" stroke-width="1.6" fill="none"/>
          <circle cx="60" cy="17" r="2.6" fill="#e0483f" opacity="0.9"/>`,
    mouth: `<path d="M43 47.6 q5 2.4 10 -0.6" stroke="#8a5a48" stroke-width="1.6" fill="none" stroke-linecap="round"/>`,
    prop: `<!-- 무릎 위 좌판 -->
           <path d="M26 74 L70 74 L72 88 L24 88 Z" fill="#5a4130"/>
           <path d="M26 74 L70 74 L69 77 L27 77 Z" fill="#7a5b41"/>
           <!-- 보석 세 알 — 붉은·푸른·흰 -->
           <path d="M36 78 l3.4 3 -3.4 3.6 -3.4 -3.6 Z" fill="#e0483f"/>
           <path d="M48 77.6 l3.6 3.2 -3.6 3.8 -3.6 -3.8 Z" fill="#4aa3e0"/>
           <path d="M60 78 l3.4 3 -3.4 3.6 -3.4 -3.6 Z" fill="#ecebe4"/>
           <path d="M36 78 l3.4 3 -3.4 0.6 Z" fill="#ff8a80" opacity="0.85"/>
           <path d="M48 77.6 l3.6 3.2 -3.6 0.7 Z" fill="#9ed2f5" opacity="0.85"/>`,
  }),
