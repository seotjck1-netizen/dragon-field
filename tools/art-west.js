 ...
 * 절벽 들판 바닥 (0.67).
 *
 * ⚠ **그라데이션도, 테두리 선도 넣지 않는다.**
 *   예전에는 위→아래로 밝기가 흐르고 칸마다 검은 테두리를 한 겹 둘렀다.
 *   같은 장을 격자로 이어 붙이면 밝기가 칸마다 끊기고 테두리가 격자로
 *   드러나서, 땅이 아니라 **바둑판**으로 보였다(들판 풀에서 0.65 에 겪은
 *   것과 같은 일이다 — 그때 이쪽은 놓쳤다).
 *   밑색은 평평하게, 변화는 흩어 놓은 얼룩과 자갈로만 준다.
 */
const wasteBase = `
  <rect width="32" height="32" fill="#8a8065"/>
  <ellipse cx="8" cy="10" rx="10" ry="6.5" fill="#938872" opacity="0.5"/>
  <ellipse cx="25" cy="24" rx="9" ry="6" fill="#7e755d" opacity="0.5"/>
  <path d="M3 8 L9 11 M18 5 L23 9 M6 22 L11 25 M21 20 L27 23 M13 15 L17 17"
        stroke="#6a6047" stroke-width="1.1" opacity="0.5" stroke-linecap="round"/>
  <path d="M8 27 q1.5-4 3 0M24 13 q1.5-4 3 0M15 24 q1.5-4 3 0"
        stroke="#b6a983" stroke-width="1.2" fill="none" stroke-linecap="round" opacity="0.6"/>
  <circle cx="12" cy="6" r="1.1" fill="#6f6650" opacity="0.6"/>
  <circle cx="28" cy="16" r="0.9" fill="#6f6650" opacity="0.55"/>
  <circle cx="19" cy="29" r="1" fill="#6f6650" opacity="0.5"/>`;

/** 두 번째·세 번째 장 — 넓은 땅은 한 장으로 깔면 격자가 보인다. */
const wasteBase2 = `
  <rect width="32" height="32" fill="#8a8065"/>
  <ellipse cx="24" cy="8" rx="10" ry="6" fill="#938872" opacity="0.45"/>
  <ellipse cx="7" cy="23" rx="9.5" ry="6.5" fill="#7e755d" opacity="0.45"/>
  <path d="M22 9 L27 12 M5 6 L10 9 M19 24 L25 27 M8 18 L13 20"
        stroke="#6a6047" stroke-width="1.1" opacity="0.45" stroke-linecap="round"/>
  <path d="M5 14 q1.5-4 3 0M27 26 q1.5-4 3 0"
        stroke="#b6a983" stroke-width="1.2" fill="none" stroke-linecap="round" opacity="0.55"/>
  <circle cx="16" cy="4" r="1" fill="#6f6650" opacity="0.55"/>
  <circle cx="9" cy="29" r="0.9" fill="#6f6650" opacity="0.5"/>`;

const wasteBase3 = `
  <rect width="32" height="32" fill="#8a8065"/>
  <ellipse cx="16" cy="16" rx="12" ry="8" fill="#928771" opacity="0.4"/>
  <ellipse cx="30" cy="4" rx="8" ry="5" fill="#7e755d" opacity="0.4"/>
  <path d="M10 12 L15 15 M24 19 L29 22 M4 26 L9 28"
        stroke="#6a6047" stroke-width="1.05" opacity="0.45" stroke-linecap="round"/>
  <path d="M18 8 q1.5-4 3 0M11 27 q1.5-4 3 0"
        stroke="#b6a983" stroke-width="1.15" fill="none" stroke-linecap="round" opacity="0.55"/>
  <circle cx="26" cy="11" r="1" fill="#6f6650" opacity="0.5"/>
  <circle cx="6" cy="17" r="0.9" fill="#6f6650" opacity="0.45"/>`;

/**
 * 네 번째·다섯 번째 장 (0.70.19).
 *
 * 서쪽 들판은 한 화면이 거의 다 이 땅이라, 세 장으로도 주기가 눈에 잡혔다.
 * 다섯 장이면 가로로 훑을 때 같은 그림이 이웃하는 일이 드물어진다.
 */
const wasteBase4 = `
  <rect width="32" height="32" fill="#8a8065"/>
  <ellipse cx="6" cy="7" rx="9" ry="6" fill="#938872" opacity="0.42"/>
  <ellipse cx="25" cy="20" rx="10" ry="6.5" fill="#7e755d" opacity="0.45"/>
  <path d="M14 5 L19 8 M27 14 L31 17 M3 20 L8 23 M17 27 L22 29"
        stroke="#6a6047" stroke-width="1.1" opacity="0.45" stroke-linecap="round"/>
  <path d="M22 10 q1.5-4 3 0M7 26 q1.5-4 3 0M29 5 q1.5-4 3 0"
        stroke="#b6a983" stroke-width="1.2" fill="none" stroke-linecap="round" opacity="0.55"/>
  <circle cx="12" cy="15" r="1" fill="#6f6650" opacity="0.52"/>
  <circle cx="30" cy="27" r="0.9" fill="#6f6650" opacity="0.46"/>`;

const wasteBase5 = `
  <rect width="32" height="32" fill="#8a8065"/>
  <ellipse cx="19" cy="6" rx="11" ry="6" fill="#7e755d" opacity="0.42"/>
  <ellipse cx="5" cy="27" rx="9" ry="6" fill="#938872" opacity="0.44"/>
  <path d="M7 11 L12 14 M21 16 L26 19 M13 29 L18 31 M28 24 L32 26"
        stroke="#6a6047" stroke-width="1.05" opacity="0.44" stroke-linecap="round"/>
  <path d="M3 4 q1.5-4 3 0M16 20 q1.5-4 3 0M25 30 q1.5-4 3 0"
        stroke="#b6a983" stroke-width="1.15" fill="none" stroke-linecap="round" opacity="0.55"/>
  <circle cx="24" cy="10" r="1" fill="#6f6650" opacity="0.5"/>
  <circle cx="9" cy="21" r="0.9" fill="#6f6650" opacity="0.45"/>`;

const WEST_TILES = {
  // 들판 바닥
  waste: `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">${wasteBase}</svg>`,
  waste2: `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">${wasteBase2}</svg>`,

  // 밟고 지나가는 것들 (0.67) — 제 밑색을 가진 통짜 그림이다.
  // (들판의 잔돌·덤불과 달리 '얹는 그림'이 아니다. 밑에 풀이 깔리면 안 되니까)
  waste_rubble: `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">
    <ellipse cx="12" cy="19" rx="5" ry="3.2" fill="#000" opacity="0.16"/>
    <ellipse cx="11.6" cy="18" rx="4.6" ry="3" fill="#6f6754"/>
    <ellipse cx="10.6" cy="17" rx="2.6" ry="1.5" fill="#8d8570"/>
    <ellipse cx="22" cy="24" rx="3.4" ry="2.2" fill="#000" opacity="0.14"/>
    <ellipse cx="21.7" cy="23.2" rx="3" ry="2" fill="#645d4c"/>
    <ellipse cx="21" cy="22.6" rx="1.7" ry="1" fill="#847c68"/>
    <ellipse cx="24.5" cy="11" rx="2.2" ry="1.4" fill="#6f6754" opacity="0.9"/>
    <ellipse cx="6" cy="26" rx="1.8" ry="1.2" fill="#6f6754" opacity="0.8"/>
  </svg>`,

  waste_bones: `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">
    <ellipse cx="17" cy="23" rx="9" ry="3" fill="#000" opacity="0.13"/>
    <g fill="#ded3b8">
      <rect x="8" y="19.4" width="15" height="2.6" rx="1.3" transform="rotate(-7 15 21)"/>
      <circle cx="8.4" cy="19.2" r="2"/><circle cx="8.4" cy="22" r="1.9"/>
      <circle cx="23" cy="17.6" r="1.9"/><circle cx="23" cy="20.4" r="1.8"/>
    </g>
    <g fill="#c3b795">
      <rect x="12" y="24.4" width="10" height="2" rx="1" transform="rotate(9 17 25)"/>
      <circle cx="12.2" cy="24.2" r="1.4"/><circle cx="22" cy="26.2" r="1.4"/>
    </g>
    <path d="M13 12 q3 -3 6 0 q1 4 -3 5 q-4 -1 -3 -5z" fill="#ded3b8" opacity="0.9"/>
    <circle cx="15" cy="13.4" r="1.1" fill="#5f5744"/><circle cx="18" cy="13.4" r="1.1" fill="#5f5744"/>
  </svg>`,
  waste3: `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">${wasteBase3}</svg>`,
  waste4: `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">${wasteBase4}</svg>`,
  waste5: `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">${wasteBase5}</svg>`,

  // 낭떠러지 — 지나갈 수 없다. 아래로 갈수록 어두워져 깊이가 보인다.
  // 낭떠러지 (0.67).
  //
  // ⚠ 예전에는 위가 밝고 아래가 검은 그라데이션이었다. 그런데 낭떠러지는
  //   **세 줄 두께로 깔린다.** 그러면 밝음→어두움이 세 번 되풀이돼서
  //   깊이가 아니라 **가로 줄무늬 세 개**가 보였다.
  //   깊이는 한 칸이 아니라 여러 칸이 모여 만드는 것이므로, 낱장은 평평하게
  //   두고 밝은 윗선은 절벽 **끝**(cliff_edge)에만 남긴다.
  cliff: `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">
    <rect width="32" height="32" fill="#24211b"/>
    <path d="M0 0 L9 0 L11 32 L2 32 Z" fill="#2b2821"/>
    <path d="M13 0 L22 0 L20 32 L11 32 Z" fill="#201d18"/>
    <path d="M24 0 L32 0 L32 32 L22 32 Z" fill="#2a2720"/>
    <path d="M5 2 L7 30 M15 1 L14 31 M26 3 L25 29"
          stroke="#0d0c0a" stroke-width="1.5" opacity="0.7"/>
    <path d="M9 6 L10 18 M20 9 L19 21 M29 14 L28 26"
          stroke="#4a4437" stroke-width="1" opacity="0.45"/>
    <circle cx="17" cy="12" r="1.1" fill="#3a362c" opacity="0.7"/>
    <circle cx="7" cy="24" r="0.9" fill="#3a362c" opacity="0.6"/>
  </svg>`,
