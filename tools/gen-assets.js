...
const TILES = {
  ...
  // 0.70.19 — 길을 **네 장**으로 늘렸다.
  //
  // 왜: 길은 곧게 뻗을수록 반복이 잘 보인다. 두 장이면 한 칸 걸러 같은 그림이라
  // 눈이 그 주기를 금방 잡아낸다. 넓은 변화는 한 칸 안의 얼룩이 아니라
  // **장수**로 만든다(0.70.18 에 얼룩으로 해 보고 바둑판이 되어 되돌린 것).
  path3: `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">
    <rect width="32" height="32" fill="#c0a072"/>
    <path d="M0 13 q7 -3 15 0 t17 -1" stroke="#ab8a5c" stroke-width="2.4" fill="none" opacity="0.32"/>
    <path d="M0 28 q10 3 18 0 t14 -1" stroke="#ab8a5c" stroke-width="2.1" fill="none" opacity="0.28"/>
    <ellipse cx="20" cy="8" rx="2.7" ry="1.7" fill="#a8875a" opacity="0.46"/>
    <ellipse cx="6" cy="19" rx="2.3" ry="1.5" fill="#a8875a" opacity="0.42"/>
    <ellipse cx="28" cy="22" rx="2" ry="1.3" fill="#a8875a" opacity="0.38"/>
    <circle cx="11" cy="4" r="1.05" fill="#e3cda6" opacity="0.7"/>
    <circle cx="24" cy="30" r="0.95" fill="#e3cda6" opacity="0.6"/>
  </svg>`,

  path4: `<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 32 32">
    <rect width="32" height="32" fill="#bfa070"/>
    <path d="M0 4 q8 3 16 0 t16 1" stroke="#a8875a" stroke-width="2" fill="none" opacity="0.26"/>
    <path d="M0 17 q9 -3 17 0 t15 1" stroke="#b08e60" stroke-width="2.7" fill="none" opacity="0.33"/>
    <ellipse cx="14" cy="24" rx="3" ry="1.9" fill="#a8875a" opacity="0.44"/>
    <ellipse cx="27" cy="12" rx="2.2" ry="1.4" fill="#a8875a" opacity="0.4"/>
    <circle cx="4" cy="9" r="1" fill="#e3cda6" opacity="0.65"/>
    <circle cx="19" cy="20" r="0.9" fill="#e3cda6" opacity="0.55"/>
    <circle cx="30" cy="27" r="0.85" fill="#e3cda6" opacity="0.5"/>
  </svg>`,
...
// ---------- 전투 배경 640x480 ----------
/**
 * 전투 배경의 땅에 깔 **풀 결** (0.70.18).
 *
 * 매끈한 초록 판은 잔디가 아니라 천으로 보인다. 짧은 획을 흩뿌리되
 * **아래로 갈수록 크고 성기게** 한다 — 가까운 것이 크게 보이는 것이 원근이다.
 * (이것은 이어 붙이는 타일이 아니라 한 장짜리 배경이라, 여기서는 쏠린 변화를 넣어도 된다)
 */
function grassField() {
  const rnd = seeded(0x5eed1eaf);
  const out = [];
  for (let i = 0; i < 260; i++) {
    const t = rnd();                 // 0 = 먼 곳, 1 = 가까운 곳
    const y = 305 + t * 170;
    const x = rnd() * 650 - 5;
    const h = 3 + t * 9;             // 가까울수록 길다
    const w = 1 + t * 1.4;
    const c = ['#4f8a58', '#5f9e63', '#3b6b46'][Math.floor(rnd() * 3) % 3];
    const op = (0.18 + rnd() * 0.3).toFixed(2);
    out.push(`<path d="M${x.toFixed(1)} ${y.toFixed(1)}q${(w).toFixed(1)} ${(-h).toFixed(1)} ${(w * 2).toFixed(1)} 0" stroke="${c}" stroke-width="${w.toFixed(1)}" fill="none" opacity="${op}" stroke-linecap="round"/>`);
  }
  return out.join('');
}
...
const BATTLE_BG = `<svg xmlns="http://www.w3.org/2000/svg" width="640" height="480" viewBox="0 0 640 480">
  <defs>
    ...
    <linearGradient id="haze" x1="0" y1="0" x2="0" y2="1">
      <stop offset="0" stop-color="#8fa8c8" stop-opacity="0"/>
      <stop offset="0.5" stop-color="#a8bcd6" stop-opacity="0.22"/>
      <stop offset="1" stop-color="#8fa8c8" stop-opacity="0"/>
    </linearGradient>
  ...
  ${/* 0.70.18 — 배경에 **거리감**을 넣는다.
       ① 먼 산 앞의 옅은 안개 — 멀수록 대비가 낮아진다. 이게 없으면 산과 땅이
          같은 거리에 있는 색종이 두 장으로 보인다.
       ② 땅에 잔디 결 — 매끈한 초록 판이 아니라 풀이 난 땅으로 읽히게.
       ③ 아래쪽 어둠 — 서 있는 사람이 배경에서 떠오르게(BattleScene 이 그 위에 그린다).
       ⚠ 아래 세 몫은 **위 3분의 2**에만 넣는다. 아래 3분의 1 은 싸우는 사람이
         서는 자리라 어지러우면 안 된다(주문서의 전투 배경 규칙과 같다). */ ''}
  ${/* ⚠ 안개는 **네모로 넣으면 안 된다.** 처음에 납작한 사각형 두 장으로 넣었더니
       산허리를 가로지르는 **띠**가 되어 오히려 더 그림 같아졌다.
       안개는 경계가 없어야 안개다 — 위아래로 사라지는 그라데이션으로 넣는다. */ ''}
  <rect y="235" width="640" height="90" fill="url(#haze)"/>
  ${grassField()}
  <rect y="300" width="640" height="180" fill="#000" opacity="0.12"/>
  <ellipse cx="320" cy="470" rx="520" ry="120" fill="#000" opacity="0.18"/>
</svg>`;
...
//
// ════════════════════════════════════════════════════════════
//  땅의 결 (0.70.18) — 평평한 색을 **재료**로 바꾼다
// ════════════════════════════════════════════════════════════
//
// ── 무엇이 밋밋했나 ────────────────────────────────────────
// 바닥 타일은 「평평한 밑색 + 큰 얼룩 두어 개 + 풀포기 몇 획」이었다.
// 한 장만 보면 깔끔한데, 화면을 가득 채우면 **색종이를 깔아 놓은 것**처럼 보인다.
// 진짜 흙·돌·재는 눈에 잘 안 보이는 **잔 알갱이**가 촘촘히 박혀 있고, 사람 눈은
// 그 고운 결로 재료를 알아본다. 큰 얼룩만으로는 그 자리가 안 채워진다.
//
// ── 어떻게 고치나 ──────────────────────────────────────────
// 타일마다 알갱이를 수백 개 흩뿌린다. 손으로 찍지 않고 **씨앗에서 뽑는다** —
// 같은 씨앗이면 언제 다시 구워도 같은 그림이 나오므로 다시 구워도 화면이 안 변한다.
//
// ⚠ **이어 붙는 것이 깨지면 안 된다.** 알갱이가 칸 경계에 걸치면 반쪽만 남아
//   경계에 점선이 생긴다. 그래서 가장자리에 걸친 알갱이는 **반대편에도 한 번 더**
//   찍는다(아래 wrap). 그러면 두 반쪽이 옆 칸에서 하나로 이어진다.
//   "가장자리를 피한다" 로는 안 된다 — 피하면 칸마다 테두리만 말끔해져서
//   그 자체가 격자로 보인다(0.65 에 겪은 것과 같은 함정이다).
//
// ⚠ **한쪽으로 쏠린 밝기는 절대 넣지 않는다.** 알갱이는 고르게 흩뿌린다.
//   위가 밝고 아래가 어두우면 깔았을 때 가로줄이 생긴다(이 파일 첫머리 참고).

/** 씨앗 하나로 굴러가는 난수. 같은 씨앗 = 같은 그림. */
 ...
 * 알갱이 한 겹.
 *
 * @param {number} seed   씨앗. 타일마다 다르게 준다(같으면 같은 무늬가 반복된다)
 * @param {object} o
 * @param {number} o.n      개수
 * @param {string[]} o.fill 색 후보. 밑색보다 조금 밝은 것과 어두운 것을 섞는다
 * @param {number} o.r      알갱이 반지름의 위쪽 한계
 * @param {number} o.a      진하기
 * @param {number} [o.size] 타일 한 변(기본 32)
 */
function grain(seed, { n, fill, r = 0.9, a = 0.18, size = 32 }) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const y = rnd() * size;
    const rr = 0.25 + rnd() * r;
    const c = fill[Math.floor(rnd() * fill.length) % fill.length];
    const op = (a * (0.45 + rnd() * 0.85)).toFixed(3);
    // 가장자리에 걸치면 반대편에도 찍어 이어 붙게 한다.
    const xs = [x];
    const ys = [y];
    if (x < rr) xs.push(x + size);
    if (x > size - rr) xs.push(x - size);
    if (y < rr) ys.push(y + size);
    if (y > size - rr) ys.push(y - size);
    for (const px of xs) for (const py of ys) {
      out.push(`<circle cx="${px.toFixed(2)}" cy="${py.toFixed(2)}" r="${rr.toFixed(2)}" fill="${c}" opacity="${op}"/>`);
    }
  }
  return `<g>${out.join('')}</g>`;
}
 ...
 * 어떤 바닥에 어떤 결을 넣나.
 *
 * 값을 고르는 규칙 하나 — **거친 재료일수록 알갱이가 크고 성기다.**
 * 잔디는 촘촘하고 곱게, 자갈길은 굵고 드문드문, 재는 가루처럼 아주 곱게.
 */
const GRAIN = {
  grass:           { n: 270, r: 0.9,  a: 0.26, fill: ['#3f9642', '#5fbc5c', '#2f7f38', '#69c866', '#7ad277', '#276f31'] },
  grass2:          { n: 270, r: 0.9,  a: 0.26, fill: ['#3f9642', '#5fbc5c', '#2f7f38', '#69c866', '#7ad277', '#276f31'] },
  grass3:          { n: 270, r: 0.9,  a: 0.26, fill: ['#3f9642', '#5fbc5c', '#2f7f38', '#69c866', '#7ad277', '#276f31'] },
  grass4:          { n: 270, r: 0.9,  a: 0.26, fill: ['#3f9642', '#5fbc5c', '#2f7f38', '#69c866', '#7ad277', '#276f31'] },
  path:            { n: 240, r: 1.25, a: 0.26, fill: ['#8a6a45', '#a3824f', '#6b4a2a', '#c2a172'] },
  path2:           { n: 240, r: 1.25, a: 0.26, fill: ['#8a6a45', '#a3824f', '#6b4a2a', '#c2a172'] },
  path3:           { n: 240, r: 1.25, a: 0.26, fill: ['#8a6a45', '#a3824f', '#6b4a2a', '#c2a172'] },
  path4:           { n: 240, r: 1.25, a: 0.26, fill: ['#8a6a45', '#a3824f', '#6b4a2a', '#c2a172'] },
  brick:           { n: 150, r: 0.8,  a: 0.16, fill: ['#8f97ab', '#b7bfd0', '#6d7386'] },
  brick2:          { n: 150, r: 0.8,  a: 0.16, fill: ['#8f97ab', '#b7bfd0', '#6d7386'] },
  castle_floor:    { n: 130, r: 0.75, a: 0.13, fill: ['#b7bfd0', '#8f97ab', '#d5dbe6'] },
  castle_floor2:   { n: 130, r: 0.75, a: 0.13, fill: ['#b7bfd0', '#8f97ab', '#d5dbe6'] },
  castle_wall:     { n: 120, r: 0.75, a: 0.12, fill: ['#b7bfd0', '#8f97ab'] },
  dungeon_floor:   { n: 200, r: 1.0,  a: 0.22, fill: ['#3a4048', '#555d68', '#272c33'] },
  dungeon_floor2:  { n: 200, r: 1.0,  a: 0.22, fill: ['#3a4048', '#555d68', '#272c33'] },
  dungeon_wall:    { n: 170, r: 0.95, a: 0.20, fill: ['#3a4048', '#555d68', '#272c33'] },
  room_floor:      { n: 160, r: 0.9,  a: 0.15, fill: ['#8b5a2b', '#a8875a', '#6b4423'] },
  room_wall:       { n: 120, r: 0.8,  a: 0.12, fill: ['#c9a06a', '#a8875a'] },
  house_wall:      { n: 120, r: 0.8,  a: 0.13, fill: ['#c9a06a', '#a8875a', '#8d6438'] },
  // 화산 — 재는 가루처럼 곱고, 현무암은 거칠다.
  ash:             { n: 260, r: 0.7,  a: 0.22, fill: ['#6f6650', '#8a8270', '#4e4738'] },
  ash2:            { n: 260, r: 0.7,  a: 0.22, fill: ['#6f6650', '#8a8270', '#4e4738'] },
  ash3:            { n: 260, r: 0.7,  a: 0.22, fill: ['#6f6650', '#8a8270', '#4e4738'] },
  ash_path:        { n: 260, r: 0.7,  a: 0.22, fill: ['#6f6650', '#8a8270', '#4e4738'] },
  ash_path2:       { n: 260, r: 0.7,  a: 0.22, fill: ['#6f6650', '#8a8270', '#4e4738'] },
  charred:         { n: 210, r: 0.9,  a: 0.24, fill: ['#2a2620', '#453e33'] },
  basalt:          { n: 180, r: 1.15, a: 0.24, fill: ['#241c17', '#3a2a20', '#55483c'] },
  // 서쪽 황무지 — 마른 흙
  waste:           { n: 230, r: 1.1,  a: 0.24, fill: ['#8a8270', '#a89a80', '#6f6650'] },
  waste2:          { n: 230, r: 1.1,  a: 0.24, fill: ['#8a8270', '#a89a80', '#6f6650'] },
  waste3:          { n: 230, r: 1.1,  a: 0.24, fill: ['#8a8270', '#a89a80', '#6f6650'] },
  waste4:          { n: 230, r: 1.1,  a: 0.24, fill: ['#8a8270', '#a89a80', '#6f6650'] },
  waste5:          { n: 230, r: 1.1,  a: 0.24, fill: ['#8a8270', '#a89a80', '#6f6650'] },
  // 물은 알갱이가 아니라 **잔물결**이다 — 아주 옅게, 밝은 쪽으로만.
  water:           { n: 90,  r: 0.7,  a: 0.14, fill: ['#9fdcf7', '#dff4ff'] },
  water2:          { n: 90,  r: 0.7,  a: 0.14, fill: ['#9fdcf7', '#dff4ff'] },
};

/** 씨앗을 이름에서 뽑는다 — 타일마다 다른 무늬가 나오도록. */
...
/**
 * **큰 얼룩** 한 겹 — 잔 알갱이와 다른 **굵기**의 변화다.
 *
 * 왜 두 겹인가: 진짜 땅은 두 가지 크기로 변한다. 가까이 보면 알갱이,
 * 멀리 보면 넓게 밝고 어두운 자리. 한 가지 크기만 넣으면 사포처럼 보인다 —
 * 곱기만 하고 **깊이가 없다.** 넓은 얼룩이 그 깊이를 만든다.
 *
 * 아주 옅게(0.05~0.09) 넣는다. 진하면 그것대로 무늬가 되어 칸이 다시 눈에 띈다.
 */
function mottle(seed, { n, fill, r = 9, a = 0.07, size = 32 }) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const y = rnd() * size;
    const rx = r * (0.5 + rnd());
    const ry = rx * (0.6 + rnd() * 0.6);
    const c = fill[Math.floor(rnd() * fill.length) % fill.length];
    const op = (a * (0.5 + rnd())).toFixed(3);
    // 알갱이와 같은 규칙 — 가장자리에 걸치면 반대편에도 찍어 이어 붙게 한다.
    for (const px of [x, x + size, x - size]) for (const py of [y, y + size, y - size]) {
      if (Math.abs(px - size / 2) > size * 1.2 || Math.abs(py - size / 2) > size * 1.2) continue;
      out.push(`<ellipse cx="${px.toFixed(2)}" cy="${py.toFixed(2)}" rx="${rx.toFixed(2)}" ry="${ry.toFixed(2)}" fill="${c}" opacity="${op}"/>`);
    }
  }
  return `<g>${out.join('')}</g>`;
}
...
/**
 * 큰 얼룩을 넣을 땅 — **지금은 비어 있다.**
 *
 * ⚠ 0.70.18 에 넣었다가 **도로 뺐다.** 겪은 것을 적어 둔다:
 *
 *   한 칸 안에 넓은 얼룩을 대여섯 개 넣으면, 한 장만 볼 때는 깊이가 생겨 좋다.
 *   그런데 40×32 칸으로 깔면 그 얼룩 배치가 **칸마다 똑같이 되풀이되어**
 *   화면에 큼직한 바둑판 무늬가 그려진다. 이 파일 첫머리에 적어 둔
 *   "쏠린 밝기가 격자를 만든다" 와 같은 함정이다 — 이번엔 밝기가 아니라
 *   **무늬의 되풀이**였을 뿐이다.
 *
 *   잔 알갱이(grain)는 괜찮다. 알갱이 하나하나가 너무 작아서 사람 눈이
 *   그 배치를 기억하지 못하기 때문이다. 눈에 잡히는 크기의 것은 안 된다.
 *
 *   **넓은 변화는 한 칸 안에서 만들 것이 아니라 여러 장으로 만든다.**
 *   풀이 네 장인 것이 그래서다(FieldScene 이 자리에 따라 골라 깐다).
 *   길·황무지·재가 밋밋하다면 얼룩을 넣을 것이 아니라 변형을 늘려야 한다.
 */
const MOTTLE = {};

/**
 * 이 타일에 결을 얹는다. 표에 없으면 그대로 돌려준다.
 *
 * 순서가 중요하다 — **큰 얼룩을 먼저, 잔 알갱이를 나중에.** 거꾸로 하면
 * 넓은 얼룩이 알갱이를 덮어 다시 밋밋해진다.
 */
function withGrain(name, svg) {
  const g = GRAIN[name];
  const m = MOTTLE[name];
  if ((!g && !m) || typeof svg !== 'string') return svg;
  const at = svg.lastIndexOf('</svg>');
  if (at < 0) return svg;
  const seed = nameSeed(name);
  const layers = (m ? mottle(seed, m) : '') + (g ? grain(seed, g) : '');
  return svg.slice(0, at) + layers + svg.slice(at);
}

// 큰 나무는 제 칸 밖으로 자란다 — 위로 두 칸 넘게, 옆으로 반 칸씩.
// 여기 적어 두지 않으면 아래 루프가 32×32 로 눌러 담아 납작한 덤불이 된다.
/**
 * 칸보다 **크게 굽는 물건** (0.70.21).
 *
 * ── 무엇이 작아 보였나 ────────────────────────────────────
 * 사람은 48×64 로 그려진다 — 가로 한 칸 반, 세로 두 칸이다.
 * 그런데 침대·탁자·창문은 32×32 한 칸에 갇혀 있었다. 사람 옆에 놓으면
 * **침대가 사람 상반신만 하다.** 여관에 들어서면 그게 제일 먼저 눈에 띈다.
 *
 * ── 어떻게 키우나 ─────────────────────────────────────────
 * 그림을 다시 그리지 않는다. **같은 그림을 그대로 크게 굽는다** —
 * viewBox 는 0 0 32 32 그대로 두고 캔버스만 키우면 SVG 가 알아서 늘려 준다.
 * 가로세로를 같은 배로 늘리므로 **찌그러지지 않는다.**
 *
 * 그리고 화면에서는 **발밑을 칸 아래 변에 맞춰** 세운다(FieldScene 의 GROWN).
 * 그래서 커진 몫은 위와 옆으로 자라고, 놓인 자리는 그대로다.
 *
 * ⚠ **줄지어 놓는 것은 키우지 않는다.** 계산대(KKKKK)·양탄자·울타리·계단처럼
 *   여러 칸이 이어져 한 덩이로 보여야 하는 것은, 한 칸씩 키우면 서로 겹쳐
 *   이음매가 두 겹으로 진해진다. 홀로 놓이는 것만 키운다.
 * ⚠ 배를 너무 올리면 옆 칸을 침범한다. 1.7 배면 32 → 54 로, 양옆으로 11px 씩
 *   넘어간다. 그 정도가 "옆에 뭔가 있어도 안 어색한" 한계다.
 */
const GROW = {
  bed: 1.95,          // 사람이 누울 수 있어 보여야 한다 — 가장 크게
                      // (여관 침대는 한 칸 띄어 놓여 있어서 이 배까지는 서로 안 겹친다)
  table: 1.6,
  cauldron: 1.55,
  barrel: 1.3,
  shelf: 1.4,
  room_window: 1.45,  // 창은 벽에 난 구멍이다. 작으면 총안처럼 보인다
  forge_hearth: 1.5,
  forge_anvil: 1.4,
  stall_weapon: 1.4,
  stall_armor: 1.4,
  cliff: 1,           // (자리만 잡아 둔다 — 키우지 않는다)
};
delete GROW.cliff;
...
for (const [name, svg] of Object.entries({
  ...
  // 0.70.21 — 칸보다 크게 굽는 물건은 여기서 배를 먹인다(위 GROW).
  const grow = GROW[name] || 1;
  const w = big ? big.w : Math.round(32 * grow);
  const h = big ? big.h : Math.round(32 * grow);
  // 0.70.18 — 바닥에는 잔 알갱이를 얹어 재료로 보이게 한다(위 withGrain).
  JOBS.push({ svg: withGrain(name, svg), w, h, out: `tiles/${name}.png` });
}
...
const MONFX0 = require('./art-monfx.js'); // 걷는 자세를 만드는 데 쓴다(아래 MONFX 와 같은 파일)
for (const [name, svg] of Object.entries({ ...CLS.CLASS_SPRITES, admin: ADMIN_ART.ADMIN })) {
  ...
  // 0.70.17 — **걷는 두 장.** 필드에서만 쓰므로 필드 크기로만 굽는다(전투는 서 있다).
  //
  // 직업 셋은 팔·다리가 실제로 엇갈린 그림이고(art-class.js 의 CLASS_WALK),
  // 운영자는 부품으로 나뉜 그림이 아니라 몸 전체를 기울여 만든다(monfx.walkPose).
  const walkPair = CLS.CLASS_WALK[name]
    || [MONFX0.walkPose(svg, { lean: 3, sx: 1.02, sy: 0.98 }),
        MONFX0.walkPose(svg, { lean: -3, sx: 1.02, sy: 0.98 })];
  walkPair.forEach((w, i) => {
    JOBS.push({ svg: w, w: 96, h: 128, out: `sprites/characters/${name}_walk${i + 1}.png` });
  });
  // ⚠ 0.70.19 — **뒷모습은 굽지 않는다(보류).**
  //
  //   0.70.18 에 직업 셋의 뒷모습을 구워 봤는데, 머리만 뒤를 보고 **몸에 걸친 것은
  //   그대로 앞**이었다. 손에 든 검이 몸 앞에 떠 있고, 허리띠 버클이 등에 붙어 있고,
  //   투구는 앞면이 뒤통수에 붙었다.
  //
  //   까닭은 그림이 아니라 **얹는 구조**에 있다. 장비는 core/Appearance.js 가 기본
  //   그림 위에 파츠(투구·소매·견갑·장갑·신발·무기)를 얹어 만든다. 그 파츠들은
  //   앞에서 본 모양 한 벌뿐이라, 밑그림만 뒤로 바꿔도 위에 얹히는 것은 앞을 본다.
  //
  //   제대로 하려면 **파츠마다 뒷면 그림**이 있어야 하고(투구 뒤통수, 버클 없는
  //   허리띠 뒤, 등 뒤로 넘어간 무기), appearance.json 에 그 한 벌을 통째로 더해야 한다.
  //   밑그림 세 장으로 끝나는 일이 아니다. 그래서 그 한 벌이 생길 때까지 미룬다.
  //
  //   그릴 줄(art-class.js 의 CLASS_BACK)은 **남겨 둔다** — 다시 켤 때 쓸 것이고,
  //   무엇이 모자라서 껐는지도 거기 적혀 있다.
}
...
// 0.70.18 — 마을 사람도 걷는 두 장을 갖는다.
//
// 마을 사람 그림은 통짜라 팔다리를 따로 못 돌린다(몬스터와 같은 사정).
// 그래서 몸 전체가 눌렸다 펴지며 좌우로 기우는 두 장을 굽는다 —
// 제자리에서 몸을 뒤척이는 것만으로도 마을이 살아 보인다.
const NPC_GAIT = { lean: 3, sx: 1.03, sy: 0.97 };
const npcWalk = (name, svg) => {
  JOBS.push({ svg: MONFX0.walkPose(svg, NPC_GAIT), w: 96, h: 128,
              out: `sprites/npc/${name}_walk1.png` });
  JOBS.push({ svg: MONFX0.walkPose(svg, { lean: -NPC_GAIT.lean, sx: 2 - NPC_GAIT.sx, sy: 2 - NPC_GAIT.sy }),
              w: 96, h: 128, out: `sprites/npc/${name}_walk2.png` });
};
...
// 게시판과 이정표는 **사람이 아니다** — 걷는 그림을 만들지 않는다.
// 돌이 숨을 쉬면 그것대로 이상하다.
...
npcWalk('gate_merchant', DEEP.GATE_MERCHANT);
JOBS.push({ svg: WEST.WITCH, w: 96, h: 128, out: 'sprites/npc/witch.png' });
npcWalk('witch', WEST.WITCH);
...
for (const [name, svg] of Object.entries(MON)) {
  ...
  // 0.70.17 — 걷는 두 장. **필드 크기로만** 굽는다(전투에서는 제자리에 서 있다).
  const gait = MONFX.MON_WALK[name];
  if (gait) {
    JOBS.push({ svg: MONFX.walkPose(svg, gait), w: 128, h: 128,
                out: `sprites/monsters/${name}_walk1.png` });
    JOBS.push({ svg: MONFX.walkPose(svg, { ...gait, lean: -gait.lean, sy: 2 - gait.sy, sx: 2 - gait.sx }),
                w: 128, h: 128, out: `sprites/monsters/${name}_walk2.png` });
  }
}
