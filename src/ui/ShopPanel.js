 ...
 *
 * 0.60 — 물건마다 다르게 둘 수 있다. items.json 의 `bulk` 에 적으면 그것을 쓴다
 * (시트에서는 '기타' 칸의 JSON). 안 적으면 아래 기본값.
 *
 * 왜 물건마다 다른가: 마력석은 룬 장비 하나를 강화하는 데 수십 개가 든다.
 * 10개씩 사면 열 번을 눌러야 한다 — 그 물건에는 100개 단추가 있어야 한다.
 * 반대로 회복약에 100개 단추를 붙이면 실수로 골드를 통째로 날린다.
 */
const BULK_BUY = [1, 10];
...
function bulkOf(def) {
  ...
  const list = Array.isArray(def.bulk) ? def.bulk.filter((n) => Number(n) >= 1) : null;
  return list && list.length ? list.map((n) => Math.floor(n)) : BULK_BUY;
}
...
export class ShopPanel {
  ...
  _buyRow(state, { id, def, price }) {
    ...
    // 뭉치로 쌓이는 물건(물약·재료)은 묶음으로도 살 수 있게 한다.
    const qtys = bulkOf(def);
