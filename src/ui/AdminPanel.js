...
export class AdminPanel {
  ...
  _rankHtml(bosses) {
    ...
    // 0.70.18 — 지금 서 있는 땅에 보스가 있나. 있으면 '리젠' 단추를 띄운다.
    const here = this.store.state.map || {};
    const hereBossId = here.boss || (here.timedBoss && here.timedBoss.monster) || null;
    const hereBoss = hereBossId ? (this.store.state.db.monsters[hereBossId] || null) : null;
    const hereAlive = (this.store.state.monsters || []).some((m) => m.isBoss && m.alive);

    return `
      <div class="admin-form">
        ${/* ── 리젠 (0.70.18) ────────────────────────────────
             보스를 고쳐 놓고 실제로 어떤지 보려면 다시 만나야 하는데,
             지하 5층 주인은 한 시간을, 고룡은 서른 분을 기다려야 한다.
             기다리는 동안에는 고친 것을 못 본다. 그래서 **선 자리의 보스를**
             바로 부른다 — 어느 보스를 부를지 고르게 하지 않는다.
             부를 곳까지 걸어가는 것 자체가 "지금 여기를 본다" 는 뜻이기 때문이다. */ ''}
        ${/* ⚠ **시즌 상자(admin-seasonbox)와 같은 이름을 쓰지 않는다.**
             처음에 그 이름을 그대로 썼더니, 시즌을 보는 시험(seasonlock)이
             '첫 번째 admin-seasonbox' 를 읽다가 이 상자를 집어서 엉뚱한 글을 봤다.
             생김새가 같다고 이름까지 같이 쓰면, 이름으로 찾는 쪽이 전부 헷갈린다. */ ''}
        <div class="admin-regenbox">
          <div class="admin-seasonbox-head">
            <span class="admin-seasonnow">지금 있는 곳 <b>${escapeHtml(here.name || '?')}</b></span>
          </div>
          ${hereBoss
            ? `<p class="admin-note">
                 이 땅의 보스는 <b>${escapeHtml(hereBoss.name)}</b> 입니다.
                 ${hereAlive
                   ? '지금 <b>서 있습니다</b> — 눌러도 달라지는 것이 없습니다.'
                   : '지금 <b>없습니다</b>. 눌러서 바로 부를 수 있습니다.'}
               </p>
               <button class="primary-btn" type="button" data-boss-now ${this.busy ? 'disabled' : ''}>
                 ${escapeHtml(hereBoss.name)} 리젠
               </button>`
            : '<p class="admin-note">이 땅에는 보스가 없습니다. 보스가 있는 땅으로 가서 누르세요.</p>'}
        </div>
  ...
  _bind() {
    ...
    // 0.70.18 — 선 자리의 보스를 바로 부른다. 서버에 물어볼 것이 없는 일이라
    // busy 로 잠그지 않는다(누르면 그 프레임에 끝난다).
    const bossNow = q('[data-boss-now]');
    if (bossNow) bossNow.addEventListener('click', () => this.bus.emit('admin:boss-now'));
