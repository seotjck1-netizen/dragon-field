...
import { captureScroll, restoreScroll, setHtml } from './keepScroll.js';
...
/** 받침이 있으면 '을', 없으면 '를'. (AffixSystem 의 josa 와 같은 규칙) */
...
export class InventoryPanel {
  ...
  _render() {
    ...
    /** 홈 뚫을 물건을 들고 있나 — 머리말에 한 줄 덧붙일지 정한다. */
    const hasDrill = countOf(state, DRILL_ITEM) > 0;
    ...
    setHtml(this.root, `
      <div class="inv-panel">
        <header class="inv-header">
          <h2>${this.title}</h2>
          ${/* 송곳을 들고 있을 때만 홈 뚫기를 덧붙인다 — 없는 사람에게는 군말이다.
                두 대장간에 같은 말을 붙인다. 0.70.16 부터 양쪽에서 다 뚫린다. */ ''}
          ${this.mode === 'forge'
            ? `<span class="inv-hint">강화할 장비를 고르세요${
                hasDrill ? ' · 홈 뚫기는 입고 있지 않아도 됩니다' : ''}</span>`
            : ''}
          ${this.mode === 'transcend'
            ? `<span class="inv-hint">+${BALANCE.ENHANCE_MAX} 장비를 고르세요 — 보석으로 +${BALANCE.TRANSCEND_MAX}까지${
                hasDrill ? ' · 홈 뚫기도 여기서 됩니다' : ''}</span>`
            : ''}
    ...
    for (const sec of this._sections(state, sorted)) {
      if (sec.title) {
        ...
        setHtml(head, `<span>${sec.title}</span><i>${sec.rows.length}</i>`);
    ...
    if (!sorted.length) {
      ...
      setHtml(list, `<li class="muted inv-empty">${
        this.category === 'all'
  ...
  _renderDetail(state) {
    ...
    const inst = this.selectedUid
      ...
      : null;
    if (!inst) {
      setHtml(box, '<p class="muted">아이템을 선택하세요.</p>');
      return;
    }
    ...
    // ── 홈 뚫기 ────────────────────────────────────────────
    //
    // 0.70.16 — **고른 장비를 뚫는다. 입고 있을 필요가 없다.**
    //
    // 규칙(AffixSystem.canDrill)은 처음부터 입고 있는지를 묻지 않았다. 막고 있던 것은
    // 화면이었다:
    //   ① **왕실 대장간(transcend)에는 이 칸이 아예 없었다.** +10 넘은 장비를 들고 가는
    //      곳이 거기라, 거기서 안 되면 "낀 것만 되는구나" 로 읽힌다.
    //   ② 뚫을 수 없을 때 **말없이 사라졌다**(sockets > 0 일 때만 까닭을 적었다).
    //      +10 이 안 된 장비를 고르면 칸이 통째로 없어져, 왜 안 되는지 알 길이 없었다.
    //
    // 그래서 이제 **두 대장간 모두**에서, 송곳을 들고 장비를 고르기만 하면
    // 뚫든지 **왜 못 뚫는지 적든지** 둘 중 하나는 반드시 보인다.
    let drillHtml = '';
    if (forge || witch) {
      const have = countOf(state, DRILL_ITEM);
      const d = canDrill(state, inst.uid);
      const drillName = (state.db.items[DRILL_ITEM] || {}).name || DRILL_ITEM;
      if (d.ok) {
        drillHtml = `<div class="enhance-box">
          <div class="enhance-row">${drillName}
            <b>${have}</b>개 — 이 장비에 <b>홈을 하나 더</b> 만든다</div>
          <div class="enhance-row muted">입고 있지 않아도 됩니다 — 고른 장비에 바로 뚫습니다.</div>
          <button class="btn-enhance" data-drill>홈 뚫기 (${sockets} → ${Math.min(MAX_SOCKETS, sockets + 1)})</button>
        </div>`;
      } else if (have > 0 && def.slot) {
        // 장비를 골랐고 송곳도 있는데 안 되는 자리 — 까닭을 반드시 적는다.
    ...
    if (sockets > 0) {
      ...
      if (this.mode === 'bag') {
        ...
        // ── 홈 고르기 (0.70.14) ────────────────────────────────
        //
        // 이제 **이미 박힌 홈에 덮어 박을 수 있다.** 그래서 "어느 홈에 박을까"를
        // 사람이 고를 수 있어야 한다. 고르지 않으면 예전처럼 빈 홈에 들어간다.
        //
        // ⚠ 덮어 박으면 그 자리 보석은 사라진다(빼서 돌려받는 것이 아니다).
        //   그래서 고른 홈에 무엇이 있는지 이름을 대고, 확인 문구에도 그 이름을 적는다.
        const pickedRaw = this._gemSlot;
        const picked = Number.isInteger(pickedRaw) && pickedRaw < sockets ? pickedRaw : null;
        const nameOfGem = (id) => (id ? ((state.db.items[id] || {}).name || id) : null);
        const slotRow = `<div class="gem-slots">${Array.from({ length: sockets }, (_, i) => {
          const has = nameOfGem(gems[i]);
          const on = picked === i;
          return `<button data-gemslot="${i}" class="${on ? 'is-armed' : ''}"
            title="${has ? `${has} — 덮어 박으면 사라집니다` : '빈 홈'}">${i + 1}번 · ${
              has || '빈 칸'
            }</button>`;
        }).join('')}</div>`;

        const target = picked != null ? picked : gems.findIndex((g) => !g);
        const willLose = target >= 0 ? nameOfGem(gems[target]) : null;
        const headline = willLose
          ? `<b>${target + 1}번 홈</b>의 ${willLose}${josaEul(willLose)} 지우고 새로 박습니다`
          : `빈 홈 <b>${freeSlots}</b>개 — 박으면 뺄 수 없습니다`;

        gemHtml = owned.length
          ? `<div class="enhance-box">
               <div class="enhance-row">${headline}</div>
               ${sockets > 1 || willLose ? slotRow : ''}
               <div class="gem-pick">${owned
                 .map(({ g, have }) => {
                   const ok = canSocket(state, inst.uid, g.id, picked).ok;
                   // 한 번 더 눌러야 박힌다. 강화가 +10 이 되는 순간 강화 단추 자리에
                   // 보석 단추가 올라와서, 이어 누른 손이 그대로 보석을 박아 버렸다.
                   const arm = this._gemArm === g.id;
                   const ask = willLose
                     ? `정말 ${willLose}${josaEul(willLose)} 지울까요? — ${g.name}`
                     : `정말 박을까요? — ${g.name}`;
                   return `<button data-gem="${g.id}" ${ok ? '' : 'disabled'}
                     class="${arm ? 'is-armed' : ''}"
                     title="${g.name} — ${g.effect} (한 번 더 누르면 박습니다)">${
                       arm ? ask : `${g.name} · <b>${g.effect}</b>`
                     } <span class="muted">×${have}</span></button>`;
                 })
                 .join('')}</div>
             </div>`
          : `<div class="enhance-box"><p class="muted">
               가진 보석이 없습니다. 보석은 <b>성 안 지하감옥</b>에서 나오고,
               <b>마을 도박 노인</b>에게서 바꾸거나 살 수 있습니다.
             </p></div>`;
      }
    }
    ...
    for (const btn of box.querySelectorAll('[data-gem]')) {
      btn.addEventListener('click', () => {
        ...
        clearTimeout(this._gemArmT);
        this._gemArm = null;
        const slot = Number.isInteger(this._gemSlot) ? this._gemSlot : null;
        this._gemSlot = null;
        this.bus.emit('ui:socket', { uid: inst.uid, gemId, slot });
      });
    }
    for (const btn of box.querySelectorAll('[data-gemslot]')) {
      btn.addEventListener('click', () => {
        const i = Number(btn.dataset.gemslot);
        // 같은 홈을 다시 누르면 고른 것이 풀린다(빈 홈 찾기로 돌아간다).
        this._gemSlot = this._gemSlot === i ? null : i;
        this._gemArm = null; // 홈을 바꿨으면 "정말?" 도 처음부터
        this.render();
      });
    }
