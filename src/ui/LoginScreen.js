...
import { RESERVED_HINT } from '../systems/ReservedId.js';
  ...
  _render() {
    ...
    this.root.innerHTML = `
      <div class="login-card">
        ...
        <form class="login-form" data-form>
          ...
          ${isLogin ? '' : `<p class="login-hint">${RESERVED_HINT}</p>`}
          <label>
            <span>비밀번호</span>
          ...
          ${
            isLogin
              ...
              : `<label>
                   <span>캐릭터 이름</span>
                   <input name="nick" maxlength="12" placeholder="남들에게 보이는 이름 (비우면 아이디)" />
                 </label>
                 <p class="login-hint">머리 위·랭킹·공지에 나오는 이름입니다.
                   아이디는 남에게 보이지 않습니다.</p>
                 <div class="class-pick">
                   <span>직업</span>
                   <div class="class-row">${this._classButtons()}</div>
                   <div class="class-detail" data-class-detail>${this._classDetail()}</div>
                 </div>`
          }
          ${
    ...
    for (const btn of this.root.querySelectorAll('[data-class]')) {
      btn.addEventListener('click', () => {
        ...
        // 선택 표시와 **아래 설명만** 바꾼다 (0.70.5).
        for (const other of this.root.querySelectorAll('[data-class]')) {
          other.classList.toggle('is-on', other.dataset.class === this.classId);
        }
        const detail = this.root.querySelector('[data-class-detail]');
        if (detail) detail.innerHTML = this._classDetail();
      });
    }
...
/**
 * 직업 고르기 — **그림과 이름만** (0.70.5).
 *
 * ── 왜 비웠나 ──────────────────────────────────────────────
 * 0.70.4 까지는 칸마다 그림·이름·한 줄 소개·능력치·패시브가 **다 들어 있었다.**
 * 셋을 나란히 놓으면 글자가 스무 줄이 넘어가 접속 화면이 빽빽해졌고,
 * 정작 "무엇을 고를까" 를 가르는 그림이 그 사이에 묻혔다.
 * 마법사는 패시브 설명이 특히 길어 그 칸만 아래로 늘어져 있었다.
 *
 * 그래서 칸에는 **그림과 이름만** 두고, 나머지는 아래 한 자리(class-detail)에
 * 고른 것만 편다. 고르기 전에는 무엇이 있는지 그림으로 훑고,
 * 고른 뒤에 그 하나를 자세히 읽는 순서가 된다.
 */
LoginScreen.prototype._classButtons = function () {
  ...
  return Object.entries(list)
    .map(([id, def]) => {
      ...
      // 마우스를 올리면 한 줄 소개는 여기서도 보인다(고르지 않아도).
      const tip = def.tagline || '';
 ...
 * 고른 직업의 설명 — 칸 아래 한 자리.
 *
 * 여기만 다시 그린다. 칸을 통째로 다시 그리면 이미 적어 둔 아이디·비밀번호가
 * 지워지므로(아래 [data-class] 이벤트 참고), 이 조각만 갈아 끼운다.
 */
LoginScreen.prototype._classDetail = function () {
  const list = (this.classes && this.classes.list) || {};
  const def = list[this.classId];
  if (!def) return '<p class="class-detail-none">위에서 직업을 고르세요.</p>';

  const stats = def.baseStats || {};
  const statLine = CLASS_STAT_ROWS
    .map(([icon, k]) => `<span>${icon} ${stats[k] ?? '-'}</span>`)
    .join('');
  const pas = def.passive;

  return `
    <p class="class-detail-name">${escapeHtml(def.name)}
      <i>${escapeHtml(def.tagline || '')}</i></p>
    <p class="class-detail-stats">${statLine}</p>
    ${pas
      ? `<p class="class-detail-passive">
