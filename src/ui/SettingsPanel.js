...
export class SettingsPanel {
  constructor({ bus, store, root }) {
    ...
    // ⚠ 예전에는 store 가 바뀔 때마다 무조건 다시 그렸다.
    //   체력이 차는 동안에는 그 알림이 **초당 스무 번** 온다. 설정 창은 체력을
    //   보여 주지도 않는데 초당 스무 번 통째로 새로 그려졌고, 그래서 값을 하나
    //   고치는 사이에도 창이 몇 번씩 갈렸다 — 스크롤이 튀는 것도, 적던 글자에서
    //   손이 튕겨 나가는 것도 여기서 왔다. 보이는 것이 그대로면 아예 안 그린다.
    const sub = subscribeRender(store, {
      isOpen: () => this.open,
      sig: () => this._sig(),
      render: () => this.render(),
    });
    this._resetSig = sub.reset;
  }

  /** 이 창이 보여 주는 것의 요약 — 설정값과, 다시 도전 칸이 쓰는 이름·직업. */
  _sig() {
    ...
    return [
      ...
      p.name || '', p.classId || '',
      this.restartOpen ? 1 : 0, this.restartClass || '',
    ].join('#');
  }
  ...
  show() {
    ...
    if (this._resetSig) this._resetSig();
    this.render();
