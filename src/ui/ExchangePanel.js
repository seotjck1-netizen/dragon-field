...
export class ExchangePanel {
  ...
  render() {
    ...
    if (!rows.length) {
      setHtml(list, '<li class="muted inv-empty">교환할 수 있는 것이 없습니다.</li>');
      return;
    }
  ...
  _row(recipe) {
    ...
    for (const g of recipe.give) {
      ...
      setHtml(label, `${g.def.name}<b>${g.have}/${g.count}</b>`);
    ...
    const label = document.createElement('span');
    setHtml(label, `${recipe.get.def.name}<b>×${recipe.get.count}</b>`);
    cell.appendChild(label);
