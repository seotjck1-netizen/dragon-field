...
export class Renderer {
   ...
   */
  /**
   * 미리 구워 둔 바닥 한 장을 통째로 붙인다 (0.70).
   *
   * 땅은 놀는 동안 변하지 않는다. 매 프레임 650번씩 다시 그리는 대신
   * 지도에 들어설 때 한 장으로 구워 두고 여기서 붙이기만 한다.
   * 보이는 곳만 잘라 붙이므로 판이 커도 값이 같다.
   */
  drawLayer(canvas) {
    if (!canvas) return;
    const sx = Math.max(0, Math.floor(this.camera.x));
    const sy = Math.max(0, Math.floor(this.camera.y));
    const sw = Math.min(canvas.width - sx, Math.ceil(this.width) + 2);
    const sh = Math.min(canvas.height - sy, Math.ceil(this.height) + 2);
    if (sw <= 0 || sh <= 0) return;
    this.ctx.drawImage(canvas, sx, sy, sw, sh, sx - this.camera.x, sy - this.camera.y, sw, sh);
  }

  /**
   * 다른 캔버스에 **카메라 없이** 그리는 손잡이 (0.70).
   *
   * 바닥 한 장을 구울 때 쓴다. 굽는 동안에는 화면이 아니라 판 좌표 그대로
   * 찍어야 하므로, 카메라를 0 으로 둔 그리기 도구를 잠깐 빌려 준다.
   *
   * ⚠ 그림 목록(assets)은 그대로 나눠 쓴다 — 두 벌로 만들면 같은 그림을
   *   두 번 읽게 되고, 한쪽만 갱신되는 일이 생긴다.
   */
  intoCanvas(ctx) {
    const proxy = Object.create(Renderer.prototype);
    proxy.ctx = ctx;
    proxy.assets = this.assets;
    proxy.camera = { x: 0, y: 0 };
    proxy.width = ctx.canvas.width;
    proxy.height = ctx.canvas.height;
    return proxy;
  }
