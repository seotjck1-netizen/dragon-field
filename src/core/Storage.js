...
export class Storage {
  ...
  async login(id, hash) {
    ...
    // 이름도 함께 준다 (0.68) — 서버 쪽과 같은 이유다.
    // 세이브가 없는 사람(다시 도전 직후)은 이것이 없으면 아이디로 불린다.
    return { token: hash, name: record.name || id, save: record.save || null };
  }

  /**
   * @param {number} [skippedMs] 건너뛴 전투 시간(0.70.17). 서버가 기록 시계에 더한다.
   *   혼자 하는 중(로컬 저장)에는 잴 시계가 없으므로 그냥 버린다.
   */
  async save(id, token, data, skippedMs = 0) {
    if (this.isServer) {
      return this._post('/api/save', {
        id, token, save: data, ...(skippedMs > 0 ? { skippedMs } : {}),
      });
    }
   ...
   * 서버가 있으면 시계(bornAt)·처음 잡은 기록·우편함까지 서버 쪽에서 함께
   * 되돌린다 — 그것들은 여기(브라우저)에 없다.
   *
   * 혼자 할 때(서버 없음)는 되돌릴 것이 세이브뿐이다. 그건 게임이 곧바로
   * 새 상태를 저장하면서 덮어쓰므로 여기서는 할 일이 없다.
   */
  async restart(id, token) {
    if (!this.isServer) return { ok: true, offline: true, bornAt: new Date().toISOString() };
    return this._post('/api/account/restart', { id, token });
  }
   ...
   */
  async submitRank(id, token, boss, where = '') {
    if (!this.isServer) return { ok: false, offline: true };
    // where — 공지에 적을 '어디에서' (0.61). 알릴지 말지와 글은 서버가 정한다.
    return this._post('/api/rank/submit', { id, token, boss, where });
  }
  ...
  /**
   * 시트 '계정' 탭의 삭제 예약을 본다. cancel 에 아이디를 넣으면 그 예약을 푼다 (0.64).
   */
  async adminAccountPending(id, token, cancel = [], restore = []) {
    if (!this.isServer) return { ok: false, offline: true };
    return this._post('/api/admin/account-pending', { id, token, cancel, restore }, 30000);
  }

  /** 시트 '계정' 탭을 지금 한 번 본다 (0.64). */
  async adminAccountSync(id, token) {
    if (!this.isServer) return { ok: false, offline: true };
    return this._post('/api/admin/account-sync', { id, token }, 60000);
  }
   ...
   * @param {number} gapDays 0 이면 곧바로 다음 시즌을 연다.
   */
  async adminSeasonSchedule(id, token, endsAt, gapDays) {
    if (!this.isServer) return { ok: false, offline: true };
    return this._post('/api/admin/season-schedule',
      { id, token, endsAt, gapDays }, 20000);
  }
