...
export class Sound {
  ...
  constructor(getSettings = () => ({})) {
    ...
    // ── 만들어 받은 곡 (0.59) ───────────────────────────────
    //
    // 0.58 까지 이 파일의 소리는 **전부 그 자리에서 만든 것**이었다(합성).
    // 그 규칙은 "밖에서 파일을 받아 오지 않는다" 를 지키려던 것이지, "음악을
    // 넣지 않는다" 가 아니었다 — 한 장짜리 html 은 mp3 도 안에 박아 넣으므로
    // 파일 하나로 오프라인에서 도는 성질은 그대로다.
    //
    // 그래서 진짜 곡이 있으면 그걸 틀고, 없으면 예전처럼 하프를 뜯는다.
    // (곡을 못 받아 오는 자리에서도 접속 화면에 음악이 끊기지 않아야 한다)
    this._tracks = {};      // 이름 → { src, loop }
    this._buffers = new Map(); // 이름 → AudioBuffer (한 번만 풀어 둔다)
    this._playToken = 0;    // 늦게 도착한 디코딩이 새 곡을 덮지 않게 하는 표
  }

  /**
   * 만들어 받은 곡 목록을 건네준다(src/data/audio.json).
   * @param {Record<string,{src:string, loop?:boolean}>} tracks
   */
  setTracks(tracks) {
    this._tracks = tracks || {};
  }

  /** 이름표가 가리키는 곡을 한 번만 풀어서 들고 있는다. */
  async _buffer(name) {
    if (this._buffers.has(name)) return this._buffers.get(name);
    const def = this._tracks[name];
    const ctx = this.ctx;
    if (!def || !def.src || !ctx) return null;
    try {
      // data: 주소도 fetch 로 읽힌다 — 한 장짜리 html 과 서버가 같은 길을 쓴다.
      ...
      // 못 읽어도 게임은 돌아가야 한다. 다시 묻지 않게 없음으로 적어 둔다.
  ...
  _pluckAt(freq, at, { gain = 0.5, seconds = 2.2, bright = 0.5, dest = null } = {}) {
    ...
    src.start(at);
    // 언제 끝나는지 적어 둔다 (0.70) — _pruneSources 가 이걸 보고 턴다.
    // onended 콜백을 다는 대신 숫자 하나로 끝내, 노드마다 붙는 짐을 아낀다.
    src._endsAt = at + seconds;
    return src;
  }
   ...
   *         |'bossroar'|'dragonroar'|'bossfall'|'door'|'drink'|'gear'
  ...
  sfx(name) {
    ...
    // ── 0.70.3 — 집 · 물약 · 장비 ──────────────────────────────
    //
    // 이 셋은 여태 **토스트 소리**를 빌려 쓰고 있었다(tone:'good' → 'loot').
    // 물약을 마셔도, 갑옷을 입어도, 문을 열어도 전부 "동전 줍는 소리"였다 —
    // 소리가 없는 것보다 나쁘다. 같은 소리는 "같은 일이 일어났다"는 뜻이라,
    // 다른 일에 같은 소리를 붙이면 귀가 둘을 구별하지 못한다.

    if (name === 'door') {
      // 나무문 — 빗장이 풀리는 '달칵' 하나와, 경첩이 돌아가는 낮은 '끼익'.
      // ⚠ portal 과 뚜렷이 달라야 한다. 저쪽은 위로 솟는 '휘익' 이므로
      //   이쪽은 **아래로 내려앉는** 소리로 잡는다.
      this._thump(t, to, { freq: 150, dur: 0.09, gain: 0.3 });
      this._noise(t + 0.02, to, { dur: 0.22, from: 1400, to: 380, gain: 0.11, q: 2.2 });
      this._ping(t + 0.04, to, { freq: 340, dur: 0.24, gain: 0.06, slideTo: 190 });
      return;
    }
    if (name === 'drink') {
      // 물약 — 목으로 넘어가는 '꼴깍' 둘. 음이 조금씩 올라가야 '비어 가는 병'이 된다.
      this._thump(t, to, { freq: 300, dur: 0.09, gain: 0.22 });
      this._thump(t + 0.11, to, { freq: 360, dur: 0.09, gain: 0.2 });
      this._ping(t + 0.2, to, { freq: 900, dur: 0.2, gain: 0.06, slideTo: 1500 });
      return;
    }
    if (name === 'gear') {
      // 장비를 걸치는 소리 — 쇳조각이 맞물리는 '차락'. 짧고 마른 소리다.
      this._noise(t, to, { dur: 0.1, from: 3600, to: 1600, gain: 0.16, q: 4 });
      this._ping(t + 0.01, to, { freq: 1500, dur: 0.16, gain: 0.08, slideTo: 900 });
      this._thump(t + 0.03, to, { freq: 190, dur: 0.08, gain: 0.14 });
      return;
    }
  ...
  playBgm(name = 'login') {
    ...
    this._bgm = { name, sources: [], timer: null, token };

    // 만들어 받은 곡이 있으면 그것을 튼다. 없으면 예전처럼 그 자리에서 만든다.
    if (this._tracks[name]) {
      this._playTrack(name, token);
      return;
    }
   ...
   * mp3 한 곡을 튼다.
   *
   * ⚠ 디코딩은 비동기다. 그 사이에 사람이 다른 화면으로 넘어가 다른 곡을 걸 수 있다.
   *   그때 늦게 도착한 이 곡이 그대로 울리면 **두 곡이 겹친다.** 그래서 표(token)를
   *   들고 나갔다가 돌아와서 아직 내 차례인지 확인한다.
   */
  async _playTrack(name, token) {
    const buf = await this._buffer(name);
    if (!this._bgm || this._bgm.token !== token) return;
    const ctx = this.ctx;
    if (!ctx) return;
    if (!buf) {
      // 곡을 못 읽었다(파일이 없거나 브라우저가 그 형식을 모른다).
      // 조용히 두지 않는다 — 예전처럼 그 자리에서 하프를 뜯어 준다.
      // 합성곡에도 없는 이름이면 _scheduleSong 이 알아서 아무것도 안 한다.
      this._scheduleLoop(name, ctx.currentTime + 0.1);
      return;
    }
    const src = ctx.createBufferSource();
    src.buffer = buf;
    src.loop = this._tracks[name].loop !== false;
    src.connect(this.musicGain);
    src.start(ctx.currentTime + 0.05);
    this._bgm.sources.push(src);
    // 한 번만 트는 곡(죽었을 때)은 끝나면 스스로 자리를 비운다 —
    // 안 그러면 다음에 같은 곡을 걸 때 "이미 그 곡이다" 로 막힌다.
    if (!src.loop) {
      src.onended = () => {
        if (this._bgm && this._bgm.token === token) this._bgm = null;
      };
    }
  }
  ...
  stopBgm() {
    ...
    for (const s of this._bgm.sources) {
      try { s.onended = null; } catch { /* 없을 수도 있다 */ }
      try { s.stop(); } catch { /* 이미 끝난 소리 */ }
    }
    this._bgm = null;
    this._playToken++; // 아직 풀리는 중인 곡이 뒤늦게 울리지 않게
  }

  /** 지금 무슨 곡이 걸려 있나(없으면 null). */
  currentBgm() {
    return this._bgm ? this._bgm.name : null;
  }
  ...
  _scheduleSong(name, at) {
    ...
    // ⚠ **끝난 소리를 버린다** (0.70).
    //
    //   여기서 만든 오실레이터를 sources 에 담아 두는 이유는 곡을 멈출 때
    //   한꺼번에 끄기 위해서다. 그런데 **한 번도 비우지 않았다.**
    //   한 바퀴에 128개씩 쌓이므로 30분만 놀아도 8천 개가 넘는다.
    //   그 목록이 통째로 살아 있으니 브라우저가 치울 수도 없고,
    //   곡을 바꿀 때는 그 8천 개에 일일이 stop() 을 부른다 —
    //   그 순간 화면과 소리가 함께 걸린다("음악이 뚝뚝 끊긴다").
    //
    //   이미 울리고 끝난 것은 들고 있을 이유가 없다. 예약할 때마다 턴다.
    this._pruneSources();

    // ⚠ 한 곡은 128개 남짓한 소리를 예약한다. 그것을 **한 프레임에 다 만들면**
    //   64ms 가 걸린다(재 봤다) — 60프레임 기준으로 네 프레임을 통째로 건너뛴다.
    //   그래서 눈에는 화면이 튀고, 귀에는 그 순간이 걸린다.
    //   한 번에 조금씩 만들되, **소리가 울릴 시각(t)은 미리 정해 두므로**
    //   나눠 만들어도 박자는 한 치도 어긋나지 않는다.
    const notes = song.notes;
    const token = this._bgm ? this._bgm.token : -1;
    const CHUNK = 24;
    const makeFrom = (i) => {
      // 곡이 바뀌었으면 나머지는 만들지 않는다(다른 곡 위에 겹쳐 울리지 않게).
      if (!this._bgm || this._bgm.token !== token) return;
      const end = Math.min(i + CHUNK, notes.length);
      for (let k = i; k < end; k++) {
        const [when, note, dur, gain] = notes[k];
        const src = this._pluckAt(noteFreq(note), at + when * beat, {
          gain: gain * 0.5,
      ...
      if (end < notes.length) setTimeout(() => makeFrom(end), 0);
    };
    // 첫 묶음은 지금 만든다 — 곡이 시작하는 순간까지 기다릴 여유가 없다.
    makeFrom(0);
  ...
  /**
   * 이미 울리고 끝난 소리를 목록에서 턴다 (0.70).
   *
   * AudioBufferSourceNode·OscillatorNode 는 한 번 울리면 다시 못 쓴다.
   * 끝난 것을 들고 있어 봐야 stopBgm 이 헛되이 stop() 을 부를 뿐이다.
   *
   * '끝났나' 는 예약해 둔 **끝 시각**으로 판단한다 — onended 를 달면 노드마다
   * 콜백이 하나씩 더 붙어서, 아끼려던 것을 도로 쓴다.
   */
  _pruneSources() {
    if (!this._bgm || !this.ctx) return;
    const now = this.ctx.currentTime;
    const kept = [];
    for (const src of this._bgm.sources) {
      // _pluckAt 이 적어 둔 끝 시각. 없으면(옛 경로) 그냥 들고 간다.
      if (typeof src._endsAt === 'number' && src._endsAt < now - 0.1) continue;
      kept.push(src);
    }
    this._bgm.sources = kept;
  }
}
