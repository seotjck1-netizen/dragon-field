...
export class Appearance {
  ...
  _build(base, look) {
    ...
    // ⑥ 전설 장비를 걸쳤으면 **몸 둘레가 빛난다** (0.70.6)
    //
    //   전설 갑옷(용비늘 성갑)과 용린 갑옷은 둘 다 초록이라 입어 봐도
    //   달라진 것이 안 보였다 — 가방에서는 붉은 글씨로 갈라 놓고 정작
    //   몸에서는 같은 물건이었다. 색만 바꾸면 "또 다른 초록 갑옷"일 뿐이라,
    //   **한눈에 다른 등급**임을 보여 주는 둘레 빛을 같이 넣는다.
    //   색은 UI 의 전설 빨강(--legendary)과 같은 계열로 맞춘다.
    const legend = armor.legendary || weapon.legendary || null;
    const out = legend ? haloed(canvas, w, h, legend) : canvas;
...
/**
 * 이 그림에서 이 파츠가 앉는 자리 (0.70.17).
 *
 * ── 왜 필요한가 ────────────────────────────────────────────
 * 걷는 그림은 **팔과 다리가 따로따로** 움직인다. 왼팔은 앞으로, 오른팔은 뒤로.
 * 그런데 pose.by 는 그림 **전체**를 한 번 기울이는 장치라(공격 자세용) 이걸 못 한다 —
 * 하나로 돌리면 왼쪽 소매와 오른쪽 소매가 같은 쪽으로 간다.
 *
 * 그래서 파츠마다 "이 자세에서는 여기" 를 따로 적을 수 있게 한다.
 * 안 적으면 원래 자리 그대로다 — 서 있는 그림은 한 픽셀도 달라지지 않는다.
 *
 * ⚠ 자리 값은 **손으로 고르지 않는다.** art-class.js 의 걷기 변환(회전각·이동량)에
 *   부품 중심을 통과시켜 뽑은 값이다. 걷기 변환을 고치면 이 값도 다시 뽑아야 한다.
 */
function posedPart(part, key) {
  const by = part.by;
  if (!by) return part;
  const name = String(key || '');
  for (const suffix of Object.keys(by)) {
    if (name.endsWith(`_${suffix}`)) return { ...part, at: by[suffix] };
  }
  return part;
}
...
/**
 * 몸 둘레를 그 색으로 빛나게 한 새 그림.
 *
 * 어떻게: 다 그린 그림의 **실루엣만** 뽑아(source-in) 그 색으로 칠하고,
 * 흐리게 번지도록 두 겹 깔아 둔 위에 원래 그림을 얹는다. 테두리를 따라 그리는
 * 것이 아니라 **몸 모양 그대로** 번지므로, 자세가 바뀌어도 따로 손댈 것이 없다.
 *
 * ⚠ 그림이 캔버스 가장자리에 붙어 있으면 번짐이 잘린다. 그래서 여백(pad)을
 *   두고 그린 뒤 같은 크기로 도로 잘라 낸다 — 바깥 테두리 한두 칸은
 *   어차피 비어 있다(스프라이트는 96×128 안에 여유를 두고 그린다).
 *
 * @param {HTMLCanvasElement} src  다 그린 그림
 * @param {[number,number,number]} c  빛 색
 */
function haloed(src, w, h, c) {
  const blur = Math.max(2, Math.round(w / 22));
  ...
  // ② 흐린 실루엣을 깔고 그 위에 원래 그림
  const out = document.createElement('canvas');
  out.width = w;
  out.height = h;
  const oc = out.getContext('2d');
  oc.globalCompositeOperation = 'lighter';
  // filter 를 못 쓰는 곳에서도 **아무것도 안 그려지지는 않게** 한다.
  // 번짐이 없으면 실루엣이 그대로 얹혀 몸이 통째로 빨개진다 — 그래서 그때는 건너뛴다.
  const canBlur = typeof oc.filter === 'string';
  if (canBlur) {
    oc.filter = `blur(${blur}px)`;
    oc.globalAlpha = 0.72;
    oc.drawImage(sil, 0, 0);
    oc.filter = `blur(${Math.round(blur * 0.45)}px)`;
    oc.globalAlpha = 0.5;
    oc.drawImage(sil, 0, 0);
    oc.filter = 'none';
  }
