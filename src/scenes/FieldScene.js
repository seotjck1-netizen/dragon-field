...
/** 뛰어오를 때 가장 높이 뜨는 몫(픽셀). 한 칸이 32 이므로 반 칸쯤이다. */
const JUMP_HEIGHT_PX = 15;
...
export class FieldScene {
  ...
  update(dt, { paused = false } = {}) {
    ...
    // 0.70.18 — 마을 사람도 움직인다. 표에 wander 가 적힌 사람만 어슬렁거리고,
    // 가게를 여는 사람은 제자리에서 advance 만 돌아 그림이 제때 갱신된다.
    for (const npc of state.npcs) Movement.updateNpc(npc, dt, map, this.rng, blockers);
   ...
   */
  /**
   * 지금 이 사람/짐승이 쓸 그림 이름 (0.70.17 · 0.70.19 에 박자를 네 배 늦춤).
   *
   * ── 왜 필요한가 ────────────────────────────────────────────
   * 예전에는 걷는 동안 그림 한 장을 위아래로 흔들기만 했다(사인파 2픽셀).
   * 그림이 바뀌지 않으니 발이 땅을 짚지 않고 **미끄러지는** 것처럼 보였다.
   *
   * 이제 **네 박자**로 그림을 갈아 낀다:
   *
   *     걷기1 → 서기 → 걷기2 → 서기
   *
   * 가운데에 서 있는 그림을 끼우는 것이 요령이다. 걷기1↔걷기2 만 오가면
   * 두 발이 동시에 벌어졌다 모이기만 해서 뒤뚱거림이 된다. 사이에 선 자세가
   * 들어가야 "짚었다 → 지나간다 → 짚는다" 가 된다.
   *
   * ── 박자 길이 (0.70.19) ────────────────────────────────────
   * **한 박자가 한 칸**이다. 0.70.17~18 은 한 칸을 네 박자로 쪼갰는데(42ms),
   * 그건 사람이 걷는 것이 아니라 **다리가 파르르 떠는** 것으로 보였다.
   * 네 배 늦춰서 네 박자가 네 칸에 걸쳐 돈다.
   *
   * ⚠ 박자는 **칸을 넘어 이어지는 시계**(actor.walkT)로 센다. 한 칸 안의 진행도
   *   (stepT)로 세면 칸마다 0 으로 되돌아가서, 애니메이션이 한 칸에 한 바퀴로 묶인다 —
   *   그러면 아무리 늦추려 해도 한 칸보다 느려질 수가 없다.
   *
   * ⚠ 다른 접속자에게는 그 시계가 없다(그물로 보내지 않는 값이다 — bobT 와 같은 이유).
   *   그래서 그쪽은 **받는 쪽 시계**로 박자를 만든다. 그림이 조금 엇나가도
   *   걷는 것처럼은 보이고, 무엇보다 NaN 이 생기지 않는다.
   *
   * ⚠ 이름 규칙은 하나다 — 뿌리 이름에 꼬리를 붙인다:
   *     chr_hero_field → 뿌리 chr_hero → chr_hero_walk1 · chr_hero_walk2
   *     npc_shopkeeper → 뿌리 npc_shopkeeper → npc_shopkeeper_walk1
   *   자산에 없으면 그냥 원래 이름으로 떨어진다. 그래서 새 그림을 안 구운 것이
   *   화면을 깨뜨리지 않는다(캔버스는 그림이 없으면 조용히 아무것도 안 그린다).
   */
  _walkKey(renderer, actor) {
    const base = actor.sprite;
    if (typeof base !== 'string') return base;
    const assets = renderer && renderer.assets;
    const has = (k) => !!(assets && assets.has(k));

    // 뿌리 이름 — `_field` 꼬리만 떼면 된다(마을 사람은 꼬리가 없다).
    //
    // ⚠ 0.70.19 — **위로 갈 때 뒷모습으로 바꾸던 것을 뺐다(보류).**
    //   밑그림만 뒤로 바꿔 봐야 그 위에 얹히는 장비(투구·허리띠·손에 든 무기)는
    //   앞을 본 채라, 얼굴만 지워진 사람이 된다. 파츠의 뒷면 한 벌이 생기기 전에는
    //   앞모습 하나로 가는 편이 낫다(tools/art-class.js 의 CLASS_BACK 주석 참고).
    const stem = base.replace(/_field$/, '');
    const stand = base;

    if (!actor.moving) return stand;

    const pick = (n) => (has(`${stem}_walk${n}`) ? `${stem}_walk${n}` : stand);

    // 뛰어오르는 동안에는 **한 자세로 고정**한다 (0.70.17).
    // 공중에서 다리를 휘저으면 뛴 것이 아니라 허우적거리는 것으로 보인다.
    // 걷기1(두 다리가 벌어진 그림)이 그대로 공중 자세가 된다.
    if (actor.jumping) return pick(1);

    // ── 박자 길이 (0.70.19 에 네 배 느리게) ─────────────────
    //
    // 0.70.17~18 은 **한 칸을 네 박자**로 나눴다. 한 칸이 170ms 이므로 한 박자가
    // 42ms — 초당 24장이다. 사람이 걷는 것이 아니라 **다리가 파르르 떠는** 것처럼 보였다.
    //
    // 이제 **한 박자가 한 칸**이다(네 박자 = 네 칸). 네 배 느려졌다.
    // 걸음이 빨라지면(질주 물약) stepMs 가 줄어드니 애니메이션도 같이 빨라진다 —
    // 빠르기와 그림이 따로 놀지 않는다.
    const beatMs = actor.stepMs > 0 ? actor.stepMs : CONFIG.PLAYER_STEP_MS;
    // 칸을 넘어서도 이어지는 시계로 센다(MovementSystem.advance 가 굴린다).
    // 다른 접속자에게는 그 시계가 없으므로 **받는 쪽 시계**로 만든다.
    const t = Number.isFinite(actor.walkT) ? actor.walkT : this.time;
    const beat = Math.floor(t / beatMs) & 3;
    if (beat & 1) return stand; // 1·3 박자는 서 있는 그림
    return pick(beat === 0 ? 1 : 2);
  }

  /**
   * 뛰어오른 높이(픽셀). 걸음의 가운데에서 가장 높다 (0.70.17).
   *
   * ⚠ 그림자는 **같이 안 올라간다.** 그림자가 몸을 따라 올라가면 그냥
   *   "그림이 위로 움직였다" 가 되어, 뛴 것으로 안 보인다. 땅에 남아 작아져야
   *   몸이 땅에서 떨어졌다는 것이 보인다(아래 그리는 자리에서 그렇게 한다).
   */
  _jumpLift(actor) {
    if (!actor.moving || !actor.jumping) return 0;
    // ⚠ 다른 접속자에게는 stepT 가 없다(그물로 보내는 것은 '뛰는 중' 이라는 사실뿐).
    //   그래서 그쪽은 **뜬 동안 꼭대기 근처에 띄워 둔다.** 곡선을 못 그리는 대신
    //   "땅에서 떨어져 있다" 는 것은 보이고, 무엇보다 NaN 이 생기지 않는다.
    //   곡선까지 맞추려면 걸음의 진행도를 보내야 하는데, 그건 100ms 전 이야기라
    //   맞춰 봐야 어차피 어긋난다.
    if (!Number.isFinite(actor.stepT) || !(actor.stepMs > 0)) return JUMP_HEIGHT_PX * 0.8;
    const dur = actor.stepMs * (actor.stepSpan || 1);
    const p = Math.max(0, Math.min(1, actor.stepT / dur));
    return Math.sin(p * Math.PI) * JUMP_HEIGHT_PX;
  }

  /**
   * 마주 본 칸이 올라설 수 있는 것이면 그 위에 작은 화살표를 띄운다 (0.70.17).
   *
   * 걷는 중에는 안 띄운다 — 지나가는 길에 깜박이면 눈에 거슬리기만 한다.
   * 멈춰 서서 그쪽을 볼 때, 곧 "올라갈까?" 하고 있을 때만 뜬다.
   */
  _drawClimbHint(renderer, map, player, T) {
    if (player.moving) return;
    const v = DIR_VECTORS[player.dir];
    if (!v || (v.x && v.y)) return; // 대각선으로는 못 뛴다(MovementSystem 과 같은 규칙)
    const tx = player.tx + v.x;
    const ty = player.ty + v.y;
    if (!Movement.isClimbable(map, tx, ty)) return;

    const { ctx } = renderer;
    const cx = tx * T + T / 2 - renderer.camera.x;
    const cy = ty * T + T / 2 - renderer.camera.y;
    // 천천히 오르내린다 — 움직이는 것이라야 눈에 들어온다.
    const rise = Math.sin(this.time / 380) * 2.2;
    ctx.save();
    ...
    ctx.lineJoin = 'round';
    ctx.beginPath();
    ctx.moveTo(cx - 5, cy - 4 - rise);
    ctx.lineTo(cx, cy - 9 - rise);
    ctx.lineTo(cx + 5, cy - 4 - rise);
    ctx.stroke();
    ctx.restore();
  }
  ...
  _groundLayer(renderer, map, T) {
    ...
    for (let ty = 0; ty < map.h; ty++) {
      for (let tx = 0; tx < map.w; tx++) {
        ...
        //   이어져 한 갈래 흙길로 보인다.
        if (tile.over) {
          // 0.70.20 — 밑에 깔 것을 **타일이 지정할 수 있다**(tileset 의 under).
          //
          //   왜 필요한가: 얹는 것이 다 바닥 위에 있는 것은 아니다. 창문·간판·성문은
          //   **벽에 붙는다.** 그 밑에 바닥을 깔면 벽 한가운데에 마룻바닥 네모가 생긴다.
          //   그래서 "무엇 위에 얹히는가" 를 표에 적게 했다. 안 적으면 그 지도의 바닥이다.
          bake.drawSprite(underAt(map, tile, tx, ty), tx * T, ty * T, { anchor: 'topleft' });
        }
        // 0.70.21 — 칸보다 큰 물건은 **사람과 같은 줄에 세운다.**
        // 바닥 판에 찍지 않고 배우 목록으로 넘긴다 — 그래야 뒤로 걸어갈 수 있다.
        if (GROWN.has(tile.sprite)) {
          props.push({
            kind: 'prop',
            sprite: tile.sprite,
            px: tx * T + T / 2,
            py: ty * T + T, // 발밑은 칸의 아래 변
          });
          continue;
        }
  ...
  render(renderer) {
    ...
    // 올라설 수 있는 것을 마주 보고 서면 **표시가 뜬다** (0.70.17).
    //
    // 왜 필요한가: 술통과 벽은 화면에서 똑같이 '못 가는 칸' 이다. 눌러 보기 전에는
    // 올라갈 수 있는지 알 길이 없고, 사람은 막힌 칸을 두 번 밀어 보지 않는다.
    // 그래서 **붙어서 그쪽을 볼 때만** 작게 알려 준다. 늘 떠 있으면 마을이 표지판 밭이 된다.
    // 사람보다 **먼저** 그린다 — 올라선 뒤에는 발밑에 깔려 보이지 않아야 한다.
    this._drawClimbHint(renderer, map, player, T);
    ...
    for (const actor of actors) {
      ...
      // 0.70.17 — 흔들림을 2 → 1 로 줄였다. 이제 다리가 실제로 움직이므로(_walkKey)
      // 예전만큼 들었다 놓으면 **두 겹으로 출렁여** 물 위를 걷는 것처럼 보인다.
      // 흔들림은 이제 걸음의 전부가 아니라 무게를 얹는 몫이다.
      const bob = actor.moving ? Math.sin((bobT / 90) * Math.PI) * 1 : 0;
      ...
      // 0.70.17 — 걷는 동안에는 **걷는 그림**으로 갈아 낀다.
      const baseKey = this._walkKey(renderer, actor);
      let sprite = baseKey;
      if (this.appearance && actor.kind === 'player' && !actor.rawSprite) {
        sprite = this.appearance.get(baseKey, myLook);
      } else if (this.appearance && actor.kind === 'peer') {
        sprite = this.appearance.get(baseKey, actor.look);
      }
      ...
      // 0.70.17 — 뛰어오른 높이. 몸만 올라가고 **그림자는 땅에 남는다.**
      const lift = this._jumpLift(actor);
      if (!(actor.kind === 'player' && state.player.hidden)) {
        // 뜬 만큼 그림자를 줄이고 옅게 한다 — 높이 뜰수록 작고 흐려야 떠 보인다.
        const k = lift / JUMP_HEIGHT_PX;
        renderer.drawShadow(actor.px, actor.py - 3, shadowW * (1 - k * 0.35),
          lift > 0 ? { alpha: 0.22 * (1 - k * 0.5) } : undefined);
      }
      renderer.drawSprite(sprite, actor.px, actor.py + 4 - bob + idle - lift, {
        anchor: 'bottom',
 ...
 */
/**
 * 지도가 말하는 **제 바닥**. maps.json 의 `ground` 가 이 표의 열쇠다.
 *
 * ⚠ 내보내는 까닭: 시험(tiletrans)이 "모든 지도가 여기 있는 바닥을 말하나" 를 본다.
 *   표를 여기 두고 시험이 읽게 해야, 새 지도를 넣고 ground 를 빠뜨리는 순간 걸린다
 *   (빠뜨리면 조용히 풀밭으로 떨어져서, 성 안 기둥 밑에 잔디가 깔린다).
 */
export const GROUNDS = {
  grass: GRASS_VARIANTS,
  // ⚠ 0.70.18 까지 **한 장**이었다. 화산 들판 넷이 전부 같은 무늬라 벽지로 보였다.
  ash: ['tile_ash', 'tile_ash2', 'tile_ash3'],
  waste: ['tile_waste', 'tile_waste2', 'tile_waste3', 'tile_waste4', 'tile_waste5'],
  // 0.70.20 — 실내·성·지하도 제 바닥을 말한다.
  //
  // ⚠ 예전에는 이 표에 풀·재·마른땅뿐이었다. 그래서 `ground` 를 안 적은 지도
  //   (마을 가게·성·지하감옥)는 전부 **풀**로 떨어졌다. 얹는 그림을 안 쓰던
  //   동안에는 아무 일도 없었지만, 가구를 얹는 그림으로 바꾸는 순간
  //   잡화점 술통 밑에 잔디가 깔린다.
  room: ['tile_room_floor'],
  castle: ['tile_castle_floor', 'tile_castle_floor2'],
  dungeon: ['tile_dungeon_floor', 'tile_dungeon_floor2'],
  brick: ['tile_brick', 'tile_brick2'],
};
...
/**
 * 얹는 그림 **밑에 깔 것** (0.70.20).
 *
 * 타일이 `under` 를 적어 두었으면 그것을 깔고, 안 적었으면 그 지도의 바닥을 깐다.
 * 적어 둔 것도 변형이 여러 장이면 자리에 따라 고른다 — 한 장으로 고정하면
 * 그 칸만 이웃과 다른 장이 깔려 오히려 더 눈에 띈다.
 */
function underAt(map, tile, x, y) {
  if (!tile.under) return groundAt(map, x, y);
  const list = VARIANTS[tile.under];
  if (!list) return tile.under;
...
const VARIANTS = {
  // 0.70.19 — 길은 곧게 뻗을수록 반복이 잘 보인다. 두 장 → **네 장**.
  tile_path: ['tile_path', 'tile_path2', 'tile_path3', 'tile_path4'],
  ...
  // 서쪽 들판은 한 화면이 거의 다 이 땅이다. 세 장 → **다섯 장**.
  tile_waste: ['tile_waste', 'tile_waste2', 'tile_waste3', 'tile_waste4', 'tile_waste5'],
};
...
/**
 * 칸보다 **큰 물건** — 발밑을 칸 아래 변에 맞춰 세운다 (0.70.21).
 *
 * ── 왜 칸에 안 찍고 세우나 ────────────────────────────────
 * 사람은 48×64 로 그려진다(가로 한 칸 반, 세로 두 칸). 그런데 침대·탁자·창문은
 * 32×32 한 칸에 갇혀 있어서, 여관에 들어서면 **침대가 사람 상반신만 했다.**
 *
 * 그래서 같은 그림을 크게 구웠다(tools/gen-assets.js 의 GROW). 커진 그림을
 * 칸에 좌상단 기준으로 찍으면 아래로 삐져나가 **한 칸 아래를 덮는다.**
 * 발밑(칸 아래 변)을 기준으로 세워야 커진 몫이 위와 옆으로 자란다 —
 * 큰 나무와 집 지붕이 쓰는 것과 똑같은 규칙이다.
 *
 * ⚠ 나무와 달리 **그늘을 깔지 않는다.** 나무 그늘(TREE_SHADE)은 잎 모양이라
 *   침대 밑에 깔면 침대가 덤불 위에 놓인 것처럼 보인다.
 * ⚠ 막는 칸(solid)은 그대로 제 한 칸이다. 그림만 커졌지 자리는 안 커졌다 —
 *   그림 크기로 길을 막으면 옆 칸까지 못 지나가게 되어 방이 좁아진다.
 */
const GROWN = new Set([
  'tile_bed', 'tile_table', 'tile_cauldron', 'tile_barrel', 'tile_shelf',
  'tile_room_window', 'tile_forge_hearth', 'tile_forge_anvil',
  'tile_stall_weapon', 'tile_stall_armor',
]);
