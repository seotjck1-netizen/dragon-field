  ...
  return map.npcDefs
    .map((entry) => {
      ...
      return createActor({
        ...
        dir: entry.dir || 'down',
        sprite: def.sprite,
        // 0.70.18 — 어슬렁거리는 사람은 실제로 걷는 빠르기로 옮긴다.
        // 9999 는 "사실상 안 움직인다" 는 뜻이었고, 그대로 두면 한 칸 가는 데 10초가 걸린다.
        stepMs: def.wander ? 320 : 9999,
        extra: {
          interactive: true,
          // 돌아다닐 칸수(표에 적힌 사람만). 집 자리는 처음 선 자리다 —
          // 여기서 그 칸수 밖으로는 안 나간다(MovementSystem.updateNpc).
          wander: Number(def.wander) || 0,
          homeX: entry.x,
          homeY: entry.y,
        },
      });
