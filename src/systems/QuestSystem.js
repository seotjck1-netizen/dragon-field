...
import { questExpReward } from '../data/formulas.js';
...
export function parseQuests(db) {
  return (db.quests['표'] || []).map((row, index) => ({
    ...
    // 0.70.18 — 눈금(BALANCE.QUEST_EXP_GAIN)을 **읽는 자리에서 한 번** 건다.
    //
    // 왜 여기인가: 이 값은 계획(plan)에도 가고 게시판 화면에도 가고 완료 알림에도 간다.
    // 주는 자리에서만 깎으면 게시판에는 안 깎인 숫자가 적혀서, 받고 나서 "왜 적지?" 가 된다.
    // 여기서 한 번 깎으면 세 곳이 저절로 같은 숫자를 말한다.
    exp: questExpReward(row[COL.exp]),
