...
function computeGearBonuses(state, foes = null) {
  ...
  // 여러 장에서 와도 **가장 큰 하나만** 세는 값. 더하면 안 되는 것들이다.
  // (skillPower — 전설 장비의 '스킬 효과 배가'. 0.70.8 참고)
  const GEAR_MAX_MODS = new Set(['skillPower']);

  const add = (kind, prop, value, label) => {
    if (kind === 'trait') traits[prop] = (traits[prop] || 0) + value;
    else if (GEAR_MAX_MODS.has(prop)) mods[prop] = Math.max(mods[prop] || 0, value);
 ...
 * 이 장비에 보석을 박을 수 있는가.
 *
 * @param {number|null} [slotIndex] 몇 번째 홈에 박을까. 주면 **이미 박힌 홈에도**
 *   덮어 박는다(0.70.14 — 사람이 요청한 규칙). 안 주면 예전처럼 빈 홈을 찾는다.
 *
 * ⚠ 덮어 박으면 그 자리에 있던 보석은 **사라진다**(빼서 돌려받는 것이 아니다).
 *   그래서 화면은 무엇이 사라지는지 이름을 대고 한 번 더 묻는다.
 */
export function canSocket(state, uid, gemId, slotIndex = null) {
  const inst = (state.inventory || []).find((i) => i.uid === uid);
  if (!inst) return { ok: false, reason: '없는 아이템입니다.' };
  const itemDef = state.db.items[inst.id];
  if (!itemDef || !itemDef.slot) return { ok: false, reason: '장비가 아닙니다.' };
  ...
  const gems = inst.gems || [];
  const over = Number.isInteger(slotIndex) && slotIndex >= 0 && slotIndex < sockets;
  const free = gems.findIndex((g) => !g);
  if (!over && free < 0 && gems.length >= sockets) {
    return { ok: false, reason: '빈 홈이 없습니다. 홈을 골라 누르면 덮어 박을 수 있습니다.' };
  }
  ...
  // 변신 장비에는 **덮어 박을 수 없다.** 차례가 곧 조리법이라, 앞에 박은 것을
  // 지우면 되돌릴 수 없이 망가진다. 규칙을 두 개로 나누지 않고 여기서 막는다.
  if (over && tm && !tm.broken) {
    return { ok: false, reason: `${itemDef.name}은(는) 차례대로만 박을 수 있습니다 — 덮어 박기는 안 됩니다.` };
  }
  ...
  const slot = over ? slotIndex : (free < 0 ? gems.length : free);
  // 그 자리에 이미 무엇이 있었나 — 화면이 "무엇이 사라지는지" 를 말할 수 있게 돌려준다.
  const wasId = gems[slot] || null;
  return { ok: true, slot, gem: g, replaces: wasId ? gemDef(state.db, wasId) : null };
}
 ...
 */
export function socketGem(state, uid, gemId, slotIndex = null) {
  const check = canSocket(state, uid, gemId, slotIndex);
  if (!check.ok) return check;
  const inst = state.inventory.find((i) => i.uid === uid);
  ...
  return {
    ok: true, gem: check.gem, replaced: check.replaces || null,
    next: tm && !tm.broken ? tm.next : null,
  };
}
