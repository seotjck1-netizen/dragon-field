...
const NO_MODS = {
  ...
  shieldBonusTurns: 0,
  evadeBonus: 0,
  // 0.59 — '바람의 걸음'. 회피에 성공하면 **다음 한 대**의 피해가 이만큼 오른다.
  // 확률(evadeBonus)이 아니라 배율이다 — 피한 값이 곧 다음 한 방이 된다.
  evadeRevenge: 0,
...
export const NO_TRAITS = {
  ...
  // 이 무기 갈래를 들었을 때만 선제 공격이 나간다 (0.70.9).
  //   'bow' 면 활을 들어야 한다. 비우면 아무 무기나.
  // 왜: 사냥꾼이 검을 들어도 선제 공격이 나가고 있었다. 그러면 활은 "그냥 무기"고
  // 직업 패시브는 무기와 아무 상관이 없어진다 — 활을 들 이유가 없다.
  openerWeapon: null,
  ...
  lastStand: 0, // 치명상을 버틸 확률 (용사). 첫 번째는 확정, 그 뒤부터 이 확률
  // 방어력 **200** 마다 위 확률에 더해지는 몫 (용사, 0.70.10 에 100 → 200).
  //   0.01 이면 방어력 3,800 에 +19%p → 50% + 19% = 69%
  // "방어를 올려서 세지는 직업" 을 **버티는 쪽**으로 돌려주는 값이다. 0.70.8 에
  // 방어력이 공격 배율을 못 타게 막고 나니, 용사는 방어력만 높고 그 방어력이
  // 아무 데도 안 쓰이는 몸이 됐다(끝 보스 전부 0%). 이제 방어력이 곧 목숨이다.
  lastStandPerDef: 0,
  lastStandCap: 0.95, // 그래도 100% 는 안 된다 — 무적을 다시 만들지 않기 위해
...
export function simulateBattle({
  ...
  // 무기 갈래에 걸린 패시브 (0.70.9).
  //
  // ⚠ 막는 자리는 **여기 하나**다. 부르는 쪽(main.js · 측정 도구)은 사람이 든
  //   무기의 갈래를 player.weaponShape 로 넘기기만 한다. 조건을 여러 곳에 흩으면
  //   게임과 측정 도구가 서로 다른 사람을 재게 된다(0.70.8 에 실제로 그랬다).
  if (tr.openerWeapon && player.weaponShape !== tr.openerWeapon) tr.opener = 0;
  ...
  const foes = list.map((m, i) => {
    ...
    return {
      ...
      side: 'monster',
      index: i,
      // 이 상대만의 보정(표의 '마법저항' 칸)을 무리 전체의 보정 위에 얹는다.
      // monsterMods 는 "이번 판의 무리 전체"에 거는 값이고, m.magicResist 는
      // "이 놈이 원래 그런 놈" 이다. 둘을 더해서 쓴다.
      mods: Number(m.magicResist) > 0
        ? { ...mMods, magicResist: (mMods.magicResist || 0) + Number(m.magicResist) }
        : mMods,
  ...
  // 0.59 — '바람의 걸음'. 피한 **바로 다음 한 대**가 세진다.
  //
  // 쌓이지 않는다. 두 번 연속 피해도 다음 한 대에 한 번만 얹힌다 —
  // 쌓이게 두면 회피율이 높은 사냥꾼이 가만히 있다가 한 방에 모든 것을 끝내게 되고,
  // 그건 '피하고 되받아친다' 가 아니라 '기다렸다 터뜨린다' 다.
  let revengeOwed = false;
  ...
  function strike(attacker, defender, { extra = false, power = 1, tag = null, gap = null } = {}) {
    ...
    if (defender.side === 'player') {
      ...
      if (guardNext) {
        guardNext = false;
        if (pMods.evadeRevenge > 0) revengeOwed = true;
      ...
      // ⚠ 보호막(1번)은 '피한 것'이 아니다 — 마법사의 것이고, 맞고도 안 아픈 것이다.
      //   여기에 얹으면 두 직업의 규칙이 섞인다.
    }
    ...
    // 이 한 대 가운데 마법인 몫(0~1). 아래 두 곳에서 쓴다 —
    //   ① 마법은 타고나기를 갑옷을 무시한다(BALANCE.MAGIC_PIERCE)
    //   ② 마법 피해 증가/감소는 이 몫에만 걸린다
    const share = magicShare(attacker);

    // 관통 = 장비에서 온 것 + 마법이 타고난 것.
    // 마법을 막는 것은 방어력이 아니라 마법 저항이라는 규칙이 여기서 선다.
    const effPierce = Math.min(BALANCE.PIERCE_CAP, aM.pierce + BALANCE.MAGIC_PIERCE * share);
    const effDef = defender.def * (1 - effPierce);
    ...
    // 0.59 — '바람의 걸음'. 피한 바로 다음 한 대가 이만큼 세진다.
    //
    // 여기에 두는 이유: 물리·마법을 가리지 않고 **주는 피해 전체**에 걸려야 하고,
    // 상대의 피해 감소보다는 앞이어야 한다(세게 때린 값에서 상대가 깎는 것이 순서다).
    // 여파(광역)나 반사로는 안 쓴다 — 내가 휘두른 한 대여야 한다.
    let revenged = false;
    if (revengeOwed && attacker.side === 'player' && aM.evadeRevenge > 0) {
      dmg = Math.max(BALANCE.MIN_DAMAGE, Math.round(dmg * (1 + aM.evadeRevenge)));
      revengeOwed = false;
      revenged = true;
    }

    if (dM.dmgReduction) {
      // ⚠ **천장을 씌운다** (0.70.8). 이게 없어서 무적이 만들어져 있었다.
      //   불굴 5·강철 의지 5 에 전설 장비의 '스킬 효과 배가'(×3)가 겹치면
      //   dmgReduction 이 1.26 — 126% 감소다. 아래 식이 음수가 되고
      //   Math.max 가 그것을 MIN_DAMAGE 로 끌어올리므로, **고룡에게 맞아도 1**이었다.
      //   숫자가 커질수록 아무것도 안 아파지는 것은 감소율의 본성이라,
      //   막는 자리는 여기 하나뿐이어야 한다.
      const cut = Math.min(BALANCE.DMG_REDUCTION_CAP, dM.dmgReduction);
      dmg = Math.max(BALANCE.MIN_DAMAGE, Math.round(dmg * (1 - cut)));
    }
    ...
    if (defender.side === 'player' && dmg >= defender.hp && tr.lastStand > 0) {
      ...
      // 방어력 200 마다 확률이 오른다(용사). 방어력이 곧 목숨이 되는 자리다.
      const bonus = ((defender.def || 0) / 200) * (tr.lastStandPerDef || 0);
      const chance = Math.min(tr.lastStandCap || 0.95, tr.lastStand + bonus);
      if (sure || rng.chance(chance)) {
        lastStandCount++;
        survived = true;
    ...
    if (
      ...
      //
      // 0.64 — **소수는 확률이다.** 1.5 대 = 한 대는 확실히, 두 번째는 반반.
      // 여기서 한 번만 굴린다(매 대마다 굴리면 1.5 가 사실상 두 대가 된다).
      // rng 는 seed 에서 나오므로 같은 판은 같은 결과다 — 다시 보기가 어긋나지 않는다.
      const want = Number(tr.shieldTurns);
      const whole = Number.isFinite(want) ? Math.floor(want) : 1;
      const frac = Number.isFinite(want) ? want - whole : 0;
      shieldLeft = Math.max(1, whole + (frac > 0 && rng.chance(frac) ? 1 : 0));
    ...
    push({
      ...
      crit: isCrit,
      extra,
      // 'opener' | 'charge' | 'counter' | 'revenge' | null — 로그 문구용.
      // '바람의 걸음'으로 세진 한 대는 그 자리에서 이유가 보여야 한다 — 안 그러면
      // 같은 상대에게 어떤 대는 배로 들어가는 것이 그냥 운으로 보인다.
      tag: revenged && !tag ? 'revenge' : tag,
