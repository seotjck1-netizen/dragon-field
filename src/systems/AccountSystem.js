...
import { reservedIdReason } from './ReservedId.js';
 ...
 *
 * @param {string} id
 * @param {{register?:boolean}} [opts] register 면 **예약어**도 함께 본다.
 *
 * ⚠ 예약어는 **계정을 만들 때만** 본다. 접속할 때 보면 진짜 운영자(`admin`)가
 *   제 계정으로 못 들어온다. 실제로 한 번 그럴 뻔했다.
 */
export { reservedIdReason, RESERVED_HINT } from './ReservedId.js';

export function validateId(id, { register = false } = {}) {
...
export function serializeSave(state) {
  ...
  return {
    ...
    // 아직 이번 시즌의 직업을 안 골랐다 (0.70.14).
    //
    // ⚠ 세이브에 적어 두는 이유: 서버가 주는 시즌 알림은 **한 번만** 온다
    //   (world.takeSeasonNotice 가 보여 주고 지운다). 그 한 번을 놓치면 —
    //   창을 새로 고치거나, 알림이 뜨기 전에 껐다가 켜면 — 직업을 고를 기회가
    //   영영 사라진다. 세이브에 남겨 두면 고를 때까지 몇 번이고 다시 묻는다.
    ...(state.needClassPick ? { needClassPick: 1 } : {}),
    // 지난 시즌 요약 — 직업을 고르는 창이 보여 준다. 고르고 나면 사라진다.
    ...(state.needClassPick && state.lastSeason ? { lastSeason: state.lastSeason } : {}),
...
export function applySave(state, save) {
  ...
  // 이번 시즌의 직업을 아직 안 골랐으면, 고를 때까지 따라온다.
  state.needClassPick = save.needClassPick ? 1 : 0;
  state.lastSeason = save.lastSeason || null;
  ...
  state.quests = save.quests && typeof save.quests.index === 'number'
    ? {
        ...
        met: save.quests.met || {},
        // 0.59 — 사냥 의뢰를 **받은 순간의 눈금**. 이게 있어야 미리 잡아 둔 것이
        // 안 얹힌다(QuestSystem.openQuest). 옛 세이브에는 없다 — 그때는
        // main.js 가 불러온 직후 syncOpenQuests 로 지금 값을 찍는다.
        starts: save.quests.starts || {},
      }
    : { index: 0, kills: {}, reached: {}, done: [], met: {}, starts: {} };
