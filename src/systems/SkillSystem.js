...
import { resetCost, BALANCE } from '../data/formulas.js';
...
export const EMPTY_MODS = {
  ...
  evadeRevenge: 0, // 회피에 성공한 바로 다음 한 대의 피해 배율 (사냥꾼 — 바람의 걸음)
  ...
  // 0.63 — 쓰러져도 골드를 안 잃는다. 매직 투구가 준다.
  //   시야·은신은 지하감옥에서만 값을 하는데, 그 땅에 갈 무렵이면 이미 룬 투구를 쓴다.
  //   그래서 매직 투구는 사실상 아무도 안 썼다. **어디서나 값을 하는** 것을 하나 준다.
  keepGoldOnDeath: 0,
};
 ...
 */
// skillPower(전설 장비의 '스킬 효과 배가') 도 같은 종류다 — 여러 장을 껴도
// **가장 큰 한 장**만 센다. 더하면 두 장에 ×3, 세 장에 ×4 가 되어
// 스킬이 제 최대치를 훌쩍 넘어선다(0.70.8 에 그렇게 나 있었다).
const MAX_KEYS = new Set(['lowHpThreshold', 'skillPower']);

/**
 * 지금 들고 있는 무기의 **갈래** — 'bow' | 'sword' | 'greatsword' | 'staff' | 'club' | null.
 *
 * 무기에 걸린 직업 패시브(사냥꾼의 선제 공격)가 이것을 본다.
 *
 * ⚠ 갈래는 **appearance.json 의 무기 모양**에서 읽는다. 그림표에서 규칙을 읽는 것이
 *   조금 이상해 보이지만, 그것이 지금 "이 무기가 검인가 활인가" 를 적어 둔 **유일한
 *   자리**다. items.json 에 갈래 칸을 따로 만들면 같은 사실이 두 곳에 적히고,
 *   언젠가 한쪽만 고쳐져 "그림은 활인데 규칙은 검" 이 된다.
 *   갈래 칸을 따로 두려면 그때 appearance 쪽을 지워야 한다.
 */
export function weaponShape(state) {
  const uid = ((state.player || {}).equipment || {}).weapon;
  if (!uid) return null;
  const inst = (state.inventory || []).find((i) => i.uid === uid);
  if (!inst) return null;
  const look = ((state.db.appearance || {}).weapon || {})[inst.id];
  return (look && look.shape) || null;
}
...
/**
 * 점수마다 늘어나는 양이 **다른** 스킬 (0.64).
 *
 * `effect` 는 '1점당 얼마' 라서 곧게만 자란다 — 1, 2, 3, 4.
 * 그런데 "1점은 한 대, 2점은 반 대 더, 3점은 한 대 더" 처럼 걸음이 고르지 않은
 * 스킬이 있다. 그런 것은 `steps` 에 **그 점수에서의 누적값**을 그대로 적는다.
 *
 *   "steps": { "shieldBonusTurns": [0.5, 1, 2] }   ← 1점 / 2점 / 3점
 *
 * 왜 누적값인가: 증가분으로 적으면 표만 보고는 "3점이면 결국 몇이지"를
 * 머릿속으로 더해야 한다. 시트에서 값을 만지는 사람이 보고 싶은 것은 그 합이다.
 *
 * 전설 장비의 skillPower 는 여기서도 **찍은 점수를 늘리는 쪽**으로 먹인다.
 * 표에 적힌 마지막 칸을 넘어서면 그 마지막 칸에서 멈춘다 — 표에 없는 값을
 * 지어내지 않는다.
 */
function addSteps(target, steps, effectiveRank) {
  for (const [k, arr] of Object.entries(steps || {})) {
    if (target[k] === undefined || !Array.isArray(arr) || !arr.length) continue;
    const i = Math.min(Math.max(1, Math.round(effectiveRank)), arr.length) - 1;
    target[k] += Number(arr[i]) || 0;
  }
  return target;
}
...
export function classSkills(state) {
  ...
  // 0.70.18 — **운영자는 세 직업의 스킬을 다 배운다.**
  //
  // 운영자는 노는 사람이 아니라 보러 온 사람이다. 어디가 세고 어디가 약한지
  // 보려면 한 직업의 눈으로만 볼 수가 없다. 몸(classId)은 하나로 두고
  // 배울 수 있는 목록만 연다 — main.js 의 applyAdminLook 이 이 표를 켠다.
  //
  // ⚠ 이건 **권한이 아니라 표시**다. 진짜 운영자인지는 서버가 정한다
  //   (server.js 의 adminOnly). 여기 값이 켜져 있다고 서버가 무엇을 해 주지는 않는다.
  const all = !!(state.player && state.player.allSkills);
  const ids = !all && cls.skills && cls.skills.length
    ? cls.skills
...
export function computeModifiers(state, foes = null) {
  ...
  //
  // ⚠ 0.70.8 — **겹치지 않게 천장을 뒀다.**
  //   전설 장비는 한 장에 skillPower 1(= 스킬 효과 두 배)이다. 그런데 그 값이
  //   장비마다 **더해지고** 있어서, 용살자 + 용비늘 성갑을 같이 끼면 ×3 이 됐다.
  //   ×3 은 그냥 센 정도가 아니라 **다른 게임**이다 — 5단계짜리 스킬이 15단계처럼
  //   먹히니 제 최대치를 세 배 넘어선다:
  //     철벽 5(+75%) → +225%  ·  전투 본능 5(40%) → 120%  ·  불굴 4(+100%) → +300%
  //     강철 의지·광폭화 → 받는 피해 **126% 감소**(= 무적)
  //   그래서 두 장을 낀 순간 방어가 공격이 되고 아무것도 아프지 않게 된다.
  //   설계값은 "전설 한 장이면 두 배" 였으므로, 더하지 않고 **가장 큰 것 하나**만 세고
  //   그 위에 천장(SKILL_POWER_CAP)을 둔다.
  const skillPower = 1 + Math.min(BALANCE.SKILL_POWER_CAP, gear.mods.skillPower || 0);
  for (const [id, rank] of Object.entries(state.player.skills || {})) {
    ...
    addSteps(mods, def.steps, rank * skillPower);
    sources.push({
