...
import { NetSystem, EMOTE_MS } from './systems/NetSystem.js';
...
import * as Gamble from './systems/GambleSystem.js';
...
import { GamblePanel } from './ui/GamblePanel.js';
import { ClassPickPanel } from './ui/ClassPickPanel.js';
...
import { EmotePicker } from './ui/EmotePicker.js';
...
const DATA_FILES = {
  ...
  // 0.59 — 만들어 받은 배경 음악 표. 그림 manifest 와 섞지 않는다(audio.json 주석 참고).
...
/** 세이브가 없을 때의 초기 상태. */
/** 서버가 준 시즌 알림에서 **번호**만 꺼낸다. 숫자든 {season,last} 든 받는다. */
function seasonOf(notice) {
  if (!notice) return null;
  return typeof notice === 'object' ? (notice.season || null) : notice;
}

/** 그 알림에 딸린 **지난 시즌 요약**. 옛 서버면 없다. */
function lastOf(notice) {
  return notice && typeof notice === 'object' ? (notice.last || null) : null;
}
...
function createInitialState(db, playerName, classId) {
  ...
  const state = {
    ...
    buffs: [],
    // 건너뛴 전투 시간의 빚(ms). 저장할 때 서버로 보내고 0 으로 돌아간다.
    // ⚠ 세이브에 넣지 않는다 — 이번 접속에만 있는 값이다(serializeSave 도 안 담는다).
    skipOwedMs: 0,
...
export async function startGame(providedDb = null) {
  ...
  // 모두가 함께 보는 공지 — 화면 맨 위 가운데 (0.61).
  const noticeBanner = new NoticeBanner({ root: document.getElementById('notice-banner') });
  bus.on('net:notice', (msg) => noticeBanner.show(msg && msg.text, msg && msg.kind));

  // 이모티콘 — 단축칸 옆 단추. 고르면 내 머리 위에 뜨고 남에게도 간다 (0.62).
  const emotePicker = new EmotePicker({ bus, root: document.getElementById('emote-picker') });

  const sound = new Sound(() => loginSettings);
  // 만들어 받은 곡들. 없으면 Sound 가 예전처럼 그 자리에서 하프를 뜯는다.
  sound.setTracks((db.audio && db.audio['곡']) || {});
  // ⚠ 브라우저는 **사람이 한 번 누르기 전에는** 소리를 못 내게 막는다.
  //   그래서 여기서 바로 켜지 않고, 접속 화면의 아무 조작에서나 열어 준다.
  //
  // ⚠ 0.63 — 이 손잡이는 **접속 화면에서만** 산다. 반드시 떼어 낸다.
  //
  //   그 전에는 window 의 keydown 에 걸어 두고 **한 번도 떼지 않았다.**
  //   그래서 게임에 들어가 걷기 시작하면, 화살표를 누를 때마다 wake 가 불려
  //   접속 화면 곡이 다시 켜졌다 — 들판에 서 있는데 로그인 음악이 흐른다.
  //   (playBgm 은 '같은 곡이면 아무것도 안 한다' 인데, 들판 곡이 흐르는 중이라
  //    '다른 곡' 으로 보고 갈아 끼웠다. 한 걸음마다 처음부터 다시 시작했다.)
  //
  //   고치는 방식이 둘 있었다 — wake 안에서 "지금 접속 화면인가" 를 묻거나,
  //   접속이 끝나면 손잡이를 떼거나. **둘 다** 한다.
  //   묻기만 하면 손잡이가 영영 남아 매 키 입력마다 헛일을 하고,
  //   떼기만 하면 떼기 전에 새는 길(다시 접속 등)이 언제든 생긴다.
  let loginAudioLive = true;
  const wake = () => {
    if (!loginAudioLive) return;                // 접속 화면을 떠났으면 아무 일도 안 한다
    if (loginSettings.music === false) return;  // 꺼 둔 사람에게는 켜지 않는다
    sound.unlock();
    sound.playBgm('login');
  };
  const loginEl = document.getElementById('login');
  loginEl.addEventListener('pointerdown', wake);
  window.addEventListener('keydown', wake);
  /** 접속 화면의 소리 손잡이를 걷는다. 게임으로 넘어갈 때 반드시 부른다. */
  const dropLoginAudio = () => {
    loginAudioLive = false;
    loginEl.removeEventListener('pointerdown', wake);
  ...
  bus.on('login:submit', async ({ mode, id, pw, nick, classId }) => {
    ...
    // 예약어는 **계정을 만들 때만** 본다 (0.69).
    // 접속할 때도 보면 진짜 운영자(admin)가 제 계정으로 못 들어온다.
    const idError = Account.validateId(id, { register: mode === 'register' });
    ...
    try {
      ...
      loginScreen.hide();
      dropLoginAudio(); // 접속 화면 곡을 되살리는 손잡이를 걷는다 (0.63)
      sound.stopBgm();
      await runGame({
        // 세상이 시작되는 자리에서 접속 화면의 소리 손잡이를 한 번 더 걷는다 (0.63).
        // (여기는 startGame 의 스코프다 — runGame 은 딴 함수라 그냥은 안 보인다)
        dropLoginAudio,
        db,
        assets,
        bus,
        ...
        //
        // 0.70.15 — 서버가 **지난 시즌 요약**을 함께 준다. 그래서 값이 두 모양이다:
        //   숫자 하나        = 옛 서버(요약 없음)
        //   { season, last } = 요약이 딸린 새 서버
        // 옛 모양도 그대로 받는다 — 서버만 먼저 올라가는 일이 흔하다.
        seasonNotice: seasonOf(result.seasonNotice),
        lastSeason: lastOf(result.seasonNotice),
...
async function runGame({ db, assets, bus, storage, account, save, viewport, sound = null,
  seasonNotice = null, lastSeason = null, dropLoginAudio = () => {} }) {
  ...
  const gamblePanel = new GamblePanel({
    bus, store, assets, root: document.getElementById('gamble-panel'),
  });
  const classPickPanel = new ClassPickPanel({
    bus, store, root: document.getElementById('classpick-panel'),
  });
  ...
  const rankPanel = new RankPanel({
    // 0.69 — 직업 마크를 그리려면 그림이 필요하다.
    bus, store, assets, root: document.getElementById('rank-panel'),
  });
  ...
  const anyPanelOpen = () =>
    inventoryPanel.open || shopPanel.open || exchangePanel.open || gamblePanel.open ||
    classPickPanel.open ||
  ...
  function changeMap(mapId, toX, toY) {
    const state = store.state;
    // 떠나는 땅을 먼저 적어 둔다 — 아래에서 "집을 드나든 것인가"를 묻는다.
    ...
    // 바닥을 **여기서** 구워 둔다 (0.70.3).
    //
    // 굽는 것은 지도마다 한 번이지만, 그 한 번이 첫 프레임 **안에서** 일어나면
    // 문을 지나는 순간 화면이 한 번 걸린다(들판이면 그 프레임에 650번을 그린다).
    // 아직 아무것도 안 그리고 있는 지금 구워 두면 첫 프레임은 붙이기만 하면 된다.
    fieldScene.prebakeGround(renderer);

    if (sound) {
      // 집 안팎을 드나드는 것은 **문**이지 순간이동이 아니다 (0.70.3).
      // 같은 '슝' 소리를 내면 가게에 들어갈 때마다 마법진을 밟는 것처럼 들린다.
      sound.sfx(map.room || (from && from.room) ? 'door' : 'portal');
   ...
   */
  let omenTold = 0;
  async function announceOmen(state, { force = false } = {}) {
    ...
    // 서버에 붙어 있으면 **외치기 전에 서버에 물어본다** (0.70.2).
    //
    // 이 예고는 여태 내 기록만 보고 외쳤다. 그런데 고룡의 몸은 서버가 들고 있다 —
    // 남이 눕혔거나 내 기록이 어긋나 있으면 "또 왔다" 는 거짓말이 된다.
    // 그 말을 믿고 절벽까지 걸어간 사람은, 한 마리 섰다가 곧바로 사라지는 것만 본다.
    // 헛걸음을 시키느니 한 번 물어보는 편이 낫다(30초에 한 번, 마을에서만).
    if (storage.isServer) {
      try {
        const sv = await storage.dragon();
        if (sv && !sv.offline && !sv.present) {
          // 서버 말이 맞다. 내 기록도 여기서 고쳐 둔다 —
          // 안 고치면 30초 뒤에 똑같은 것을 또 물어보게 된다.
          if (sv.downedAt) markDragonDowned(state, at.map, sv.since, sv.downedAt);
          omenTold = at.since; // 이번 마리는 끝났다. 다음 주기에 다시 본다.
          return;
        }
      } catch { /* 서버가 대답 못 하면 내 기록대로 외친다 */ }
      // 물어보는 사이에 마을을 떠났으면 그만둔다.
      if (store.state.map !== state.map || state.map.kind !== 'town') return;
      if (!force && omenTold === at.since) return;
    }
  ...
  function runNpcAction(def, action = null) {
    ...
    // 보석 노인 — 셋을 주고 하나를 고르거나, 돈으로 한 알을 뽑는다(0.70.14).
    if (act.type === 'gamble') return gamblePanel.show({ name: def.name });
  ...
  function applyTimedBoss(state, map) {
    ...
    if (!at.present) {
      // 아직 안 왔거나 이미 잡혔다.
      //
      // 0.64.2 — 여기도 '다음 용까지 3분 20초' 였다. HUD 는 숫자를 뺐는데
      //   띠만 세고 있으면 한 화면에서 두 말이 어긋난다. 같이 뺀다.
      fieldScene.showBanner(`${map.name} — 아무도 없는 것 같다..`);
      showDragonGone(state, map, at.nextAt, at.downed ? downedNote(state, map) : null);
  ...
  /**
   * 고룡이 없는 자리에 붙일 한 줄 (0.60).
   *
   * 세 가지를 갈라 적는다 — 같은 '비어 있음'이라도 뜻이 다르다.
   *   · 내가 눕혔다        "고룡의 비늘이 아직 뜨겁다"
   *   · 남이(딴 데서) 눕혔다 "아무것도 없지만, 곧 뭔가 나올 것 같다"
   *   · 아직 안 왔다·떠났다  null (부르는 쪽이 '아무도 없는 것 같다..' 를 적는다)
   *
   * 어떻게 가르나: **내 기록**(state.timedBoss)에 이번 주기의 downedAt 이 있으면
   * 내가 마지막 한 대를 넣은 것이다. 서버는 "눕었다" 만 알려 주므로,
   * 그것만 보면 남이 잡은 것도 내가 잡은 것처럼 읽힌다.
   *
   * ⚠ 옛 문구 '이번 마리는 이미 눕었다' 는 맞춤법이 틀렸다(눕었다 → 누웠다).
   *   고치는 김에 세 갈래로 나눴다.
   */
  function downedNote(state, map) {
    const rec = TimedBoss.recordOf(state, map.id);
    const at = TimedBoss.evaluate(state, map);
    const mine = !!(rec && rec.since === at.since && rec.downedAt);
    return mine ? '고룡의 비늘이 아직 뜨겁다' : '아무것도 없지만, 곧 뭔가 나올 것 같다';
  }

  /**
   * "이번 마리는 눕었다" 를 **내 기록에 적는다** (0.70.2).
   *
   * ── 왜 따로 두는가 ──────────────────────────────────────
   * 고룡이 눕는 길이 셋이다. 셋 다 여기로 모아야 한 군데만 고쳐도 새는 곳이 안 생긴다.
   *   ① 내가 마지막 한 대를 넣었다               (전투 결과)
   *   ② 싸움을 걸며 보낸 피해로 서버에서 눕었다   (reportDragonDamage)
   *   ③ 남이 눕힌 것을 서버에서 전해 들었다       (syncDragonFromServer)
   *
   * ⚠ 여태 ①만 적었다. 그런데 0.61 부터 피해는 **싸움을 걸 때** 먼저 보낸다.
   *   그 한 방에 눕으면 서버가 곧바로 '눕었다' 고 답하고, 그 답이 고룡을
   *   `state.monsters` 에서 빼 버린다. 그러면 싸움이 끝날 때 고룡을 못 찾아
   *   ①이 통째로 건너뛰어졌다 — **잡고도 내 기록에는 '아직 살아 있다'** 였다.
   *   그래서 마을에서 "서쪽에서 이상한 기운이" 가 다시 뜨고(방금 잡았는데!),
   *   절벽에 가면 한 마리 세웠다가 서버 답을 받고 곧바로 지웠다.
   *   (0.60 에 골드 벌금에서 똑같은 실수를 한 번 고쳤다. 같은 함정이다)
   */
  function markDragonDowned(state, mapId, since, at = Date.now()) {
    const rec = (state.timedBoss || {})[mapId];
    if (rec && rec.since === since && rec.downedAt) return false; // 이미 적혀 있다
    state.timedBoss = { ...(state.timedBoss || {}), [mapId]: { since, hp: 0, downedAt: at } };
    scheduleSave();
    return true;
  }
   ...
   * 고룡이 없는 땅에 서 있을 때.
   *
   * 0.64.2 — 예전에는 '다음 마리까지 3분 20초' 처럼 **남은 시각**을 세어 보여 줬다.
   *   그런데 그 숫자는 사람을 그 자리에 붙들어 놓기만 한다 — 초를 세며 기다리는
   *   것 말고 할 일이 없어진다. 게다가 언제 오는지를 정확히 알려 주면
   *   '들렀다 아니면 말고' 가 아니라 '시계를 보고 오는 곳' 이 된다.
   *   비어 있다는 사실만 적고 숫자는 뺀다.
   *   (waitText 는 다른 데서 쓰므로 그대로 둔다)
   */
   ...
   */
  async function reportDragonDamage(damage, atMap = store.state.map) {
    ...
    try {
      ...
      if (res.downed) {
        ...
        const state = store.state;
        // ⚠ **내 기록에도 적는다** (0.70.2). 이걸 빠뜨려서, 싸움을 걸며 보낸
        //   피해로 눕은 경우 잡고도 '아직 살아 있다' 로 남았다(markDragonDowned 참고).
        //   자리를 비우는 것과 같은 자리에서 함께 적어야 다시 새지 않는다.
        if (atMap && TimedBoss.timedBossOf(atMap)) {
          markDragonDowned(state, atMap.id, TimedBoss.evaluate(state, atMap).since);
        }
        ...
        if (state.dragonHud && state.dragonHud.show) {
          // 내가 마지막 한 대를 넣었다.
          showDragonGone(state, state.map, res.nextAt, '고룡의 비늘이 아직 뜨겁다');
        }
        store.notify();
  ...
  async function syncDragonFromServer(state, map, dragon, fullHp) {
    ...
    try {
      ...
      if (!at.present || at.hp <= 0) {
        ...
        //
        // ⚠ **내 기록도 여기서 서버에 맞춘다** (0.70.2).
        //   안 맞추면 절벽을 나서는 순간 내 기록은 다시 '아직 살아 있다' 라고 말하고,
        //   마을에서 헛 예고가 뜨고, 다시 절벽에 오면 또 한 마리 세웠다 지운다.
        //   이 자리가 서버 말을 듣는 유일한 곳이니 여기서 적는 것이 맞다.
        // ⚠ 주기 번호(since)는 **내 시계로 센 것**을 쓴다. 서버가 준 값이 아니다.
        //   내 쪽의 다른 판단(evaluate·downedNote)이 전부 내 시계를 쓰기 때문이다.
        //   둘이 어긋나면(서버 설정을 바꿔 두면 그럴 수 있다) 적어 놓고도 못 알아본다.
        if (at.downedAt && TimedBoss.timedBossOf(map)) {
          markDragonDowned(state, map.id, TimedBoss.evaluate(state, map).since, at.downedAt);
        }
        if (dragon) {
        ...
        fieldScene.showBanner(`${map.name} — 아무도 없는 것 같다..`);
        showDragonGone(state, map, at.nextAt, at.downedAt ? downedNote(state, map) : null);
        store.notify();
        return;
      }
  ...
  bus.on('battle:request', async ({ monsterUid }) => {
    ...
    // ── 고룡은 **싸움을 걸기 직전에** 서버 체력을 다시 받는다 (0.61) ──
    //
    // 고룡의 몸은 서버가 하나로 들고 있고 여럿이 함께 깎는다. 그런데 그 값을
    // 땅에 들어설 때 한 번(applyTimedBoss) 과 12초마다 한 번만 받아 왔다.
    // 그 사이에 남이 때린 만큼은 내 화면에 없다 —
    // 그래서 **한 사람이 깎아 둔 용을 이어서 치는데 풀피로** 시작하곤 했다.
    // (혼자 시험하면 안 보인다. 깎아 두는 사람이 없으니까.)
    //
    // 여기서 한 번 더 묻는 이유: '들어설 때' 와 '붙을 때' 사이가 사람마다 다르다.
    // 걸어가는 동안, 창을 열어 둔 동안, 딴 놈을 잡는 동안 값은 계속 낡는다.
    // 붙는 순간에 물으면 그 틈이 아예 없어진다.
    //
    // 답이 늦거나 안 오면 그냥 있는 값으로 붙는다 — 싸움을 막지는 않는다.
    if (firstActor.timedBoss && storage.isServer) {
      try {
        const at = await Promise.race([
          storage.dragon(),
          new Promise((done) => setTimeout(() => done(null), 1200)),
        ]);
        // 기다리는 사이에 딴 데로 갔거나 이미 다른 전투가 시작됐으면 버린다.
        if (store.state !== state || scenes.current === battleScene) return;
        if (!firstActor.alive) return;
        if (at && !at.offline && at.present && at.hp > 0) {
          firstActor.carriedHp = Math.max(1, Math.min(firstActor.fullHp || at.maxHp, at.hp));
          firstActor.fullHp = at.maxHp || firstActor.fullHp;
          firstActor.serverHp = true;
          showDragonHud(state, state.map, firstActor);
        }
      } catch { /* 못 물어봐도 싸움은 시작한다 */ }
    }
    ...
    for (const defId of foes) {
      ...
      for (const q of opened) {
        // 여는 그 순간의 눈금을 찍는다 — 사냥 의뢰라면 이 뒤에 잡은 것만 센다(0.59).
        Quests.openQuest(state, q);
    ...
    // 무기 갈래 — 무기에 걸린 직업 패시브(사냥꾼의 선제 공격)가 이것을 본다.
    //
    // ⚠ 조건을 여기서 판단하지 않는다. **갈래만 실어 보내고** 켜고 끄는 것은
    //   CombatSystem 한 곳에서 한다. 그래야 게임과 측정 도구가 같은 사람을 잰다.
    playerCombatant.weaponShape = Skills.weaponShape(state);
    ...
    const monsterCombatants = defs.map((d, i) => {
      ...
      // ── 분노 (0.70.3에 고침) ────────────────────────────────
      //
      // ⚠ **표의 '분노' 칸이 전투에 한 번도 닿은 적이 없었다.**
      //
      //   CombatSystem 은 `m.rage` 를 읽는다(휘두를 때마다 공격력이 그만큼 붙는다).
      //   그런데 그 값을 실어 나르는 toCombatant 는 hp·atk·def·spd·crit 다섯 개만
      //   담아 준다 — rage 는 스탯이 아니라서 조용히 빠졌다. 그래서 표에 0.1 이라
      //   적어 두어도 전투에서는 늘 0 이었고, 시트에서 그 칸을 올려도 아무 일도
      //   일어나지 않았다. 고룡 둘이 특히 그랬다 — 오래 끄는 상대라 분노가
      //   값어치의 절반인데, 그 절반이 통째로 없었다.
      //
      //   toCombatant 에 넣지 않고 여기서 얹는 이유: 그 함수는 사람도 같이 쓴다.
      //   사람에게는 분노가 없다(있으면 안 된다).
      c.rage = Number(d.rage) > 0 ? Number(d.rage) : 0;
      // 받는 마법 피해 감소 (0.70.14). rage 와 같은 이유로 여기서 얹는다 —
      // 스탯이 아니라 보정이라 toCombatant 가 담아 주지 않는다.
      // 고룡처럼 **마법으로 때리는 상대**에게 주면, 마법사가 타고난 마법 관통으로
      // 혼자 앞서 나가던 자리를 되돌릴 수 있다(표의 '마법저항' 칸).
      c.magicResist = Number(d.magicResist) > 0 ? Number(d.magicResist) : 0;
    ...
    pendingBattle = {
      ...
      dragonReported: 0,
      // 이번 판을 시작할 때 고룡의 몸(0.70.2).
      // 싸우는 동안 고룡이 목록에서 빠질 수 있어서, 끝난 뒤에 물어볼 수가 없다.
      dragonCarried: firstActor.timedBoss ? Math.round(firstActor.carriedHp || 0) : 0,
    };

    // ── 고룡에게 준 피해는 **싸움이 시작될 때** 알린다 (0.61) ──
    //
    // 그 전에는 싸움이 **끝난 뒤**에 알렸다. 고룡전은 길다. 그동안 서버의 체력은
    // 한 칸도 안 줄어든다 — 그래서 "한 명이 잡고 있는데 다른 사람이 이어서 붙으면
    // 깎인 피가 아니라 풀피로 시작" 했다. 둘이 같이 때리는 것이 아니라
    // **각자 한 판씩 번갈아** 때리는 꼴이었다.
    //
    // 그럴 필요가 없다: 이 싸움의 결과는 **이미 다 계산돼 있다**(simulateBattle).
    // 화면은 그것을 되감아 보여 줄 뿐이고, 사람이 도중에 끼어들 수도 없다
    // (물약도 계산 안에서 마신다 — 0.58 에서 배속·스킵으로 확인했다).
    // 그러니 지금 알려도 값이 달라지지 않는다.
    //
    // 창을 닫아도 그 피해는 남는다 — 용이 실제로 맞은 것이니 그게 맞다.
    if (firstActor.timedBoss) {
      const left = Math.max(0, result.monstersHp[0] ?? 0);
      const dealt = Math.max(0, Math.round((firstActor.carriedHp || 0) - left));
      if (dealt > 0) {
        pendingBattle.dragonReported = dealt;
  ...
  // 0.70.17 — 건너뛴 전투만큼 **기록 시계에 빚을 진다.**
  //
  // 스킵·빨리감기는 '빨리 보는 것' 이지 '빨리 끝내는 것' 이 아니어야 한다.
  // 안 그러면 타임어택 표가 "누가 스킵을 잘 누르나" 가 된다.
  // 여기서는 쌓아만 두고, 서버로 보내는 것은 saveNow 가 한다(저장과 같은 편에 실어 보낸다).
  bus.on('battle:timeOwed', ({ ms }) => {
    if (!(ms > 0)) return;
    store.state.skipOwedMs = (store.state.skipOwedMs || 0) + ms;
  });
  ...
  bus.on('battle:finished', () => {
    ...
    // ⚠ 0.70.2 — 잣대를 "**이번 판에 누구와 붙었나**" 로 바꿨다.
    //
    //   그 전에는 `state.monsters` 에서 고룡을 지금 다시 찾아, 못 찾으면
    //   이 블록을 통째로 건너뛰었다. 그런데 싸우는 동안 고룡이 목록에서 빠질 수 있다 —
    //   싸움을 걸며 보낸 피해로 서버에서 눕거나(reportDragonDamage),
    //   12초마다 도는 dragonSync 가 "남이 눕혔다" 는 답을 받거나.
    //   둘 다 **잡았을 때** 일어나는 일이라, 하필 잡은 판에서만 기록이 안 남았다.
    //   (0.60 에 골드 벌금에서 같은 실수를 고쳤다. 잣대도 그때와 같은 것을 쓴다)
    const foughtTimed = !!(timed && (pendingBattle.monsterDefIds || []).includes(timed.monster));
    if (foughtTimed) {
      const left = Math.max(0, result.monstersHp[0] ?? 0);
      // 이번 판을 시작할 때의 몸. 고룡이 목록에서 빠졌어도 알 수 있어야 하므로
      // 싸움을 걸 때 적어 둔 값을 쓴다(pendingBattle.dragonCarried).
      const carried = dragon ? (dragon.carriedHp || 0) : (pendingBattle.dragonCarried || 0);
      const dealt = Math.max(0, carried - left);
      ...
      //
      // ⚠ 0.61 — 보통은 **싸움이 시작될 때 이미 알렸다**(위 battle:request 참고).
      //   여기서 또 보내면 두 번 깎인다. 그때 못 보낸 몫만 보낸다.
      const owed = Math.max(0, dealt - (pendingBattle.dragonReported || 0));
      if (owed > 0) reportDragonDamage(owed);
      if (left > 0) {
        ...
        // 목록에서 이미 빠졌다면(남이 눕혔다) 세울 것이 없다. 서버 동기화가 마무리한다.
        if (dragon) {
        ...
        state.monsters = state.monsters.filter((m) => m !== dragon);
        showDragonGone(state, state.map, TimedBoss.evaluate(state, state.map).nextAt, '고룡의 비늘이 아직 뜨겁다');
      }
    }
    ...
    if (result.winner === 'monster') {
      ...
      //
      // ⚠ 0.60 — 잣대를 "**이번 판에서 누구와 붙었나**" 로 바꿨다.
      //
      //   그 전에는 `state.monsters` 에서 고룡을 **지금 다시 찾아** 판단했다.
      //   그런데 12초마다 도는 dragonSync 가, 서버가 "그 마리는 이미 눕었다" 고 하면
      //   그 자리에서 고룡을 목록에서 빼 버린다(syncDragonFromServer).
      //   고룡전은 12초보다 훨씬 길다. 그래서 **같이 접속한 사람이 그 사이에 눕히면**,
      //   내 화면에서는 아직 싸우고 있는데 목록에서는 이미 사라져 있고 —
      //   전투가 끝나는 순간 "고룡이 없다" 로 읽혀 골드를 10% 뜯겼다.
      //   혼자 할 때는 아무도 안 눕히므로 한 번도 안 생기는 일이었다.
      //
      //   싸움이 끝난 뒤에 물어야 할 것은 "그 놈이 아직 서 있나" 가 아니라
      //   "내가 방금 그 놈과 붙었나" 다. 그건 pendingBattle 에 이미 적혀 있고,
      //   누가 무엇을 하든 바뀌지 않는다.
      const foughtIds = pendingBattle.monsterDefIds || [];
      const foughtTimedBoss = !!(timed && foughtIds.includes(timed.monster));
      // 상처가 남는 보스(고룡의 둥지) — 그 땅의 보스와 붙었으면 값을 안 치른다.
      const foughtKeepBoss = !!(state.map.bossKeepHp
        && (pendingBattle.isBoss || (state.map.boss && foughtIds.includes(state.map.boss))));
      // 매직 투구 — 쓰고 있으면 쓰러져도 지갑은 그대로다 (0.63).
      //
      // 왜 이 투구에 붙였나: 시야·은신은 **지하감옥에서만** 값을 하는데, 그 땅에 갈
      // 무렵이면 이미 룬 투구를 쓰고 있다. 게다가 용린 투구는 상점에서 판다.
      // 그래서 매직 투구는 사실상 아무도 안 썼다 — 값이 없는 물건이었다.
      // 어디서나 값을 하는 것을 하나 준다. 방어는 여전히 얇으니 맞바꾸는 셈이다.
      const keepGold = (computePlayerStats(state).mods || {}).keepGoldOnDeath > 0;
      const free = (timed && timed.noPenalty && foughtTimedBoss) || foughtKeepBoss;
      if (!free && keepGold) {
        // 값은 안 치르지만 **쓰러진 것은 쓰러진 것**이다 — HP 는 절반으로 떨어진다.
        const stats = computePlayerStats(state);
        state.player.hp = Math.max(1, Math.floor(stats.hp * 0.5));
        summary.win = false;
        summary.note = '쓰러졌다... 매직 투구가 지갑을 지켰다.';
      } else if (free) {
        const stats = computePlayerStats(state);
        state.player.hp = Math.max(1, Math.floor(stats.hp * 0.5));
        summary.win = false;
  ...
  // ── 이모티콘 (0.62) ──────────────────────────────────────
  //
  // 고른 것을 내 상태에 얹고 그물로도 보낸다. 내 화면에 바로 뜨는 것이 중요하다 —
  // 서버를 한 바퀴 돌고 오면 누른 느낌과 뜨는 시각이 어긋나서 안 눌린 줄 안다.
  bus.on('ui:emote', ({ icon } = {}) => {
    if (!icon) return;
    const st = store.state;
    st.player.emote = { icon, at: Date.now() };
    net.sendEmote(icon);
  });
  // 시간이 지나면 내 것도 사라진다(남의 것은 NetSystem 이 지운다).
  setInterval(() => {
    const st = store.state;
    if (st && st.player && st.player.emote && Date.now() - st.player.emote.at > EMOTE_MS) {
      st.player.emote = null;
    }
  }, 500);
  ...
  function useConsumable(itemId) {
    ...
    if (def.use.hp) {
      ...
      // 마시는 소리 (0.70.3). 여태는 토스트가 내는 '동전 줍는 소리' 를 빌려 썼다.
      if (sound) sound.sfx('drink');
      bus.emit('toast', {
    ...
    if (def.use.buff) {
      ...
      if (sound && !def.use.hp) sound.sfx('drink');
  ...
  bus.on('ui:socket', ({ uid, gemId, slot = null }) => {
    const state = store.state;
    // slot 이 있으면 **그 홈에 덮어 박는다**(0.70.14). 그 자리 보석은 사라진다.
    const res = Affix.socketGem(state, uid, gemId, slot);
    if (!res.ok) return bus.emit('toast', { text: res.reason, tone: 'bad' });
    ...
    // 덮어 박았으면 무엇이 사라졌는지 말해 준다 — 값을 치른 일은 말없이 지나가면 안 된다.
    if (res.replaced) {
      bus.emit('toast', {
        text: `${res.replaced.name} 자리에 ${nm}${j} 박았다. ${res.replaced.name}은(는) 부서졌다.`,
        tone: 'rare',
      });
      return;
    }
  ...
  async function refreshRanks() {
    ...
    try {
      ...
      state.ranks = {
        ...
        // 지금 도전에 쓴 시간 — **서버가 잰 값**이다 (0.70.2).
        //   시계가 '접속해 있는 동안만' 흐르게 바뀌었는데 그 값이 안 보이면
        //   사람은 여전히 벽시계로 짐작한다. 랭킹 창 맨 위에 한 줄로 적는다.
        //   받아 온 때(playedAt)를 함께 적어 두어야 창을 열어 둔 채로도 흘러간다.
        played: typeof res.played === 'number' ? res.played : null,
        playedAt: Date.now(),
        loading: false,
        offline: false,
      };
  ...
  function applyAdminLook() {
    ...
    p.rawSprite = true;
    // 0.70.18 — 운영자는 **세 직업의 스킬을 모두** 배울 수 있다.
    //
    // 운영자는 노는 사람이 아니라 보러 온 사람이다. 어디가 센지 어디가 약한지
    // 보려면 한 직업의 눈으로만 볼 수가 없다. classId 는 그대로 두고(몸은 하나여야 한다)
    // 배울 수 있는 목록만 연다 — SkillSystem.classSkills 가 이 표를 본다.
    p.allSkills = true;
  ...
  /**
   * 리젠 — **지금 서 있는 땅의 보스를 그 자리에서 부른다** (0.70.18).
   *
   * ── 왜 필요한가 ────────────────────────────────────────────
   * 보스 값을 고쳐 놓고 실제로 어떤지 보려면 다시 만나야 하는데,
   * 지하 5층 주인은 한 시간, 고룡은 서른 분을 기다려야 한다.
   * 기다리는 동안에는 고친 것을 못 본다.
   *
   * ── 두 갈래를 다 다룬다 ────────────────────────────────────
   *   ① 보통 보스(지하 5층 주인 · 단계 보스 · 아그라모스)
   *      죽으면 respawnTimer 가 돌고 state.bossRespawn 에 "언제 다시" 가 적힌다.
   *      시계를 0 으로 놓고 그 적어 둔 것을 지운다 — FieldScene 이 다음 틱에 세운다.
   *      ⚠ bossRespawn 을 안 지우면 지도를 나갔다 오는 순간 changeMap 이 그 표를
   *        다시 읽어서 **도로 눕는다.**
   *   ② 시각 보스(서쪽 절벽의 고룡)
   *      시계에 맞춰 나타난다. downedAt("이미 잡음")만 지우면 이번 주기 안에서는
   *      다시 선다. 주기 밖이면 시계를 이길 수 없으므로 그렇다고 말해 준다 —
   *      아무 일도 안 일어나면 고장으로 보인다.
   */
  bus.on('admin:boss-now', () => {
    if (!isAdmin) return;
    const state = store.state;
    const map = state.map || {};
    const timed = map.timedBoss && map.timedBoss.monster;
    let woke = 0;

    if (timed && state.timedBoss && state.timedBoss[map.id]) {
      delete state.timedBoss[map.id];
      applyTimedBoss(state, map);
      woke += (state.monsters || []).filter((m) => m.isBoss && m.alive).length;
    }

    for (const m of (state.monsters || [])) {
      if (!m.isBoss || m.alive) continue;
      if (state.bossRespawn) delete state.bossRespawn[m.uid];
      m.respawnTimer = 0; // FieldScene 이 다음 틱에 빈 칸을 찾아 세운다
      woke += 1;
    }

    const alive = (state.monsters || []).some((m) => m.isBoss && m.alive);
    store.notify();
    scheduleSave();
    adminPanel.render();

    if (!timed && !(state.monsters || []).some((m) => m.isBoss)) {
      return bus.emit('toast', { text: '이 땅에는 보스가 없습니다.', tone: 'info' });
    }
    if (alive && !woke) {
      return bus.emit('toast', { text: '보스가 이미 서 있습니다.', tone: 'info' });
    }
    if (!woke && timed) {
      return bus.emit('toast', {
        text: '고룡은 때가 되어야 옵니다 — 시계는 못 이깁니다.\n(잡아 둔 기록만 지웠습니다)',
        tone: 'warn',
        ms: 5000,
      });
    }
    bus.emit('toast', { text: '보스를 불렀습니다.', tone: 'rare' });
  });
  ...
  // ── 시트 '계정' 탭 (0.64) ────────────────────────────────
  //
  // 서버는 시트에 **쓸 수 없다**(읽기 전용 공개 링크로 본다). 그래서 계정을 시트에
  // 올리는 일은 사람이 복사해 붙이는 것으로 하고, 그 반대(시트에서 지운 줄)는
  // 곧바로가 아니라 **예약**으로 받는다. 여기 세 손잡이가 그 셋이다.
  const loadAcctPending = async (cancel = [], restore = []) => {
    try {
      const res = await storage.adminAccountPending(account.id, account.token, cancel, restore);
      if (!res || !res.ok) throw new Error((res && res.error) || '예약을 받지 못했습니다.');
      adminPanel.setAcctPending(res);
      return res;
    } catch (err) {
      // 못 받아도 창은 열려 있어야 한다 — 지우기 자체는 이것과 상관없이 된다.
      adminPanel.setAcctPending({ on: false, graceMs: 0, pending: [] });
      adminPanel.setStatus(err.message || '시트 계정 상태를 받지 못했습니다.', 'bad');
      return null;
    }
  };

  bus.on('admin:account-pending', () => { loadAcctPending(); });

  bus.on('admin:account-keep', async ({ id }) => {
    const res = await loadAcctPending([id]);
    if (res && (res.cancelled || []).includes(id)) {
      adminPanel.setStatus(`${id} 의 삭제 예약을 되돌렸습니다.`, 'good');
    }
  });

  // 휴지통에서 되살리기 (0.64.2).
  bus.on('admin:account-restore', async ({ id }) => {
    const res = await loadAcctPending([], [id]);
    const r = res && res.restored;
    if (r && (r.restored || []).includes(id)) {
      adminPanel.setStatus(
        `${id} 을 되살렸습니다. 캐릭터·소지품은 그대로입니다.\n${r.note || ''}`, 'good');
      // 계정 목록도 다시 받아 온다 — 안 그러면 되살린 사람이 목록에 안 보인다.
      adminPanel.setAccounts(null);
      bus.emit('admin:accounts');
    } else {
      const why = ((r && r.failed) || []).map((f) => `${f.id}(${f.reason})`).join(', ');
      adminPanel.setStatus(why || '되살리지 못했습니다.', 'bad');
    }
  });

  bus.on('admin:account-sync', async () => {
    try {
      const r = await storage.adminAccountSync(account.id, account.token);
      if (!r || !r.ok) throw new Error((r && (r.why || r.error)) || '보지 못했습니다.');
      const bits = [];
      if ((r.scheduled || []).length) bits.push(`예약 ${r.scheduled.length}개`);
      if ((r.cancelled || []).length) bits.push(`취소 ${r.cancelled.length}개`);
      if ((r.deleted || []).length) bits.push(`삭제 ${r.deleted.length}개`);
      adminPanel.setStatus(
        `시트 계정 탭 ${r.sheetCount}줄 · 서버 ${r.serverCount}개`
        + (bits.length ? ` — ${bits.join(' · ')}` : ' — 바뀐 것 없음'),
        'good'
      );
    } catch (err) {
      adminPanel.setStatus(err.message || '보지 못했습니다.', 'bad');
    }
    loadAcctPending();
  });

  // 클립보드가 막혀 있을 때 — 글을 띄워 사람이 직접 고르게 한다.
  bus.on('admin:acct-copy-fallback', ({ text }) => {
    adminPanel.setStatus(
      '이 브라우저가 복사를 막았습니다. 아래 글을 직접 골라 복사해 시트 계정 탭 A1 에 붙여 넣으세요.\n\n'
      + text,
      'bad'
    );
  });
  ...
  bus.on('admin:account-delete', async ({ ids }) => {
    try {
      ...
      adminPanel.setAccounts(null);
      bus.emit('admin:accounts');
      // 휴지통도 새로 받는다 — 방금 지운 사람이 거기 들어갔다(0.64.3).
      // 이게 없으면 "되돌릴 수 있다" 고 적어 놓고 정작 목록에는 안 보인다.
      loadAcctPending();
      adminPanel.setStatus(
  ...
  // 시즌 일정 — 언제 끝나고, 끝난 뒤 며칠 쉬었다 다음을 여나 (0.70.5).
  bus.on('admin:season-schedule', async ({ endsAt, gapDays }) => {
    try {
      const res = await storage.adminSeasonSchedule(
        account.id, account.token, endsAt, gapDays);
      if (!res || !res.ok) throw new Error((res && res.error) || '정하지 못했습니다.');
      adminPanel.setStatus(
        res.endsAt
          ? `${new Date(res.endsAt).toLocaleString()} 에 시즌이 끝납니다.`
            + (res.gapDays ? ` 다음 시즌은 ${res.gapDays}일 뒤에 열립니다.` : ' 곧바로 다음 시즌이 열립니다.')
          : '끝을 없앴습니다 — 이 시즌은 손으로 끝낼 때까지 이어집니다.',
        'good'
      );
      refreshRanks();
    } catch (err) {
      adminPanel.setStatus(err.message || '정하지 못했습니다.', 'bad');
      refreshRanks();
    }
  });
  ...
  bus.on('admin:sheet-pull', async () => {
    try {
      ...
      // 0.64 — 서버가 주는 경고를 **버리지 않는다.**
      //   시트가 한 판 뒤처졌다는 이야기가 여기로 온다. 안 보여 주면
      //   "반영했습니다" 만 뜨고, 새로 생긴 줄이 왜 없는지 알 길이 없다.
  ...
  async function submitTimeAttack(state, monsterDefId) {
    ...
    if (!online()) return;
    // 운영자는 표에 오르지 않는다 (0.70.18). 서버도 막지만, 굳이 보내지 않는다 —
    // 보내 놓고 거절당하면 알림이 한 박자 늦게 뜨고, 그 사이 화면은 올라간 줄 안다.
    // ⚠ 이건 예의지 자물쇠가 아니다. 진짜 잠금은 server.js 의 /api/rank/submit 에 있다.
    if (isAdmin) return;
    try {
      // '어디에서' 를 함께 보낸다 — 1~3위면 서버가 모두에게 알리는 글에 들어간다.
      const where = (state.map && state.map.name) || '';
      const res = await storage.submitRank(account.id, account.token, monsterDefId, where);
      if (!res || !res.ok) return;

      // 서버가 준 시간을 그대로 화면에도 쓴다(두 곳의 숫자가 갈리지 않게).
      //
      // ⚠ 표에 남는 것은 res.best 다 — 지난 도전이 더 빨랐으면 그쪽이 남는다(0.68).
      //   화면의 '내 기록'도 표와 같아야 하므로 best 를 적는다.
      state.bossFirstKill = state.bossFirstKill || {};
      state.bossFirstKill[monsterDefId] = res.best || res.ms;
      scheduleSave();

      if (res.first && res.rank) {
        const name = state.db.monsters[monsterDefId].name;
        // 기록을 못 갈아 치웠으면 그렇다고 말해 준다 (0.69).
        // 아무 말 없이 등수만 띄우면 "왜 시간이 안 바뀌지" 가 된다.
        const kept = res.beat === false;
        bus.emit('toast', {
          text: kept
            ? `${name} 처치 — 이번 ${timeText(res.ms)}\n표에는 더 빠른 ${timeText(res.best)} 가 남습니다 (${res.rank}위)`
            : `타임어택 ${res.rank}위! — ${name} ${timeText(res.ms)}`,
          tone: kept ? 'info' : 'rare',
          ms: kept ? 6000 : undefined,
        });
      }
  ...
  // ── 보석 노인 (0.70.14) ────────────────────────────────────
  //
  // 규칙은 systems/GambleSystem.js 한 곳에 있다. 여기서는 그 판정을 받아
  // 소지품을 실제로 옮기고, 무엇이 오갔는지 한 줄로 말해 준다.
  bus.on('ui:gemTrade', ({ picks, want }) => {
    const state = store.state;
    const plan = Gamble.planTrade(state, picks, want);
    if (!plan.ok) return bus.emit('toast', { text: plan.reason, tone: 'bad' });

    for (const [id, n] of plan.take) Inventory.removeItem(state, id, n);
    Inventory.addItem(state, plan.give, 1);
    store.notify();
    scheduleSave();

    const gaveText = plan.take
      .map(([id, n]) => `${(state.db.items[id] || {}).name || id}${n > 1 ? ` ${n}알` : ''}`)
      .join(' · ');
    const gotName = (state.db.items[plan.give] || {}).name || plan.give;
    bus.emit('toast', { text: `${gaveText} → ${gotName}`, tone: 'rare' });
  });

  bus.on('ui:gemBuy', () => {
    const state = store.state;
    const check = Gamble.canBuy(state);
    if (!check.ok) return bus.emit('toast', { text: check.reason, tone: 'bad' });
    const rolled = Gamble.rollGem(state, rng);
    if (!rolled) return bus.emit('toast', { text: '팔 보석이 없습니다.', tone: 'bad' });

    state.player.gold -= check.price;
    Inventory.addItem(state, rolled.id, 1);
    store.notify();
    scheduleSave();
    bus.emit('toast', {
      text: `노인이 천을 들췄다 — ${rolled.name} (${rolled.effect})`,
      tone: 'rare',
    });
  });
  ...
  bus.on('ui:save-import', ({ code, done }) => {
    ...
    try {
      ...
      // 사냥 의뢰의 눈금. 옛 저장본에는 없으므로 **불러오는 지금** 찍는다(0.59).
      Quests.syncOpenQuests(store.state);
  ...
  function unstick({ quiet = false } = {}) {
    ...
    for (const panel of Object.values({
      inventoryPanel, shopPanel, exchangePanel, gamblePanel, classPickPanel,
      characterPanel, questPanel,
   ...
   * **처음부터 다시 도전** (0.68).
   *
   * 계정은 그대로 두고 캐릭터만 갓 만든 상태로 되돌린다.
   *
   * 순서가 중요하다. **서버가 먼저다.**
   *   ① 서버에 되돌려 달라고 한다 — 시계(bornAt)·처음 잡은 기록·우편함·이벤트 표시
   *   ② 그게 성공한 뒤에야 화면의 캐릭터를 새로 만든다
   *   ③ 새 상태를 곧바로 저장한다
   *
   * 반대로 하면(화면 먼저) 서버가 거절했을 때 **캐릭터만 사라진다** —
   * 옛 세이브가 서버에 그대로 있으니 다음 접속에 되살아나서, 사람 눈에는
   * "지웠는데 도로 생겼다" 로 보인다. 그 혼란이 가장 나쁘다.
   *
   * ⚠ 상태 객체를 **새것으로 바꾸지 않고 속을 갈아 끼운다.** 씬·창·시스템이
   *   모두 같은 객체를 들고 있어서, 새 객체로 바꾸면 그것들이 옛 객체를 계속
   *   본다(화면은 옛 캐릭터, 저장은 새 캐릭터가 된다).
   */
  // 새 시즌 첫 접속에서 고른 직업으로 몸을 새로 세운다 (0.70.14).
  //
  // ⚠ storage.restart 를 부르지 않는다. 시즌 초기화는 **서버가 이미** 했고
  //   (세이브가 비어 있다), 여기서 또 부르면 서버가 찍어 둔 출발 시각이
  //   한 번 더 밀려 시계가 두 번 0 이 된다.
  bus.on('ui:pickClass', async ({ classId } = {}) => {
    const state = store.state;
    const want = db.classes.list[classId] ? classId : state.player.classId;
    const fresh = createInitialState(db, account.name, want);
    const settings = state.settings;
    const accountId = state.accountId;
    const bornAt = state.bornAt;
    for (const k of Object.keys(state)) delete state[k];
    Object.assign(state, fresh, { settings, accountId });
    if (bornAt) state.bornAt = bornAt;
    state.needClassPick = 0; // 골랐으니 다시 묻지 않는다

    // ⚠ 0.70.18 — **운영자는 초기화를 지나도 운영자다.**
    //
    //   위에서 state 를 통째로 비우고 새 캐릭터로 덮었다. 그러면 운영자 모습도
    //   투명도 전 직업 스킬도 함께 날아간다 — 시즌이 바뀔 때마다 운영자가
    //   용사 얼굴로 마을 한가운데 서 있게 된다(실제로 그랬다).
    //   운영자인지 아닌지는 서버가 정한 것이고 시즌과 상관이 없으므로, 다시 입힌다.
    if (isAdmin) applyAdminLook();

    classPickPanel.close();
    if (scenes.current !== fieldScene) scenes.replace(fieldScene);
    fieldScene.active = true;
    fieldScene.controls = true;
    changeMap(db.player.startMap, db.player.startTile.x, db.player.startTile.y);
    state.player.hp = computePlayerStats(state).hp;
    store.notify();
    try { await saveNow(); } catch { /* 저장이 늦어도 몸은 이미 바뀌었다 */ }
    if (sound) sound.playBgm(bgmOf(state.map));
    bus.emit('toast', {
      text: `${db.classes.list[want].name}의 길을 골랐습니다.\n시계가 0초부터 다시 갑니다.`,
      tone: 'rare',
      ms: 5000,
    });
  });

  bus.on('ui:restart', async ({ classId } = {}) => {
    // 되돌리는 동안 저장이 끼어들면 **옛 캐릭터가 새 세이브를 덮어쓴다.**
    savingStopped = true;
    try {
      const res = await storage.restart(account.id, account.token);
      if (!res || res.ok === false) throw new Error(res && res.reason ? res.reason : '되돌리지 못했습니다.');

      const want = db.classes.list[classId] ? classId : store.state.player.classId;
      const fresh = createInitialState(db, account.name, want);

      const state = store.state;
      // 기기에 딸린 것은 그대로 둔다 — 소리 크기나 화면 방향까지 되돌릴 이유는 없다.
      const settings = state.settings;
      const accountId = state.accountId;
      for (const k of Object.keys(state)) delete state[k];
      Object.assign(state, fresh, { settings, accountId });
      // 서버가 찍은 출발 시각을 그대로 쓴다 — 화면의 시계와 랭킹의 잣대가
      // 어긋나면 "0초부터 다시" 가 거짓말이 된다.
      const born = Date.parse(res.bornAt || '');
      if (Number.isFinite(born) && born > 0) state.bornAt = born;

      // ⚠ 운영자는 '다시 도전' 을 지나도 운영자다 (0.70.18 — pickClass 와 같은 까닭).
      //   방금 state 를 통째로 비웠으므로 모습·투명·전 직업 스킬을 다시 입힌다.
      if (isAdmin) applyAdminLook();

      // 열려 있던 창을 전부 내린다(설정 창 자신도 — 방금 그 안에서 눌렀다).
      for (const panel of [
        inventoryPanel, shopPanel, exchangePanel, gamblePanel, classPickPanel,
        characterPanel, questPanel,
        settingsPanel, waypointPanel, mailPanel, rankPanel, adminPanel,
      ...
      // 다음에 설정 창을 열었을 때 '준비' 화면이 아니라 처음 모습으로 돌아오게.
      settingsPanel.restartOpen = false;
      if (dialogueBox.open && dialogueBox.close) dialogueBox.close();
      if (scenes.current !== fieldScene) scenes.replace(fieldScene);
      fieldScene.active = true;
      fieldScene.controls = true;
      changeMap(db.player.startMap, db.player.startTile.x, db.player.startTile.y);
      state.player.hp = computePlayerStats(state).hp;
      fieldScene.showBanner(state.map.name);
      ...
      if (sound) sound.playBgm(bgmOf(state.map));
      bus.emit('toast', {
        text: `✦ 다시 도전 ✦\n${account.name}님, 시계가 0초부터 다시 갑니다.\n행운을 빕니다.`,
        tone: 'rare',
        ms: 6000,
      });
    } catch (err) {
  ...
  // 시즌 일정만 바뀐 것 (0.70.5) — 마감 시각이나 쉬는 날수를 운영자가 고쳤다.
  //
  // ⚠ 시즌이 넘어간 것이 **아니다.** 저장을 끄면 안 된다(위와 헷갈리기 쉽다).
  //   랭킹 창이 열려 있으면 남은 시간이 곧바로 새 값으로 바뀌게만 한다.
  bus.on('net:season-schedule', (msg) => {
    const st = store.state;
    if (!st.ranks || !msg || !msg.season) return;
    st.ranks = { ...st.ranks, season: { ...(st.ranks.season || {}), ...msg.season } };
    store.notify();
  });
  ...
  bus.on('battle:result', (s) => {
    if (!sound) return;
    // 0.59 — 쓰러졌을 때 흐르는 곡. 이겼거나 비겼을 때는 걸지 않는다.
    //
    // 되풀이하지 않는다(audio.json 의 loop:false). 결과창을 닫으면 battle:close 가
    // 그 땅의 곡으로 되돌리므로, 오래 들여다보고 있어도 한 번으로 끝난다.
    const lost = s && !s.win && !s.draw;
    if (lost) sound.playBgm('death');
  ...
  async function saveNow() {
    ...
    saving = true;
    // 건너뛴 전투 시간 — 보내기 **전에** 덜어 낸다.
    //
    // ⚠ 보낸 뒤에 0 으로 만들면, 보내는 동안 끝난 전투의 몫이 같이 지워진다.
    //   먼저 덜어 내고, 못 보냈으면 도로 넣는다. 그래야 한 판도 안 빠지고,
    //   같은 판이 두 번 세어지지도 않는다.
    const owed = Math.round(store.state.skipOwedMs || 0);
    store.state.skipOwedMs = 0;
    try {
      await storage.save(account.id, account.token, Account.serializeSave(store.state), owed);
      bus.emit('save:done');
    } catch (err) {
      store.state.skipOwedMs = (store.state.skipOwedMs || 0) + owed; // 못 보냈으니 도로
  ...
  // 시즌이 넘어간 사이에 자고 있던 사람 (0.70.14 — 창을 띄운다).
  //
  // 예전에는 한 줄짜리 알림만 띄웠다. 그런데 시즌이 끝나면 세이브가 통째로
  // 비워지므로(server/world.js 의 resetAllSaves) **이 사람은 갓 만든 캐릭터**다.
  // 그 자리에서 직업만 지난 시즌 것을 물려받는 것은 "처음 만든 것과 같다" 가 아니다.
  // 그래서 마을에 선 채로 직업을 다시 고르게 한다 — 이 시즌을 어떻게 걸을지부터 정한다.
  //
  // ⚠ 세이브를 되살리는 아래 블록보다 **뒤**에서 띄운다. 세이브가 있으면
  //   (=시즌이 안 넘어갔으면) 애초에 seasonNotice 가 없다.
  if (seasonNotice) {
    // 알림은 한 번뿐이므로 **세이브에도 적어 둔다** — 고르기 전에 창을 껐다 켜도
    // 다시 묻는다. 이 표시는 직업을 고르는 순간 지워진다.
    store.state.needClassPick = 1;
    store.state.seasonName = seasonNotice;
    // 지난 시즌 요약도 함께 남긴다 — 고르기 전에 껐다 켜도 안내가 그대로 뜬다.
    if (lastSeason) store.state.lastSeason = lastSeason;
    saveNow().catch(() => {});
  }

  /**
   * 이번 시즌의 직업을 아직 안 골랐으면 마을에서 한 번 묻는다 (0.70.14).
   * 세이브를 되살린 **뒤**에 부른다 — 되살리는 쪽이 needClassPick 를 덮어쓰기 때문이다.
   */
  function askClassIfNeeded() {
    if (!store.state.needClassPick) return;
    setTimeout(() => {
      if (store.state.needClassPick) {
        classPickPanel.show({
          season: store.state.seasonName || null,
          last: store.state.lastSeason || null,
        });
      }
    }, 700);
  }
  ...
  // 이번 시즌의 직업을 아직 안 골랐으면 여기서 묻는다.
  // (세이브가 있든 없든 물어야 한다 — 고르기 전에 껐다 켠 사람도 세이브가 생긴다)
  askClassIfNeeded();
  ...
  bus.on('input:action', (action) => {
    ...
    // 직업 고르기는 **닫을 수 없다**. 고르는 것이 유일한 출구다 —
    // 안 고르고 나가면 지난 시즌의 직업으로 이번 시즌을 걷게 된다.
    if (classPickPanel.open) return;
    if (shopPanel.open) {
    ...
    if (questPanel.open) {
      if (action === 'cancel' || action === 'inventory' || action === 'quest') questPanel.close();
      return;
    }
    ...
    // 0.69 — 손이 자주 가는 셋을 키로 뺐다.
    //   Q 의뢰 · T 마을 귀환 · ? 조작법
    // 어느 것도 전투 중에는 안 듣는다. 전투는 이미 짜인 판을 보여 주는 자리라,
    // 거기서 창이 열리면 "지금 무슨 일이 일어나고 있나" 를 놓친다.
    if (action === 'quest') {
      if (scenes.current !== battleScene) questPanel.show();
      return;
    }
    if (action === 'town') {
      // 귀환은 창이 아니라 **시전**이다. 되는지 안 되는지는 그쪽이 판단하고
      // 안 될 때는 까닭까지 말해 준다(ui:return-town 참고).
      if (scenes.current !== battleScene) bus.emit('ui:return-town');
      return;
    }
    if (action === 'help') {
      hud.toggleHelp();
  ...
  const loop = new GameLoop({
    update: (dt) => {
      ...
      net.update(dt, {
        ...
        // 0.70.18 — 뛰어오르는 중인지도 보낸다.
        // 안 보내면 남의 화면에서 친구가 술통 위로 **순간이동**한다
        // (몸이 뜨는 그림 없이 한 칸 위에 나타난다).
        jumping: !!state.player.jumping,
      ...
      if (uiAcc > 500) {
        uiAcc = 0;
        // 접속 인원은 **어느 땅에 있든** 다 센다(0.61).
        // 같은 맵만 세면 친구가 옆 들판으로 넘어간 순간 '0명' 이 되어,
        // 서버가 끊긴 것과 구별이 안 된다.
        hud.setNet(net.peerList().length, net.online, !!wsUrl,
          net.peersOnMap(state.map.id).length);
        if ((state.buffs || []).length) hud.render(); // 남은 시간 갱신
      }
    },
    render: () => {
      scenes.render(renderer);
      // 초당 프레임 (0.70.1). 설정에서 껐으면 이 안에서 바로 되돌아간다.
      // 화면을 다 그린 **뒤**에 센다 — 그려야 한 프레임이다.
  ...
  input.attach();
  pointer.attach();
  // 두 번째 빗장 (0.63) — 여기까지 왔으면 세상이 도는 것이다.
  // 접속 화면을 지나오는 길이 하나 더 생기더라도 그 곡이 되살아나지 않게,
  // 손잡이를 걷는 일을 **세상이 시작되는 자리에서도** 한 번 더 한다.
  dropLoginAudio();
  loop.start();
  hud.render();
  quickSlots.render();
